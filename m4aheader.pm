...
INIT {
	...
	$IsParent{$_} = 0 for qw/moov trak udta mdia minf stbl ilst moof traf/;
...
sub ParseAtomTree {
	...
	while (read($fh, $buffer, 8) == 8) {
		...
		if ($length < $headsize) {
			warn "error atom '$name' has an invalid size of $length bytes";
		...
		if (@left && $length > $left[-1]) {
			warn "Premature end of atom, parent '$parents[-1]' has only "
				. $left[-1]
				. " bytes left, but child '$name' says it is $length bytes long\n";
		...
		if (defined $isparent) {
			...
			# for atom 'meta'
			if (my $offset = $isparent) {
				seek $fh, $offset, 1;
				$left[-1] -= $offset;
			}

			$otherkey = undef;
		} elsif (@parents > 1 && $parents[-2] eq 'ilst') {
			#in moov.udta.meta.ilst.XXXX
			my $key = $parents[-1];
			read($fh, my ($data), $datalength);
			if ($key eq '----') {
				# freeform tag
				unless ($otherkey) {
					push @$ilst_data, $key, $otherkey = {};
				}
				$otherkey->{$name} = $data;
			} elsif ($name eq 'data') {
			...
			if (($type eq 'mp4a' || $type eq 'alac') && !$info{traktype}) {
				...
				# doesn't seem to work for alac files, will use calculated bitrate instead
				$info{bitrate} = unpack 'N', $1
					if $buffer =~ m/^.{48}esds.{4}\x03(?:\x80\x80\x80)?.{4}\x04(?:\x80\x80\x80)?.{10}(.{4})/s;
			}

			$info{traktype} ||= $type;
		} elsif ($name eq 'cmov') {
			warn "Compressed moov atom found, unsupported";

			return;
		} else {
			if ($name eq 'mdat') {
				$info{audiodatasize} += $datalength;
			} elsif ($name =~ m/^stco|^co64|^tfhd/) {
				push @$stco, $name, $offset - $poffset[0];
				$self->{nofullrewrite} = 1 unless $name eq 'stco';
			}

			unless (seek $fh, $datalength, 1) {
				warn $!;
	...
	if (!$info{audiodatasize}) {
		warn "Error reading m4a file : no mdat atom found\n";
	...
	while (@ilst) {
		...
		if ($flag == 1) {
			...
			$val = unpack 'n', $val;
		} elsif ($key =~ m/^cpil$|^pgap$|^pcst$/) {
			$val = unpack 'C', $val;
		} else {
			$key = 'Unknown tag with flag=' . $flag . ' and key=' . $key;
		}
...
sub write_file {
	...
	if ($free >= 2**32) {
		warn "file too big, size>4GB are not supported\n";
		...
		# too much or not enough padding -> set padding to 1024 and resize
	...
	# destroy the object to make sure it is not reused as many of its data
	# are now invalid
	%$self = ();
