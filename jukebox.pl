...

{
	...
	*Gtk2::AboutDialog::set_url_hook = sub { }  unless *Gtk2::AboutDialog::set_url_hook{CODE};

	# for perl-Gtk2 version < 1.080~1.083
	*Gtk2::Label::set_ellipsize = sub { }  unless *Gtk2::Label::set_ellipsize{CODE};

	# for perl-Gtk2 version < 1.180,  pango < 1.20
	*Gtk2::Pango::Layout::set_height = sub { }  unless *Gtk2::Pango::Layout::set_height{CODE};

	# for gtk2 version < 2.9 or perl-Gtk2 < 1.131
	*Gtk2::Label::set_line_wrap_mode = sub { }  unless *Gtk2::Label::set_line_wrap_mode{CODE};

	# for gtk2 version < 2.16 or perl-Gtk2 < 1.230
	*Gtk2::Scale::add_mark = sub { }  unless *Gtk2::Scale::add_mark{CODE};

	# for gtk2 version < 2.16 or perl-Gtk2 < 1.230
	*Gtk2::ImageMenuItem::set_always_show_image = sub { }  unless *Gtk2::ImageMenuItem::set_always_show_image{CODE};
	...
	*Gtk2::Gdk::GC::set_clip_rectangle = sub {
	...
	if (eval($POSIX::VERSION) < 1.18) {
		...
		*strftime_utf8 = sub {
			$encoding  ?  Encode::decode($encoding, &strftime)  :  &strftime
		};
	}
}
...
BEGIN {
	...
	my @dirs = ($FindBin::RealBin, join(SLASH, $FindBin::RealBin, '..', 'share', 'jukebox'));
...
# remove repeated slashes, /./, and make sure it (does or doesn't) end with a slash
sub cleanpath {
	my ($path, $end_with_slash) = @_;
...
# Parse command line
# in a BEGIN block so that commands for a running instance are sent sooner/faster
BEGIN {
	$DBus_id     = 'org.jukebox';
	$DBus_suffix = '';

	my $default_home = Glib::get_user_config_dir . SLASH . 'jukebox';
	if (!-d $default_home && -d (my $old = Glib::get_home_dir . SLASH . '.jukebox')) {
...
-C FILE, -cfg FILE: use FILE as configuration file (instead of 'jukeboxrc' in default folder), if FILE is a folder, sets the default folder to FILE.
...
EOF
	...
	while (defined(my $arg = shift)) {
		if ($arg eq '-c' || $arg eq '-nocheck') {
			$CmdLine{nocheck} = 1
		} elsif ($arg eq '-s' || $arg eq '-noscan') {
			$CmdLine{noscan} = 1
		} elsif ($arg eq '-demo') {
			$CmdLine{demo} = 1
		} elsif ($arg eq '-play') {
			...
			$CmdLine{gst0} = 1; # prefer gstreamer-0.10
		} elsif ($arg eq '-ro') {
			$CmdLine{ro} = $CmdLine{rotags} = 1
		} elsif ($arg eq '-rotags') {
			$CmdLine{rotags} = 1
		} elsif ($arg eq '-port') {
			$CmdLine{port} = shift if $ARGV[0]
			...
			$ImportFile = rel2abs(shift) if $ARGV[0]
		...
		} elsif ($arg eq '-geometry') {
			$CmdLine{geometry} = shift if $ARGV[0];
			...
			push @cmd, shift if $ARGV[0];
		} elsif ($arg eq '-ifnotrunning') {
			$ifnotrunning = shift if $ARGV[0];
		} elsif ($arg eq '-nolaunch') {
			$ifnotrunning = 'abort';
		} elsif ($arg eq '-dbus-id') {
			if (my $id = shift) {
				if ($id =~ m/^\w+$/) {
					$DBus_id .= $DBus_suffix = '.' . $id;
				} else {
					warn "invalid dbus-id '$id', only letters, numbers and _ allowed\n";
				}
			}
		} elsif ($arg eq '-add') {
			$filescmd = 'AddToLibrary';
		} elsif ($arg eq '-playlist') {
			$filescmd = 'OpenFiles';
		} elsif ($arg eq '-enqueue') {
			$filescmd = 'EnqueueFiles';
		} elsif ($arg eq '-addplaylist') {
			$filescmd = 'AddFilesToPlaylist';
		} elsif ($arg eq '-insertplaylist') {
			$filescmd = 'InsertFilesInPlaylist';
		} elsif ($arg eq '-use-gnome-session') {
			$CmdLine{UseGnomeSession} = 1;
	...
	if (defined $save) {
		# $save is considered a folder if ends with a "/"
		my $isdir = $save =~ m#/$#;
	...
	# auto import from old v1.0 tags file if using default savefile,
	# it doesn't exist and old tags file exists
	if (!$SaveFile && !find_gmbrc_file($HomeDir . 'jukeboxrc') && -e $HomeDir . 'tags') {
		$ImportFile ||= $HomeDir . 'tags';
	}

	$SaveFile ||= $HomeDir . 'jukeboxrc';
	$FIFOFile = $HomeDir . 'jukebox.fifo' if !defined $FIFOFile && $^O ne 'MSWin32';
	...
	unless ($ignore) {
		...
		if (!$running && !$CmdLine{noDBus}) {
			eval { require 'jukebox_dbus.pm' } || warn "Error loading jukebox_dbus.pm :\n$@ => controlling jukebox through DBus won't be possible.\n\n";
...
BEGIN {
	...
	$HTTP_module =
		-e $DATADIR . SLASH . 'simple_http_wget.pm' && (grep -x $_ . SLASH . 'wget', split /:/, $ENV{PATH})
		? 'simple_http_wget.pm'
		: -e $DATADIR . SLASH . 'simple_http_AE.pm' && (grep -f $_ . SLASH . 'AnyEvent' . SLASH . 'HTTP.pm', @INC)
		? 'simple_http_AE.pm'
		: 'simple_http.pm';
	...
	for my $file (qw/jukebox_123.pm jukebox_mplayer.pm jukebox_mpv.pm jukebox_server.pm/) {
		# each file sets $::PlayPacks{PACKAGENAME} to 1 for each of its included playback packages
		eval { require $file } || warn $@;
	}
...
BEGIN {
	...
	if ($@) {
		warn "Cairo perl module not found -> transparent windows and other effects won't be available\n";
	}
}
...
# warning: doesn't force end of string (with a "$") as sometimes needs to
# include/extract a :\w+ at the end, so need to use it with /$EmbImage_ext_re$/
# or /$EmbImage_ext_re(:\w+)?$/
our $EmbImage_ext_re = qr/\.(?:mp3|flac|m4a|m4b|ogg|oga)/i;

##########

# breaks "widget1(options with spaces) widget2" in "widget1(options with
# spaces)" and "widget2" #replaced by ExtractNameAndOptions
#our $re_spaces_unlessinbrackets=qr/([^( ]+(?:\(.*?\))?)(?: +|$)/; 
...
unshift @SongCMenu, (
...

	{
		...
		code => sub {
			::DoActionForList('addplay', $_[0]{IDs});
		},
		notempty => 'IDs',
		test => sub { $::ListMode },
	},
	...
	# edit submenu for label-type fields
	{
		...
		notempty => 'IDs',
		submenu => sub {
			LabelEditMenu($_[0]{field}, $_[0]{IDs});
		},
		foreach => sub {
			'field',
			Songs::FieldList(
	...
	#{
	#	label => sub {
	#		'Remove from ' . ($_[0]{mode} eq 'Q' ? 'queue' : 'this list')
	#	},
	#	code => sub {
	#		$_[0]{self}->RemoveSelected;
	#	},
...
our @TrayMenu = (
...

	{
		...
		submenu => sub {
			[
				map { $_->layout_name => $_ } grep $_->isa('Layout::Window'), Gtk2::Window->list_toplevels
			];
		},
	},
...
# escape special chars for pango ( & < > ) #replaced by Glib::Markup::escape_text if available
sub PangoEsc {
	local $_ = $_[0];
...
# force bringing the window to the current workspace, $win->present does not always do that
sub Gtk2::Window::force_present {
	my $win = shift;
...
# increment a number suffix from a string
sub IncSuffix {
...
#sort (s1 s10 s2) into (s1 s2 s10)
sub sort_number_aware {
	my %h;

	# format numbers with at least 20 digits, will fail for numbers with more
	# than 20 digits, but probably not interesting anyway
	($h{$_} = superlc($_)) =~ s/(\d+)/"0"x(20-length($1)).$1/ge for @_;

	return sort { $h{$a} cmp $h{$b} } @_;
}

# lowercase, normalize and remove accents/diacritics #not sure how good it is
...
sub superlc {
	...
	# almost never change anything and should not change comparison result
	# anyway; so better leave it out as it's rather costly
	#$s = Unicode::Normalize::compose($s); 
...
# return keys of $hash sorted by $hash->{$_}{$sort_subkey} or by $hash->{$_} using superlc
sub sorted_keys {
	my ($hash, $sort_subkey) = @_;
	if (defined $sort_subkey) {
		return sort {
			superlc($hash->{$a}{$sort_subkey}) cmp superlc($hash->{$b}{$sort_subkey})
		} keys %$hash;
	} else {
...
# return true if 1st argument is a word in contained in the 2nd argument (space-separated words)
sub WordIn {
	return 1 if first { $_[0] eq $_ } split / +/, $_[1];
	return 0;
}

# true if at least one string common to both list
sub OneInCommon {
	my ($l1, $l2) = @_;
...
sub ReplaceFields {
	...
	my $display =
		$esc ? ref $esc ? sub { $esc->(Songs::Display(@_)) } : \&Songs::DisplayEsc
		     : \&Songs::Display;
...
sub MakeReplaceTable {
	...
	for my $letter (split //, $fields) {
		for my $text (
			'%' . $letter,
			$special{$letter} || Songs::FieldName($ReplaceFields{'%' . $letter})
		)
		{
...
sub MakeReplaceText {
	my ($fields, %special) = @_;
	my $text = join "\n",
		map "%$_ : " . ($special{$_} || Songs::FieldName($ReplaceFields{'%' . $_})),
		split //, $fields;
...
sub dates_to_timestamps {
	...
	for my $date ($date1, $date2) {
		...
		{
			# yyyy/MM/dd hh:mm:ss
			($y, $M, $d, $h, $m, $s) = $date =~ m#^(\d\d\d\d)(?:[-/.](\d\d?)(?:[-/.](\d\d?)(?:[-T ](\d\d?)(?:[:.](\d\d?)(?:[:.](\d\d?))?)?)?)?)?$# and last;

			# hh:mm:ss or hh:mm or hh:
			($h, $m, $s, $pm) = $date =~ m#^(\d\d?)[:](?:(\d\d?)(?:[:](\d\d?))?)?([ap]m?)?$#i and last;

			# dd/MM or dd/MM/yyyy
			($d, $M, $y) = $date =~ m#^(\d\d?)(?:[-/.](\d\d?)(?:[-/.](\d\d\d\d))?)?$# and last;

			# MM/yyyy
			($M, $y) = $date =~ m#^(\d\d?)[-/.](\d\d\d\d)$# and last;

			# yyyyMMdd
			($y, $M, $d) = $date =~ m#^(\d{4})(\d\d)(\d\d)$# and last;

			# jan or jan99 or 10jan or 10jan12 or jan2012
			if ($date =~ m#^(?:(\d\d?)[-/. ]?)?(\p{Alpha}+)(?:[-/. ]?(\d\d(?:\d\d)?))?$#
			...
			# name of week day
			if (defined(my $wday = $DAYS{::superlc $date})) {
		...
		# complete relative dates with current date
		for ($y, $M, $d, $h, $m, $s) {
			last if defined;
			$_ = pop @now;
		}

		# unit to change if in the future
		$past_var = scalar @now unless defined $past_var;
		...
		# for relative dates, choose between previous and next match
		# (for example, this year's july or previous year's july)
		if ($past_var < 6 && $date > time) {
...
# convert date/time pattern into nb of seconds
sub ConvertTimeLength {
	my ($number, $unit) = $_[0] =~ m/^\s*(\d*\.?\d+)\s*([a-zA-Z]*)\s*$/;

	return 0 unless $number;

	if (my $ref = $DATEUNITS{$unit}) {
		$number *= $ref->[0];
	} elsif ($unit) {
		warn "ignoring unknown unit '$unit'\n";
	}

	return $number;
}

# convert date pattern into nb of seconds since epoch
sub ConvertTime {
...
# for Save Vol Time Queue Lock Repeat Sort Filter Pos CurSong Playing
# SavedWRandoms SavedSorts SavedFilters SavedLists Icons Widgets connections
my %EventWatchers;
...
# don't use these
$Encoding_pref{$_} = -2 for qw/null AdobeZdingbat ascii-ctrl dingbats MacDingbats/; 

# these can generate lots of warnings, skip them when trying encodings
$Encoding_pref{$_} = -1 for qw/UTF-32BE UTF-32LE/;

# use these first when trying encodings
$Encoding_pref{$_} = 2 for qw/utf8 cp1252 iso-8859-15/;
...
our %Options = (
	...
	# check if songs have changed on startup
	StartCheck => 0,

	# scan @LibraryPath on startup for new songs
	StartScan  => 0,

	FilenameSchema => ['%a - %l - %n - %t', '%l - %n - %t', '%n-%t', '%d%n-%t'],
	FolderSchema => ['%A/%l', '%A', '%A/%Y-%l', '%A - %l'],

	# Threshold to count a song as played in percent
	PlayedMinPercent => 80,

	# Threshold to count a song as played in seconds
	PlayedMinSeconds => 600,
	...
	# most of presets are taken from gstreamer equalizer plugin
	equalizer_presets =>  {
...
# returns a hash used in the combobox of the starprefix option
sub Find_all_stars {
...
sub Find_star_pictures {
	...
	unless (file_name_is_absolute($prefix)) {
		...
		for my $path (@dirs) {
			# FIXME extension shouldn't be case-sensitive
			next unless -f $path . $prefix . '0.svg' || -f $path . $prefix . '0.png';
	...
	# FIXME extension shouldn't be case-sensitive
	my $suffix = (-f $prefix . '0.svg') ? '.svg' : '.png';
	my @files;
...
sub LoadIcons {
	...
	# for gtk version 2.6
	unless (Gtk2::Stock->lookup('gtk-fullscreen')) {
...
# contains sub,description,argument_tip, argument_regex or code returning a
# widget, or '0' to hide it from the GUI edit dialog
our %Command = (
	NextSongInPlaylist => [
		\&NextSongInPlaylist,
	...
	OpenCustom  => [
		sub {
			Layout::Window->new($_[1]);
		},
		"Open Custom window",
		"Name of layout",
		sub {
			TextCombo::Tree->new(Layout::get_layout_list());
		}
	],
	...
	CloseWindow => [
		sub {
			$_[0]->get_toplevel->close_window if $_[0];
		},
		"Close Window"
	],
	...
	SetPlayerLayout => [
		sub {
			SetOption(Layout => $_[1]);
			CreateMainWindow();
		},
		"Set player window layout",
		"Name of layout",
		sub {
			TextCombo::Tree->new(Layout::get_layout_list('G'));
		},
	],
	...
	OpenSongProp => [
		sub {
			DialogSongProp($SongID) if defined $SongID
		},
	...
	SetNextAction => [
		...
		sub {
			TextCombo->new(
				{
					map { $_ => $QActions{$_}{short} } sort grep $QActions{$_}{can_next}, keys %QActions
				}
			);
		}
	],
	...
	RunSysCmd => [
		sub {
			call_run_system_cmd($_[0], $_[1], 0, 0)
		},
		"Run system command",
	...
	RunShellCmd => [
		sub {
			call_run_system_cmd($_[0], $_[1], 0, 1)
		},
		"Run shell command",
		"Shell command" . "\n" . "Some variables such as %f (current song filename) are available",
		qr/./
	...
	RunSysCmdOnSelected => [
		sub {
			call_run_system_cmd($_[0], $_[1], 1, 0)
		},
	...
	RunShellCmdOnSelected => [
		sub {
			call_run_system_cmd($_[0], $_[1], 1, 1)
		},
	...
	ToggleRandom => [
		\&ToggleSort,
		"Toggle between Random/Shuffle and Ordered"
	],

	Shuffle => [
		\&Shuffle,
		"Shuffle or re-shuffle the playlist"
	],
	...
	AddFilesToPlaylist => [
		sub {
			DoActionForList('addplay', Uris_to_IDs($_[1]));
		},
	...
	InsertFilesInPlaylist => [
		sub {
			DoActionForList('insertplay', Uris_to_IDs($_[1]));
		},
	...
	EnqueueFiles => [
		sub {
			DoActionForList('queue', Uris_to_IDs($_[1]));
		},
...
sub run_command {
	...
	if (my $ref = $Command{$cmd}) {
		$ref->[0]->($self, $arg);
	} else {
		warn "Unknown command '$cmd' => can't execute '$cmd($arg)'\n";
	}
}
...
sub split_with_quotes {
	...
	# remove quotes and put back unused backslashes
	for (@w) {
		if (s/^"//) {
			s/"$//;
			s#\\39#\\92'#g;
...
sub call_run_system_cmd {
	...
	if ($use_selected || $cmd =~ s#%F\b#\$files#) {
		...
		unless (@IDs) {
			warn "Not executing '$cmd' because no song is selected in the current window\n";

			return;
		}
	} else {
		@IDs = ($SongID) if defined $SongID;
	}
...
sub run_system_cmd {
	...
	if (!ref $cmd && !$use_shell) {
		$cmd = [ split_with_quotes($cmd) ]
	}
	...
	if ($IDs) {
		if ($cmd =~ m/\$files\b/) {
			$files = join $join, map $quotesub->(Songs::GetFullFilename($_)), @$IDs;
		}
...
sub forksystem {
	...
	if ($ChildPID == 0) {
		...
		if (ref $cmd[0]) {
			...
			# execute one command in the child process
			exec @cmd;
		}
		POSIX::_exit(0);
	}
...
if ($FIFOFile) {
	if (-e $FIFOFile) {
		unlink $FIFOFile unless -p $FIFOFile;
	} else {
...
if ($CmdLine{UseGnomeSession}) {
	# use the gnome libraries, if present, to enable some session management
	eval {
		require Gnome2;
...
# gstreamer (how I use it) needs the mainloop running to skip, so this is done after the main window is created
#SkipTo($PlayTime) if $PlayTime; 
...
if (my $cmds = delete $CmdLine{runcmd}) {
	run_command(undef, $_) for @$cmds;
}
$SIG{TERM} = \&Quit;
...
sub Edittag_mode {
	...
	$Songs::Def{$_}{flags} =~ m/w/ || $Songs::Def{$_}{flags} =~ s/e// for grep $Songs::Def{$_}{flags}, keys %Songs::Def;
...
# replaced by Glib::filename_display_name if available
sub filename_to_utf8displayname {
	my $utf8name = eval {
		filename_to_unicode($_[0])
	};
	if ($@) {
		$utf8name = $_[0];
...
sub HVpack {
	...
	while (@list) {
		...
		if ($end) {
			$box->pack_end($w, $exp, $exp, $pad);
		} else {
			$box->pack_start($w, $exp, $exp, $pad);
		}
	}
...
sub TurnOff {
	my $dialog = Gtk2::MessageDialog->new($MainWindow, [qw/modal destroy-with-parent/], 'warning', 'none', '');
	$dialog->add_buttons('gtk-cancel' => 2, 'gmb-turnoff' => 1);

	my $sec   = 21;
	my $timer = sub {
		# FIXME can be more than 1 second
		return 0 unless $sec;
...
sub CmdFromFIFO {
	while (my $cmd = <$fifofh>) {
		...
		#if ($cmd eq 'Print') {
		#	print $fifofh "Told to print : $arg\n";
		#	return;
		#}
		...
		if (exists $Command{$cmd}) {
			...
			warn "fifo:received unknown command : '$cmd'\n";
		}
	}

	# FIXME replace 1 by gtk+ version check once the gtk+ bug is fixed
	# (http://bugzilla.gnome.org/show_bug.cgi?id=321053)
	if (1) {
...
# search for file with a relative path among a few folders, used to find pictures used by layouts
sub SearchPicture {
	my ($file, @paths) = @_;
	...
	push @paths, $HomeDir . 'layouts', $CmdLine{searchpath}, PIXPATH, $DATADIR . SLASH . 'layouts';

	@paths = grep defined, map ref() ? @$_ : $_, @paths;

	for (@paths) {
		# replace files by their folder
		$_ = dirname($_) if -f;
	}
...
sub FileList {
	...
	for my $search (@search) {
		if (-f $search) {
			push @found, $search if $search =~ $re;
			next;
		}
...
sub CheckPluginRequirement {
	...
	if (my $req = $ref->{req}) {
		...
		while (@req) {
			...
			$packages = $packages
				? ("Possible package names providing this :" . ' ' . $packages . "\n")
				: '';
...
sub ChangeVol {
	...
	if ($_[0] eq 'mute') {
		...
		if ($Vol_package->GetMute) {
			$cmd = 'unmute';
		} elsif ($cmd eq 'up') {
			$cmd = "+$Options{VolumeStep}";
		} elsif ($cmd eq 'down') {
			$cmd = "-$Options{VolumeStep}";
		}
	}
...
sub SetEqualizer {
	...
	if ($key eq 'smart') {
		if ($value =~ m/^[01]$/) {
			...
			warn "SetEqualizer: invalid argument : $value\n";
		}
	}
...
sub FirstTime {
	...
	my @dirs = reverse map $_ . SLASH . 'jukebox', Glib::get_system_config_dirs;
...
sub ReadOldSavedTags {
	...
	if ($oldversion < 0.9464) {
		# cleanup old options
		delete $Options{$_} for qw/BrowserTotalMode FilterPane0Page FilterPane0min FilterPane1Page FilterPane1min LCols LSort PlayerWinPos SCols Sticky WSBrowser WSEditQueue paned StickyFilters/;
	}
	...
	# cleanup old options
	delete $Options{$_} for qw/Device 123options_mp3 123options_ogg 123options_flac test Diacritic_sort gst_volume Simplehttp_CacheSize/;

	# don't bother supporting upgrade for these
	delete $Options{$_} for qw/SavedSongID SavedPlayTime Lock SavedSort/;
	...
	# for versions <=1.0
	delete $Options{$_} for grep m/^PLUGIN_MozEmbed/, keys %Options;
	...
	# for version <1.1.2
	$Options{Labels}         = [ split "\x1D", $Options{Labels} ] unless ref $Options{Labels};

	$Options{Fields_options}{label}{persistent_values} = delete $Options{Labels};
	$Options{Artists_split_re} = [
		map { $artistsplit_old_to_new{$_} || $_ } grep $_ ne '$', split /\|/, delete $Options{ArtistSplit}
	];
	...
	# to prevent 'utf8 "\xE9" does not map to Unicode' type warnings about
	# path and file which are stored as they are on the filesystem #FIXME
	# find a better way to read lines containing both utf8 and unknown
	# encoding
	no warnings 'utf8';
	...
	while (<$fh>) {
		...
		# 0=SONG_FILE 1=SONG_PATH 2=SONG_MODIF
		next unless $song[0] && $song[1] && $song[2];
	...
	if ($Options{Sort} =~ m/random|shuffle/) {
		...
		$Options{Sort_LastOrdered} = $Options{Sort} || 'path file';
	}
...
# for versions <=1.1.7
sub Filter_new_from_string_with_upgrade {
	...
	for my $f (@filter) {
		...
		if ($cmd eq 's') {
			...
			$pat =~ s#([^/\$_.+!*'(),A-Za-z0-9-])#sprintf('%%%02X',ord($1))#seg;
		} elsif ($field eq 'rating' && $cmd eq 'e') {
			$cmd = '~';
			...
			if ($pat =~ m/[a-zA-Z]/) {
				...
				$pat =~ s/(\d\d\d\d)-(\d\d?)-(\d\d?)/mktime(0,0,0,$3,$2-1,$1-1900)/eg;
			}
		} elsif ($cmd eq 'b') {
			my ($n1, $n2) = split / /, $pat;
...
sub ReadSavedTags {
	...
	# will use .gz/.xz to save if read from a .gz/.xz gmbrc
	$SaveFile .= $1 if $loadfile =~ m#($gmbrc_ext_re)# && $SaveFile !~ m#$gmbrc_ext_re#; 

	# so that '.' is used as a decimal separator when converting numbers into strings
	setlocale(LC_NUMERIC, 'C');

	# read first line to determine if old version, version >1.1.7 stars
	# with "# gmbrc version=",  version <1.1 starts with a letter, else
	# it's version<=1.1.7 (starts with blank or # (for comments) or [
	# (section name))
	my $firstline = <$fh>;
	unless (defined $firstline) {
		die "Can't read '$loadfile', aborting...\n";
	}
	my $oldversion;
	...
	if (!$oldversion || $oldversion > 1) {
		...
		unless ($lines{Options}) {
			warn "Can't find Options section in '$loadfile', it's probably not a jukebox save file -> aborting\n";
		...
		if ($oldversion > VERSION) {
			warn "Loading a jukeboxrc saved with a more recent version of jukebox, try upgrading jukebox if there are problems\n";
		}
		if ($oldversion < 1.10091) {
			# cleanup old options
			delete $Options{$_} for qw/Diacritic_sort gst_volume Simplehttp_CacheSize mplayer_use_replaygain/;
		}
		...
		if ($oldversion <= 1.1014) {
			$Options{$_} = delete $Options{"gst_$_"} for qw/equalizer use_equalizer equalizer_preset equalizer_preamp use_replaygain rg_albummode rg_fallback rg_preamp rg_limiter/;
		}
		...
		for my $extra (keys %$extra_sub) {
			...
			next unless $lines;
			shift @$lines;
			#my @properties = split / /, shift @$lines;
		...
		SongArray::updateIDs(\@newIDs);
		if (my $l = delete $Options{SongArray_Estimated}) {
			# for $oldversion<1.1008
	...
	if ($oldversion <= 1.1009) {
		...
		bless $_, 'SongArray::Named' for values %{$Options{SavedLists}};
		...
		)
		{
			...
			$floatvector = pack "F*", map $_ || "nan", unpack("F*", $floatvector) if $floatvector;
		}
		...
		IdleDo(
			...
			sub {
				...
				while (my ($gid, $IDs) = each %$h) {
					...
					$type =~ s/2,5/2.5/;
					$type =~ s/^mp3 l(\d)v(\d.*)/mp$1 mpeg-$2 l$1/ && Songs::Set($IDs, filetype => $type);
				}
			}
		);
	}
	...
	# for versions <=1.1.1
	$Options{LibraryPath} ||= [];
	$Options{LibraryPath} = [ map url_escape($_), split "\x1D", $Options{LibraryPath} ] unless ref $Options{LibraryPath};
	&launchIdleLoop;
...
sub Open_gmbrc {
	...
	if ($write) {
		...
		# if not found as is try with/without .gz/.xz
		unless (-e $file) {
			$file =~ s#$gmbrc_ext_re##;
		...
		if (-z $file) {
			warn "Warning: save file '$file' is empty\n";
			return;
		}
		...
		if (@cmd) {
			...
			if (findcmd($cmd[0])) {
				...
				warn "Can't find $cmd[0], you could uncompress '$file' manually\n";
			}
		}
...
sub SaveTags {
	...
	# do a local copy to locally remove .gz extension if present
	my $SaveFile = $SaveFile; 

	# remove .gz/.xz extension from the copy of $SaveFile, put it in $ext
	$ext = $1 if $SaveFile =~ s#($gmbrc_ext_re)$##;
	...
	# delete old temporary save files
	unlink $savedir . SLASH . $_ for grep m/^\Q$savefilename\E\.new\.\d+(?:$gmbrc_ext_re)?$/, readdir $dh;
	closedir $dh;
	...
	for my $key (keys %{$Options{Layouts}}) {
		my $lastseen = $Options{LayoutsLastSeen} ||= {};
		if (exists $Layout::Layouts{$key}) {
			delete $lastseen->{$key};
		} elsif (!$lastseen->{$key}) {
			$lastseen->{$key} = $DAYNB;
	...
	# default is, for some reason, IGNORE, which causes "gzip: stdout:
	# Broken pipe" after closing $fh when using gzip for unclear reasons
	...
	if ($previous) {
		# keep some old files as backup
		{
			...
			if (!$cmpr && (!exists $CmdLine{gzip} || $CmdLine{gzip})) {
		...
		unlink $_ for find_gmbrc_file($SaveFile);
	}
...
# convert a string written by SaveRefToLines to a hash/array # can only read a small subset of YAML
sub ReadRefFromLines {
	my ($lines, $return) = @_;
	my @todo;
	...
	for my $line (@$lines) {
		...
		$d = length $d;
		if ($parentval) {
			# first value of new array or hash
			next unless $d >= $ident;
			push @todo, $ref, $ident;
			$ident     = $d;
			$ref       = $$parentval = $array ? [] : {};
		...
		if ($val =~ m/^!/) {
			...
			if ($val =~ s/^!([^ !]+)\s*//) {
				...
				warn "Unsupported value : '$val'\n";
				next;
			}
		}

		if ($val eq '') {
			# array or hash or object as array/hash

			$parentval = $array ? \$ref->[@$ref] : \$ref->{$key};
			push @objects, $class, $parentval if $class;
		} else {
			# scalar or empty array/hash or object as string
...
# convert hash/array into a YAML string readable by ReadRefFromLines
sub SaveRefToLines {
	my $ref = $_[0];
...
sub SkipTo {
	...
	if (defined $PlayingID && defined $PlayTime) {
...
sub PlayPause {
	if (defined $TogPlay) {
		# paused or playing => resume or pause
		Pause();
	} else {
		# stopped => play
		Play()
...
sub ErrorPlay {
	...
	my $dialog = Gtk2::MessageDialog->new($MainWindow, [qw/modal destroy-with-parent/], 'error', 'close', '%s', $error);
	...
	# will avoid counting the song as played or skipped
	$PlayingID = undef;
...
# find number of unique seconds played from a list of start,stop times
sub Coverage {
	my @segs = @_;
	my $sum  = 0;
	while (@segs) {
		...
		while ($i < @segs) {
			...
			# segments overlap
			if ($start - $s1 <= $th && $s1 - $stop <= $th || $start - $s2 <= $th && $s2 - $stop <= $th) {
				$stop  = $s2 if $s2 > $stop;
...
sub Played {
	...
	# FIXME maybe only count as a skip if played less than ~20% ?
	if ($partial) {
...
# for a given ID, returns the Play, Pause, Stop or Queue icon, or undef if none applies
sub Get_PPSQ_Icon {
...
sub Update_QueueActionList {
	# check if current one is still valid
	if ($QueueAction) {
		my $ok;
...
sub List_QueueActions {
	...
	if ($nextonly) {
		@list = grep $QActions{$_}{can_next}, @list;
	}
...
# if no aguments, returns next song and makes the changes assuming it is to
# become next song (remove it from queue, ...)
sub GetNextSongs {
...

	{
		...
		while ($nb) {
			if ($pos + $nb > $#$ListPlay) {
				...
				push @IDs, @$ListPlay[$pos + 1 .. $pos + $nb];
...
# go to next song whose $field value is different than current's
sub NextDiff {
	my $field = $_[0];
	...
	if (@$Queue) {
		...
		if (@$list) {
			...
			# empty queue if not matching song in it
			$Queue->Replace();
		}
	}
	...
	if ($TogLock && $TogLock eq $field) {
		# remove lock on a different field if not found ?
	...
	if (@$list) {
		# there is at least one matching  song
...
sub DoActionForFilter {
	...
	if ($action eq 'play') {
		...
		DoActionForList($action, $filter->filter);
	}
}
...
sub DoActionForList {
	...
	if ($action eq 'queue') {
		Enqueue(@list);
	} elsif ($action eq 'queueinsert') {
		QueueInsert(@list);
	} elsif ($action eq 'replacequeue') {
		ReplaceQueue(@list);
	} elsif ($action eq 'properties') {
		DialogSongsProp(@list);
	} else {
		...
		if ($action eq 'playlist') {
			$ListPlay->Replace(\@list);
		} elsif ($action eq 'addplay') {
			$ListPlay->Push(\@list);
...
sub ToggleSort {
	my $s = ($RandomMode || $Options{Sort} =~ m/shuffle/)
...
#CHECKME if everything works with sort="",
#with source
#with empty Library
#with !defined $SongID
#implement row=> or pos=>

# Set filter, sort order, selected song, playing state, staticlist, source
sub Select {
	my %args = @_;
	...
	if (defined $filter) {
		...
		if ($sort) {
			$ListPlay->SetSortAndFilter($sort, $filter);
		} else {
...
sub UpdateCurrentSong {
	...
	if ($ChangedID) {
		...
		if (defined $RecentPos && (!defined $SongID || $SongID != $Recent->[$RecentPos - 1])) {
			$RecentPos = undef;
		}
	...
	#Stop(); if !defined $SongID; XXX ???????
	#if ($forceplay) { Play(); } XXX
...
sub FindPositionSong {
	...
	for my $i (0 .. $#$list) {
		return $i if $list->[$i] == $ID;
	}

	return undef; # not found
}

# Choose a song in @$lref based on sort order, if possible in the current
# playlist. In sorted order, choose a song after current song
...
# sort @$listref according to current sort order, or last ordered sort if no current sort order
sub SortList {
	my $listref = shift;
	my $sort = $Options{Sort};
...
sub Delayed {
	...
	$Delayed{$task_id} = Glib::Timeout->add(
		$timeout,
		sub {
			delete $Delayed{$task_id};
			$cb->(@args);
...
sub IdleLoop {
	if (@$ToCheck) {
		...
		SongAdd(shift @ToAdd_Files);
	} elsif (@ToAdd_IDsBuffer > 1000) {
		SongAdd_now();
	} elsif (@ToScan) {
		$ProgressNBFolders++;

		ScanFolder(shift @ToScan);
	} elsif (@ToAdd_IDsBuffer) {
		SongAdd_now();
	} elsif (%ToDo) {
		DoTask((sort keys %ToDo)[0]);
	} elsif (@$ToCheckLength) {
...
sub WEditList {
	...
	if ($window) {
		$window->force_present;

		return;
	}

	# list that will be used by SongList/SongTree in 'editlist' mode
	$SongList::Common::EditList = $name;
	...
	Watch(
		...
		# close window if the list is deleted, update title if renamed
		SavedLists => sub {
...
sub CalcListLength {
	...
	if ($return eq 'long') {
		my $format = $h ? "{hours} hours {min} min {sec} s" : "{min} min {sec} s";
...
sub openurl {
	...
	unless ($browsercmd) {
		ErrorMessage("No web browser found.");
...
sub openfolder {
	...
	$opendircmd ||= findcmd($Options{OpenFolder}, qw/gio xdg-open gnome-open nautilus konqueror thunar spacefm/);

	unless ($opendircmd) {
		ErrorMessage("No file browser found.");
...
sub ChooseAAPicture {
	...
	if (defined $ID) {
		$path = Songs::Get($ID, 'path');
	} else {
		$path = AA::GuessBestCommonFolder($col, $key);
	}

	my $title = sprintf("Choose picture for '%s'", Songs::Gid_to_Display($col, $key));
...
# FIXME limit the number of songs if HUGE number of songs (>100-200 ?)
sub ChooseSongsFromA {
	...
	unless ($opt{nocover}) {
		...
		# check if fits in the screen
		if ($maxwidth > $picsize * ($nbcols + 1)) {
			...
			if (my $img = AAPicture::newimg(album => $album, $picsize)) {
				...
				# if few songs put the cover below
				if ($w > $h && $nbcols == 1) {
					$nbcols = 0;
					$row0 = $height;
				}
...
sub ChooseSongs {
	...
	my $format = $opt{markup} || __x("{song} by {artist}", song => "<b>%S</b>%V", artist => "%a");
	...
	my $click_callback = sub {
		...
		if ($event->button == 2) {
			...
			my $state = Gtk2->get_current_event_state;
			if ($state && ($state >= ['shift-mask'] || $state >= ['control-mask'])) {
	...
	if (@IDs <= $maxrows) {
		...
		# one column => split it into multiple columns if too big
		if (@columns == 1) {
			...
			# multiple columns => try to combine them

			my $c = 0;
			my $r = $columns[$c++];
			while ($r) {
				if ($new[-1] > 1.2 * @IDs / $maxcols) {
					push @new, 0;
				}

				if ($new[-1] + $r < $maxrows * 1.1) {
					$new[-1] += $r;
					$r = 0;
				} elsif ($r < $maxrows * 1.1) {
					push @new, $r;
			...
			@columns = @new if @new <= $maxcols;
		}
	}

	# use submenus if too many columns
	if (@columns > $maxcols) {
		my $row = 0;
		for my $c (@columns) {
			...
			if ($ID =~ m/^\d+$/) {
				...
				$item->signal_connect(
					button_release_event => sub {
						return 1 if delete $_[0]->parent->{keep_it_up};
	...
	# update queue/playing icon when needed
	::Watch($menu, $_, $update_icons) for qw/CurSongID Playing Queue/;
...
sub PopupAA {
	...
	if (defined $list) {
		...
		if ($field eq 'album') {
			...
			while (my ($album, $list) = each %alb) {
				# FIXME PHASE1
				my $artist = join ' & ', map Songs::Gid_to_Display('artist', $_), @$list;
			...
			if (1 == keys %art_keys) {
				...
				# multiple artists -> create a submenu for each artist
				my $menu = Gtk2::Menu->new;
	...
	my $altcallback = $field eq 'album' && !$noalt
		?	sub {
				# Albums button-press event : set up a songs
				# submenu on right-click, alternate action on
				# middle-click
		...
		?	sub  {
				# Artists button-press event : set up an album submenu
				# on right-click, alternate action on middle-click
	...
	my $createAAMenu = sub {
		...
		for my $i ($start .. $end) {
			...
			# FIXME doesn't work as I want, used to force wrapping
			# at a smaller width than default, but result in
			# requesting $maxwidth when the width of the wrapped
			# text may be significantly smaller
...
sub Breakdown_List {
	...
	# FIXME is that ok for multi-monitors ?
	my $screen =
		$widget
		? $widget->get_screen
		: Gtk2::Gdk::Screen->get_default;
	...
	if ($makemenu && !$options{norecurse}) {
		...
		#probably shouldn't happen, in case number too low assume the screen can fit 10 items
		$parentopt = .65 * 10  if $parentopt < .65 * 10;
	...
	while (@todo) {
		...
		for my $i ($i0 + 1 .. $#breakpoints) {
			...
			if ($bestscore[$i]) {
				...
				# ignore the new path if not better
				next if $bestscore[$i] < $score;
	...
	for my $i (@$tbref) {
		...
		if (my $stock = $i->{stockicon}) {
			if ($toggle) {
				...
				$item = Gtk2::ToolButton->new_from_stock($stock);
			}
...
sub BuildMenu {
	...
	$args ||= {};
	$menu ||= Gtk2::Menu->new; # append to menu if menu given as argument
	...
	for my $m (@$mref) {
		...
		if (my $mod = $m->{change_input}) {
			...
			next;
		} elsif (my $include = $m->{include}) {
			# append items made by $include
			$include = $include->($args, $menu) if ref $include eq 'CODE';

			if (ref $include eq 'ARRAY') {
				BuildMenu($include, $args, $menu);
			}
		...
		if (my $submenu = $m->{submenu}) {
			...
			$item->signal_connect(
				activate => sub {
					...
					if (my $ref = $self->{toggleref}) {
						$$ref ^= 1;
					}

					my $on;
					$on = $self->get_active if $self->isa('Gtk2::CheckMenuItem');

					if (my $code = $self->{code}) {
						if (ref $code) {
							$code->($args, $on);
						} else {
							run_command(undef, $code);
						}
					}
				},
				$args
			...
			# set a submenu on right-click
			if (my $submenu3 = $m->{submenu3}) {
			...
			} elsif (my $code3 = $m->{code3}) {
				# alternate action on right-click
				$item->signal_connect(
					button_press_event => sub {
...
sub PopupMenu {
	...
	if ($event) {
		...
		if (!$posfunction && !$args{nomenupos} && $event->window) {
			...
			# ignore the event's widget if too big, widget can be
			# the whole window if coming from a shortcut key, it
			# makes no sense poping-up a menu next to the whole
			# window
			$posfunction = \&menupos if $h < 300;
		}
	}
...
# function to position popupmenu below clicked widget
sub menupos {
	my $event = Gtk2->get_current_event;
	...
	# position of the clicked widget on the screen
	my ($x, $y) = $event->window->get_origin;

	# width and height of the clicked widget
	my ($dw, $dy) = $event->window->get_size;
...
sub BuildChoiceMenu {
	...
	# append items to an existing menu or create a new menu
	my $menu = delete $options{menu} || Gtk2::Menu->new;
	...
	if ($check = $options{check}) {
		if (ref $check) {
			...
			my ($not, $ref) = ParseKeyPath($args, $check);
			$check = $$ref;
		}
	}
...
sub set_drag {
	...
	if (my $dragdest = $params{dest}) {
		...
		$widget->signal_connect(drag_motion        => $params{motion}) if $params{motion};
...
sub drag_data_get_cb {
	my ($self, $context, $data, $destinfo, $time) = @_;
	#warn "drag_data_get_cb @_";

	#my $sub= $self->{dragsrc};
	return unless $context->{data};
	my @values = @{$context->{data}};
	#my @values=$sub->($self); return unless @values;
	
	my $srcinfo = $context->{srcinfo};
	if ($destinfo != $srcinfo) {
		my $convsub = $DRAGTYPES[$srcinfo][1]{$destinfo};

		# not sure of the encoding I should use, it's for app that don't
		# accept 'text/plain;charset=UTF-8', only found/tested with
		# gnome-terminal
...
sub drag_data_received_cb {
	my ($self, $context, $x, $y, $data, $info, $time) = @_;
	#warn "drag_data_received_cb @_";
	my $ret = my $del = 0;

	# should maybe have been passed in dragdest arguments, but would
	# require editing all existing dragdest functions
	$self->{dragdest_suggested_action} = $context->suggested_action;

	if ($data->length >= 0 && $data->format == 8) {
		my @values = split "\x0d\x0a", $data->data;

		# some apps send file:/path instead of file:///path
		s#file:/(?!/)#file:///# for @values;
...
# makes the mouse wheel scroll vertically when the horizontal scrollbar has grab (boutton pressed on the slider)
sub set_biscrolling {
	my $sw = $_[0];

	# needs gtk>=2.8
	if (*Gtk2::ScrolledWindow::get_hscrollbar{CODE}) {
...
sub CreateDir {
	...
	for my $dir (split /$QSLASH/o, $path) {
		...
		until (mkdir $current) {
			#if (-f $current) {
			#	ErrorMessage("Can't create folder '$current' :\na file with that name exists");
			#
			#	return undef;
			#}
...
sub CopyMoveFiles {
	...
	my ($sub, $errormsg0, $abortmsg) = $copy
	...
	my $dialog = Gtk2::Dialog->new($action, $parentwindow, [], 'gtk-cancel' => 'none',);
...
COPYNEXTID: for my $ID (@$IDs) {
		...
		until ($sub->($old, $new)) {
			...
			my $details = join "\n",
				__x(
					"Source: {file}",
					file => filename_to_utf8displayname($old)
				),
				__x(
					"Destination: {file}",
...
sub ChooseDir {
	...
	# there is no mode in Gtk2::FileChooserDialog that let you select both
	# files or folders (Bug #136294), so use Gtk2::FileChooserWidget
	# directly as it doesn't interfere with the ok button (in "open" mode
	# pressing ok in a Gtk2::FileChooserDialog while a folder is selected
	# go inside that folder rather than emiiting the ok response with that
	# folder selected)
	my $dialog = Gtk2::Dialog->new(
		$msg,
		undef,
...
sub FileChooser_add_filters {
	...
	for my $aref (@patterns) {
		...
		if ($aref->[2]) {
			$filter->add_pattern($_) for split / /, $aref->[2];
		}
...
sub ChoosePix {
	...
	$preview->{set_pic} = sub {
		...
		my $p = $image->{pixbuf};
		if ($p) {
			$label->set_text($p->get_width . ' x ' . $p->get_height);
		} else {
			$label->set_text('');
		}
	...
	my $update_preview = sub {
		...
		if ($file =~ m/$EmbImage_ext_re$/) {
			...
			$max = GMB::Picture::pdf_pages($file);
		}
	...
	if ($file && $file =~ s/:(\w+)$//) {
		$nb = $1;
		$lastfile = $file;
		$nb = FileTag::PixFromMusicFile($file, $nb, 1, 1) || 0 if $nb =~ m/\D/;
	}
	...
	if ($file && -f $file) {
		...
		$dialog->set_current_folder_uri("file://$path");
	}
...
# returns one of 'retry abort skip skip_all'
sub Retry_Dialog {
...
sub filenamefromformat {
	...
	if ($ext) {
		...
		# only extension -> base name on song's filename
...
sub pathfromformat {
	# $format is in utf8, $basefolder is a byte string
	my ($ID, $format, $basefolder, $icase) = @_;

	my $path = defined $basefolder ? $basefolder . SLASH : '';

	if ($format =~ s#^([^\$%]*$QSLASH)##) {
		# move constant part of format in path
		my $constant = $1;
	...
	# replace leading ~/ by homedir
	$path =~ s#^~($QSLASH)#$ENV{HOME}$1#o;

	# use song's path as base for relative paths
	$path = Songs::Get($ID, 'path') . SLASH . $path if $path !~ m#^$QSLASH#o;
...
# tries to find an existing file/folder with different case
sub ICasePathFile {
	my ($path, $folder) = @_;
	...
	unless (-e $path . $folder) {
		...
		my @files = readdir $d;
		closedir $d;
		my $lc = lc $folder; # XXX or superlc?
...
# find case-sensitive filename from a case-insensitive filename
sub CaseSensFile {
	my $file0 = shift;
...
sub DialogMassRename {
	...
	# remove duplicates IDs in @_ => @IDs
	my @IDs = uniq(@_);
	...
	# prepend because filling is a bit faster in reverse
	$store->set($store->prepend, 0, $_) for reverse @IDs;
	...
	$albox->set_sensitive($folders);
	my $vbox = Vpack($title, $table, [$checkfile, $combo], $checkfolder, $albox,);
	...
	# FIXME without it, scrollbars synchronization doesn't work until
	# treeview1 is displayed (by clicking on the 1st tab)
	$treeview1->realize;
		...
		response => sub {
			...
			if ($response eq 'ok') {
				...
				if ($folders) {
					...
					unless (defined $base) {
						ErrorMessage("You must specify a base folder", $dialog);

						return;
					}

					until (-d $base) {
						last unless $base = parentdir($base);
					}

					unless (-w $base) {
						ErrorMessage(
							__x(
								"Can't write in base folder '{folder}'.",
...
sub AddToListMenu {
	...
	my $menusub = sub {
		...
		$Options{SavedLists}{$key}->Push($IDs);
	};
...
# FIXME special case for no @keys, maybe a menu with a greyed-out item "no #none#"
sub MakeFlagMenu {
...
sub DialogSongsProp {
	...
	my $dialog = Gtk2::Dialog->new(
		"Edit Multiple Songs Properties",
		undef,
		'destroy-with-parent',
		'gtk-save'   => 'ok',
		'gtk-cancel' => 'none'
	...
	$dialog->signal_connect(
		response => sub {
			...
			if ($response eq 'ok') {
				...
				# the closure will be called when tagging finished
				# FIXME not very nice
...
sub DialogSongProp {
	...
	if (exists $Editing{$ID}) {
		$Editing{$ID}->force_present;
...
sub SongAdd_now {
	...
	# FIXME remove IDs already in Library
	# FIXME check against master filter?
...
sub SongsRemove {
	...
	# update RecentPos if needed
	$RecentPos-- while $RecentPos && $Recent->[$RecentPos - 1] != $SongID;
...
#return filenames from playlist files (.m3u, .pls, ...)
sub Parse_playlist_file {
	my $pl_file = shift;
	...
	if (!$sub) {
		warn "Unsupported playlist format '$name.$ext'\n"
	}

	open my ($fh), '<', $pl_file or do {
		warn "Error reading $pl_file : $!";
	...
	# try hard to find the correct filenames by trying different
	# encodings and case-incensivity
	# XXX if (1) ???
	if (1) {
		my @enc =
		...
		for my $enc (@enc) {
			...
			if (@found == @list) {
				...
				# give some priority to found encoding
				$Encoding_pref{$enc} ||= 1;
...
#create saved lists from playlist files (.m3u, .pls, ...)
sub Import_playlist_file {
	my $pl_file = shift;
	...
	for my $file (@files) {
		...
		#unless (defined $ID) {
		#	warn "Can't find file $file in the library\n";
		#	next;
		#}
	...
	if (@list) {
		printf STDERR "$pl_file lists %d files, %d were found, %d were not already in the library\n",
				scalar @files,
				scalar @list,
				scalar @toadd
			if $Verbose || @files != @list;
	} else {
		warn "No file from '$pl_file' found in the library\n";
...
sub FolderToIDs {
	...
	while (defined(my $dir = shift @dirs)) {
		...
		if (-d $dir) {
			...
			if ($dir =~ m/$ScanRegex/) {
				push @files, $dir;
			} elsif ($dir =~ m/\.([^.]*)$/ && $playlist_file_parsers{lc $1}) {
	...
	for my $file (@files) {
		# check if missing => check if modified
		my $ID = Songs::FindID($file);
	...
	# add IDs to the Library if needed
	SongAdd_now(@toadd) if $add && @toadd;
...
sub ScanFolder {
	...
	if (-d $dir) {
		if (opendir my ($DIRH), $dir) {
			...
			warn "ScanFolder: can't open folder $dir : $!\n";
		...
		warn "ScanFolder: can't find $dir\n"
	}

	#my @toadd;
	for my $file (@files) {
		# skip . .. and hidden files/folders
		next if $file =~ m#^\.#;
	...
	for my $jobarray ($ToCheck, $ToReRead, $ToCheckLength) {
		...
		if (@$jobarray) {
			::Progress(
				...
				# returns title, details(optional), current, end
				$jobarray->progress,
			);
...
sub AutoSelPictures {
	...
	$ref->{timeout} ||= Glib::Timeout->add(
		1000,
		\&AutoSelPictures_progress_cb,
		$field
	); #if @{$ref->{todo}}>10 ??
}
...
sub AutoSelPictures_progress_cb {
	...
	unless ($ref) {
		Progress('autopic_' . $field, abort => 1);
...
sub AutoSelPicture {
	...
	for my $m (qw/embedded guess/) {
		if ($m eq 'embedded') {
			...
			# ignore common parent folder if too short
			# FIXME compare the depth of $common with others, ignore
			# it if more than 1 or 2 depth diff
			if (length $common > 5) {
				if (!$path->{$common}) {
			...
			for my $folder (@folders) {
				...
				for my $file (grep m/$Image_ext_re/, readdir $dh) {
					...
					if ($field eq 'album') {
						$score += 100 if $file =~ m/(?:^|[^a-zA-Z])(?:cover|front|folder|thumb|thumbnail)[^a-zA-Z]/i;
					} elsif (index($file, $field) != -1) {
						$score += 10
					}
	...
	if ($set) {
		AAPicture::SetPicture($field, $gid, $set);
	}
}
...
EOF
	$dialog->set_website('https://github.com/jukebox-sound/jukebox/');
...
sub PrefKeys {
	...
	my $combo = TextCombo->new(
		{
			...
			grep defined $Command{$_}[1] && !defined $Command{$_}[3] || ref $Command{$_}[3], keys %Command
		}
	);
	...
	$combochanged = sub {
		...
		if ($Command{$cmd}[2]) {
			...
			$child = (ref $Command{$cmd}[3] eq 'CODE')
				? $Command{$cmd}[3]()
				: Gtk2::Entry->new;
	...
	my $butadd = ::NewIconButton(
		...
		sub {
			...
			if (my $child = $entry_extra->child) {
				my $extra = (ref $child eq 'Gtk2::Entry')
					? $child->get_text
					: $child->get_value;
	...
	$refresh_sensitive = sub {
...

		{
			...
			if ($Command{$cmd}[2]) {
				...
				last if $re && ref($re) ne 'CODE' && $entry_extra->child->get_text !~ m/$re/;
			}
...
sub PrefAudio {
	...
	my ($radio_gst, $radio_123, $radio_mp, $radio_mpv, $radio_ice) = NewPrefRadio(
		'AudioOut',
		...
		[
			...
			mpv                 => 'Play_mpv',
			"icecast server"    => sub {
				$Options{use_GST_for_server}  ?  'Play_GST_server'  :  'Play_Server';
			},
		],
	...
	if (exists $PlayPacks{Play_GST}) {
		...
		$vbox_gst->pack_start($_, FALSE, FALSE, 2) for $radio_gst, Gtk2::Label->new("GStreamer module not loaded.");
	}
...
EOT
	...
	$EQbut->signal_connect(
		clicked => sub {
			OpenSpecialWindow('Equalizer');
		}
	);
	...
	my $eq_cb = sub {
		$_[0]->set_sensitive($Play_package->{EQ});
	};
	...
	$rga_start->set_tooltip_text((<<EOT) =~ s/[\r\n]/ /gr);
Analyse and add replaygain tags for all songs that don't have
replaygain tags, or incoherent album replaygain tags.
EOT
...
sub PrefAudio_makeadv {
	...
	if (1) {
		...
		$label->signal_connect(
			# delay finding supported formats because mplayer is slow
			realize => sub {
				my @ext;
...
sub replaygain_options_dialog {
	if ($RG_dialog) {
		$RG_dialog->force_present;

		return;
	}

	$RG_dialog = Gtk2::Dialog->new(
		"ReplayGain options",
		undef,
		[],
		'gtk-close' => 'close'
	...
	my $songmenu = ::NewPrefCheckButton(
		gst_rg_songmenu => "Show replaygain submenu" . ($Glib::VERSION >= 1.251 ? '' : ' ' . "(unstable)")
	);
	...
	my $nolimiter_update = sub {
		$_[0]->set_sensitive($Options{AudioOut} ne 'Play_mplayer');
	};
...
sub PrefMisc {
	...
	my $checkR1 = NewPrefCheckButton(RememberPlayFilter => "Remember last Filter/Playlist between sessions");
	my $checkR3 = NewPrefCheckButton(RememberPlayTime => "Remember playing position between sessions");
	my $checkR2 = NewPrefCheckButton(RememberPlaySong => "Remember playing song between sessions", widget => $checkR3);
	my $checkR4 = NewPrefCheckButton(RememberQueue => "Remember queue between sessions");
	...
	# articles
	my $articles = NewPrefEntry(
		'Articles',
		undef,
		cb => sub {
			Delayed('UpdateArticleRE', 1000, \&Songs::UpdateArticleRE);
		}
	);
	my $articles_check = NewPrefCheckButton(
		...
		horizontal      => 1,
		tip => "If one of these words is at the start of the string, it will be ignored when sorting most fields"
	);

	# date format
	my $dateex  = mktime(5, 4, 3, 2, 0, (localtime)[5]);
	my $datetip = join "\n",
		"use standard strftime variables",
		"examples :",
		map(
			sprintf("%s : %s", $_, strftime_utf8($_, localtime($dateex))),
			split(/ *\| */, "%a %b %d %H:%M:%S %Y | %A %B %I:%M:%S %p %Y | %d/%m/%y %H:%M | %X %x | %F %r | %c | %s")
		),
		'',
	...
	my $preview = Label::Preview->new(
		...
		#preview => sub { Songs::DateString(localtime $dateex)}
		preview => sub {
			my @sec = (
				$dateex,
				map time - $_,
	...
	my $always_in_pl = NewPrefCheckButton(
		...
		tip => "- When selecting a song, the playlist filter will be reset if the song is not in it\n- Skip to another song when removing the current song from the playlist"
	);
	...
	my $recent_include_not_played = NewPrefCheckButton(
		AddNotPlayedToRecent => "Recent songs include skipped songs that haven't been played.",
		tip => "When changing songs, the previous song is added to the recent list even if not played at all."
	);

	my $playedpercent = NewPrefSpinButton(
		'PlayedMinPercent',
		0,
		100,
	...
	my $playedseconds = NewPrefSpinButton(
		'PlayedMinSeconds',
		0,
		99999,
		text => "or %d seconds"
...
sub PrefLayouts {
	...
	my $checkT5 = NewPrefCheckButton(
		...
		tip => 'If checked, the application will start with its main window hidden in the system tray instead of being shown normally.'
	);

	my $checkT2 = NewPrefCheckButton(
		CloseToTray => 'Close button sends window to tray',
		tip => "If checked, clicking the main window's close button will send the window to the system tray (hiding it) instead of exiting the application."
	);

	my $checkT6 = NewPrefCheckButton(
	    TrayIconMinimize => 'Tray icon minimizes window instead of hiding',
	    tip => 'If checked, clicking the tray icon (when the main window is visible) will minimize the window to the taskbar/dock.  If unchecked, clicking the tray icon will hide the window, removing it from the taskbar/dock/alt-tab list.  This option can be useful with certain window managers (e.g., WindowMaker).'
	);

	my $checkT3 = NewPrefCheckButton(
		ShowTipOnSongChange => "Show tray tip on song change",
		widget => $traytiplength,
		tip => 'If checked, a brief notification (tray tip) will appear near the system tray icon every time the currently playing song changes.'
	);
	...
	$vbox->pack_start($_, FALSE, FALSE, 1) for @layouts_combos, $reloadlayouts, $checkT1, $checkT2, $checkT6, $fullbutton, $icotheme;
...
sub PrefTags {
	...
	my $checkv4 = NewPrefCheckButton(
		...
		tip => "Use ID3v2.4 instead of ID3v2.3 when creating an ID3v2 tag, ID3v2.3 are probably better supported by other softwares"
	);

	my $checklatin1 = NewPrefCheckButton(
		TAG_use_latin1_if_possible => "Use latin1 encoding if possible in id3v2 tags",
	...
	my $check_unsync = NewPrefCheckButton(
		...
		tip => "itunes doesn't support unsynchronised tags last time I checked, mostly affect tags with pictures"
	);
	...
	my $nowrite = NewPrefCheckButton(
		...
		tip => "Will not write the tags except with the advanced tag editing dialog. The changes will be kept in the library instead.\nWarning, the changes for a song will be lost if the tag is re-read."
	);
...
sub UpdateTags {
	...
	my $label1 = Gtk2::Label->new("Write value of selected fields in the tags. Useful for fields that were previously not written to tags, to make sure the current value is written.");
	...
	::set_drag(
		...
		dest => [
			...
			sub {
				...
				$dialog->{label}->set_text(::__n('%d song', '%d songs', scalar @IDs));
			}
		]
	);
...
sub AskRenameFolder {
	...
	$dialog->vbox->pack_start(
		Gtk2::Label->new("Rename this folder to :"),
		FALSE,
		FALSE,
		1
	);
...
sub UpdateFolderNames {
	...
	# for unclear reasons, it is needed for non-utf8 folder names. Things
	# should be clearer once the filter code is changed to keep patterns in
	# variables, instead of including them in the eval
	utf8::upgrade($filter);
...
sub PrefLibrary {
	...
	my $addbut = NewIconButton(
		'gtk-add',
		"Add folder",
		sub { ChooseAddPath(1); }
	);
	my $rmdbut = NewIconButton(
		'gtk-remove',
		"Remove"
	);
	...
	my $CScan = NewPrefCheckButton(StartScan => "Search for new songs on startup");
	my $CCheck = NewPrefCheckButton(StartCheck => "Check for updated/deleted songs on startup");

	my $BScan = NewIconButton(
		'gtk-refresh',
		"scan now",
		sub { IdleScan(); }
	);

	my $BCheck = NewIconButton(
		'gtk-refresh',
		"check now",
		sub { IdleCheck(); }
	);
	...
	my $autoremove = NewPrefCheckButton(AutoRemoveCurrentSong => "Automatically remove current song if not found");
	my $lengthcheck = NewPrefCombo(
		...
		tip => "MP3 files without a VBR header requires a full scan to check its real length and bitrate.\n"
			. 'You can force a check for these files by holding shift when selecting "Re-read tags"'
	);

	my $Blengthcheck = NewIconButton(
		'gtk-refresh',
		"check length now",
		sub { CheckLength(); }
	);

	# refresh it 1s after creation
	$Blengthcheck->{timeout} = Glib::Timeout->add(
		1000,
		\&PrefLibrary_update_checklength_button,
		$Blengthcheck
	);

	# refresh it 2s after a change
	Watch(
		$Blengthcheck,
		$_ => sub {
			my ($button, $IDs, $fields) = @_;

			$button->{timeout} ||= Glib::Timeout->add(
				2000,
				\&PrefLibrary_update_checklength_button,
				$button
			) if !$fields || grep($_ eq 'length_estimated', @$fields);
		}
	) for qw/SongsAdded SongsChanged/;

	my $masterfilter = FilterCombo->new(
		$Options{MasterFilter},
		sub {
			$Options{MasterFilter} = $_[1];
			UpdateMasterFilter();
		}
	);
...
sub RenameLabel {
	...
	if ($ok && $new !~ m/^\s*$/ && $new ne $old) {
		my $list = Songs::AllFilter(
			Songs::MakeFilterFromGID($field, $gid)
		);
...
sub NewPrefRadio {
	...
	while ($i < $#$text_val) {
		...
		my $label = Gtk2::Label->new_with_format(
			$opt{markup} || '%s',
			$text
		);
		$radio->add($label);
...
# To be used with NewPrefComboText and NewPrefFileEntry
sub PrefSaveHistory {
...
sub NewPrefFileEntry {
	...
	if ($key_history) {
		...
		# won't work with filenames with broken encoding
		$widget->append_text(decode_url($_)) for grep length, @$hist;    
	}
	...
	my $enc_warning = Gtk2::Label->new("Warning: using a folder with invalid encoding, you should rename it.");
	...
	if ($sg2) {
		$sg2->add_widget($hbox2);
	}

	$entry->set_tooltip_text($tip) if defined $tip;
	if (defined $Options{$key}) {
		$entry->set_text(
			filename_to_utf8displayname(
				decode_url($Options{$key})
			)
		);
	...
	$button->signal_connect(
		clicked => sub {
			...
			# could simply $entry->set_text(), but wouldn't work with
			# filenames with broken encoding
...
sub NewPrefSpinButton {
	...
	# FIXME using text1 and text2 is deprecated and will be removed, use
	# text with %d instead, for example text=>"value : %d seconds"
	my ($text, $text1, $text2, $sg1, $sg2, $tip, $sub, $climb_rate, $digits, $stepinc, $pageinc, $wrap) = @opt{
...
sub NewPrefMultiCombo {
	...
	# label in the button is a constant
	$display_cb = sub { $opt{display} } if !ref $display_cb;
	my $button = Gtk2::Button->new;
...
sub SaveList {
	...
	if (defined $newname) {
		...
		delete $saved->{$name};
		HasChanged('SavedLists', $name, 'remove');
	}
...
sub Watch {
	...
	# key used to store the callback(s) in the object's hash
	my $cbkey = 'WatchUpdate_' . $key;
...
# warning: if one object watch the same event with multiple callbacks, all of them will be removed
sub UnWatch {
	my ($object, $key) = @_;
	...
	# re-weaken references (the grep above made them strong again) #do not
	# weaken code ref as they might not be linked to anything else and
	# should be permanent
	ref $_ ne 'CODE' && weaken($_) for @{$EventWatchers{$key}};

	delete $object->{'WatchUpdate_' . $key};
}

# for when destructing object (unwatch Watch() AND WatchFilter())
sub UnWatch_all {
	my $object = shift;
...
sub SetFilter {
	...
	# set filter for level $level
	$filters->[$level + 1] = $filter;

	# set higher level filters to undef by truncating the array
	$#$filters = $level + 1;

	# sum filter
	$filters->[0] = Filter->newadd(TRUE, map($filters->[$_], 1 .. $#$filters));

	AddToFilterHistory($filters->[0]);

	for my $r (@{$FilterWatchers{$group}}) {
		$r->{'UpdateFilter_' . $group}($r, $Filters{$group}[0], $level, $group);
	}

	if ($group eq 'Play') {
		$ListPlay->SetFilter($filters->[0])
	}
}
...
sub Progress {
	...
	if ($Progress{$pid} && !$self->{widget} && (!$EventWatchers{Progress} || @{$EventWatchers{Progress}} == 0)) {
		# create the progress window only after a short timeout to ignore short jobs
		$ProgressWindowComing ||= Glib::Timeout->add(
			1000,
			sub {
				my $still_progress = grep !$Progress{$_}{widget}, keys %Progress;
...
sub CreateTrayIcon {
	if ($TrayIcon) {
		...
		return;
	} elsif (!$Options{UseTray} || !$TrayIconAvailable) {
		return;
	}
	...
	if ($UseGtk2StatusIcon) {
		...
		# recreate Trayicon if it is deleted, for example when the
		# gnome-panel crashed, but only if it has lived >1sec to avoid
		# an endless loop
...
sub windowpos {
	...
	if ($event->isa('Gtk2::StatusIcon')) {
		# position and size of statusicon
		($x, $y, $dx, $dy) = ($event->get_geometry)[1]->values;
		...
		# position of the clicked widget on the screen
		($x, $y) = $event->window->get_origin;

		# width,height of the clicked widget
		($dx, $dy) = $event->window->get_size;
...
sub ShowHide {
	...
	if ($hide) {
		...
		for my $win (@windows, $MainWindow) {
			...
			#need to re-set it, is it a gtk bug, metacity bug ?
			#$win->set_opacity($win->{opacity}) if exists $win->{opacity} && $win->{opacity}!=1;
		}
...
sub new {
	...
	if (defined $name && $name ne '') {
		my $button = ::NewIconButton('gtk-save', ::__x("save as '{name}'", name => $name));
	...
	if (defined $name) {
		if ($name eq '') {
			...
			$init = $self->{hash}{$name} unless defined $init;
		}
	}
...
sub Result {
		...
		$self->signal_connect(
			response => sub {
				...
				if ($ans eq 'ok' || $ans eq 'apply') {
					...
					::SaveSFRG($self->{key}, $self->{save_name}, $result) if $ans eq 'ok' && defined $self->{save_name} && $self->{save_name} ne '';
	...
	# FIXME stop using this, always supply a $sub
	if ('ok' eq $self->run) {
			...
			sub {
				...
				if ($rowref_todel) {
					my $path =
						...
						: $pos =~ m/after$/    ? Gtk2::TreePath->new_from_indices(0, 0)
...
sub Set {
	...
	if ($startpath) {
		...
		# add a root
		if (!$parent && !$store->iter_has_child($iter)) {
	...
	unless ($store->get_iter_first) {
		# default filter if no/invalid filter
...
sub _set_row {
	...
	my $desc =
		  $content eq '&'  ?  "All of :"
		: $content eq '|'  ?  "Any of :"
		:                     Filter::_explain_element($content) || "Unknown filter :" . " '$content'"
		;
...
sub new {
	...
	my $order_column = Gtk2::TreeViewColumn->new_with_attributes(
		'Order',
	...
	my $butadd = ::NewIconButton('gtk-add', "Add", sub { $self->Add_selected });
	my $butrm = ::NewIconButton('gtk-remove', "Remove", sub { $self->Del_selected });
	my $butclear = ::NewIconButton('gtk-clear', "Clear", sub { $self->Set(''); });
...
sub Add_selected {
	...
	my $i = (Songs::SortICase($f)) ? INSENSITIVE : 0;
...
sub Move_Selected {
	...
	if ($max) {
		if ($up) {
			$store2->move_after($iter, undef);
		} else {
			$store2->move_before($iter, undef);
		}
...
# Update sensitive state of buttons
sub Buttons_update {
	my $self = shift;
	...
	if ($sel) {
		...
		$self->{$_}->set_sensitive(0) for qw/butrm butup butdown/;
	}
}
...
sub new {
	...
	$addhbox->pack_start($_, FALSE, FALSE, 0) for Gtk2::Label->new("Add rule : "), $addlist, $addbut;
	...
	$histogram->signal_connect(
		enter_notify_event => sub {
			$_[0]{timeout} = Glib::Timeout->add(500, \&UpdateTip_timeout, $histogram);
	...
	$histogram->signal_connect(
		leave_notify_event => sub {
			Glib::Source->remove($_[0]{timeout});
...
sub AddRow {
	...
	my $check_tip;
	if ($type eq 'f') {
		$check_tip = "ON less probable if label is set\nOFF more probable if label is set";
		...
		$check_tip = "ON less probable if genre is set\nOFF more probable if genre is set";
		...
		$check_tip = "ON -> smaller means more probable\nOFF -> bigger means more probable";
		...
		$hbox->pack_start($_, FALSE, FALSE, 0) for Gtk2::Label->new("half-life : "), $spin, Gtk2::Label->new($frame->{unit} || '');
	...
	$_->signal_connect(parent_set => sub { $_[0]->destroy unless $_[0]->parent }) for $frame, $button;    
}
...
sub UpdateID {
	...
	if ($s) {
		...
		$prob = ::__x(
			...
			probability => ::format_number($prob, ($prob >= 10 ? '%.0f' : '%.1f'))
		);
	} else {
		$prob = "0 chance";
	}
...
sub new {
	my ($class, $val, $opt) = @_;
	my $self = bless GMB::ListStore::Field::Combo->new(
		$opt->{field},
		$val,
...
# Alternative to GMB::FilterEdit::Combo that better handle long list
# of values, and works with album filters.
...
sub set_gid {
	...
	# FIXME ugly way to get the sgid
...
sub new {
	...
	$button->signal_connect(
		clicked => sub {
			my $self = $_[0]->parent;
			my $folder = ::ChooseDir("Choose a folder", path => $self->{value});
...
our @ISA;
BEGIN {
	unshift @ISA, 'TextCombo';
}

sub new {
	my ($class, $val, $opt) = @_;

	my $self = bless TextCombo->new(
		[ keys %{$::Options{SavedLists}} ],
		$val,
		\&GMB::FilterBox::changed
	), $class;
...
sub fill {
	$_[0]->build_store([ keys %{$::Options{SavedLists}} ]);
}
...
sub new {
	...
	if (my $unit = $opt->{unit}) {
		...
		if (ref $unit eq 'HASH') {
			...
			$set_digits->($extra);
			$spin->signal_connect(
				# catch letter key-press to change unit
				key_press_event => sub {
...
sub new {
	...
	# default to today 0:00 or today 23:59
	$self->{date} = $val || ::mktime(($opt->{value_index} ? (59, 59, 23) : (0, 0, 0)), (localtime)[3, 4, 5]);
...
sub pixbuf {
	...
	unless ($pb || $cacheonly) {
		...
		# don't bother caching animation
		GMB::Cache::add_pb($key, $pb) if $pb && $pb->isa('Gtk2::Gdk::Pixbuf');
	}
...
sub load {
	...
	if ($file =~ m/$::EmbImage_ext_re$/) {
		...
		# pdftocairo with svg, slower but no temp file
		#if ($pdftocairo) {
		#	open $fh,'-|', "pdftocairo -svg -r 150 -f $n -l $n $qfile -";
		#}
		...
		# eval{Gtk2::Gdk::Pixbuf->new_from_file(filename_to_unicode($file))};
...
sub RenameFile {
	...
	$old = qr/^\Q$dir$old\E$/;
	for my $ref (@ArraysOfFiles) {
		s#$old#$dir$new# for grep $_, @$ref
	}
	return $new;
}
...
sub UpdatePixPath {
	...
	# make sure the path ends with SLASH
	$_ = ::pathslash($_) for $oldpath, $newpath;

	$oldpath = qr/^\Q$oldpath\E/;
	for my $ref (@ArraysOfFiles) {
		s#$oldpath#$newpath# for grep $_, @$ref
	}
}
...
# zoom picture when clicked
sub pixbox_button_press_cb {
...
sub new {
	...
	# rows with empty string in col 1 is separator
	if ($opt{separator}) {
		$self->set_row_separator_func(sub { $_[0]->get($_[1], 1) eq ''; });
	}

	# hide title of submenus
	...
	) if $self->get_model->isa('Gtk2::TreeStore');
...
sub build_store {
	...
	$self->{ordered_hash} = 1 if $opt{ordered_hash};
...
sub make_toolitem {
	# $self should be contained in $widget (or $widget=undef)
	my ($self, $desc, $menu_item_id, $widget) = @_;
	$widget       ||= $self;
	$menu_item_id ||= "$self";
	my $titem = Gtk2::ToolItem->new;
	$titem->add($widget);
...
sub fill_store {
	...
	my $store = $self->get_model;
	$store->clear;
	$store->set($store->append, 0, "All songs", 1, Filter->new, 2, 'gmb-library');
...
sub new {
	...
	if ($event) {
		::Watch($self, $_ => \&queue_update) for split / /, $event;
	}
...
sub Start {
	...
	# can be a scalar ref to get downloaded data into it, not currently
	# supported for local files, and only supported for 1 uri
	...
	if ($uri =~ s#^file://##) {
		...
		my ($sub, $errormsg, $abortmsg) = $is_move
			? (\&::move, "Move failed", "abort move")
			: (\&::copy, "Copy failed", "abort copy");
		...
		unless (eval { require $::HTTP_module }) {
			warn "Loading $::HTTP_module failed, can't download $display_uri\n";
...
sub Downloaded {
	...
	# if uri doesn't have a file name, try to find a good default name,
	# currently mostly used for pictures, so doesn't try very hard for
	# non-pictures
	my $is_pic = $type =~ m#^image/(gif|jpeg|png|bmp)# ? $1 : 0;

	# make sure common picture types have the correct extension
	$ext = '' if $is_pic && $ext !~ m/^(?:gif|jpe?g|png|bmp)$/i;
	if ($ext eq '') {
		$ext =
	...
	if ($file eq '') {
		if ($is_pic) {
			...
			# not a picture and no name, should maybe ask to confirm
...
# file done, if no $self->{newfile} it means the file has been skipped
sub Done {
	my $self    = shift;
...
# GMB::DropURI object must not be used after that
sub Abort {
	my $self = shift;
