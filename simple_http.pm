...
sub receiving_cb {
	...
	if ($headers{'content-disposition'}
	 && $headers{'content-disposition'} =~ m#^\s*\w+\s*;\s*filename(\*)?=(.*)$#mgi) {
		$filename = $2;
		my $rfc5987 = $1;
		...
		if ($rfc5987 && $filename =~ s#^([A-Za-z0-9_-]+)'\w*'##) {
			...
			if ($filename =~ s/^"(.*)"$/$1/) {
				...
				$filename =~ s#\\(.)#"\x00".ord($1)."\x00"#ge;
			} elsif ($filename =~ m#[^A-Za-z0-9_.\x00-]#) {
				$filename = ''
			}
		}
	...
	if (my $enc = $headers{'content-encoding'}) {
		if ($enc eq 'gzip' && $gzip_ok) {
			...
			IO::Uncompress::Gunzip::gunzip(\$gzipped, \$response) or do {
				warn "simple_http : gunzip failed: $IO::Uncompress::Gunzip::GunzipError\n";
	...
	} elsif ($result =~ m|^HTTP/1\.\d+ 30[123]| && $headers{location}) {
		# Redirection
