...
my ($GST_visuals_ok, $GST_EQ_ok, $GST_RG_ok);
our $GST_RGA_ok;
...
BEGIN {
	...
	if ($reg->lookup_feature('equalizer-10bands')) {
		...
		warn "gstreamer1 plugin 'equalizer-10bands' not found -> equalizer not available\n";
	}

	if ($reg->lookup_feature('rglimiter') && $reg->lookup_feature('rgvolume')) {
		$GST_RG_ok = 1;
	} else {
		warn "gstreamer1 plugins 'rglimiter' and/or 'rgvolume' not found -> replaygain not available\n";
	}

	if ($reg->lookup_feature('rganalysis')) {
		$GST_RGA_ok = 1;
	} else {
		warn "gstreamer1 plugins 'rganalysis' not found -> replaygain analysis not available\n";
	}
	...
	*GStreamer1::Element::link_many = sub { while (@_ > 1) { my $e = shift; $e->link($_[0]) } } unless *GStreamer1::Element::link_many{CODE};
	...
	eval {
		Glib::Object::Introspection->setup(
			basename => 'GstVideo',
			version  => '1.0',
			package  => 'GStreamer1::Video'
		);
	}; #unless *GStreamer1::Video::VideoOverlay::set_window_handle{CODE};
	if ($@) {
		$GST_visuals_ok = 0;
	...
	if (1) {
		eval {
			Glib::Object::Introspection->setup(
				basename => 'GstPbutils',
				version  => '1.0',
				package  => 'GStreamer1::Pbutils'
			);
		}; # unless *GStreamer1::Pbutils::pb_utils_init{CODE};
		if (!$@) {
			...
			warn "Can't setup GStreamer1::Pbutils -> Installing missing plugins not supported\n";
		}
	}
}
...
sub init {
	...
	# find a default sink
	$::Options{gst_sink} ||= (grep($reg->lookup_feature($_ . 'sink'), qw/autoaudio pulse alsa oss oss4/), 'autoaudio')[0];
...
sub create_playbin {
	...
	# can cause a freeze in some circumstances, in particular when using
	# the scroll wheel on the time bar, see perl-glib bug #620099
	# (https://bugzilla.gnome.org/show_bug.cgi?id=620099)
	$pb->signal_connect(
		"notify::volume" => sub {
			Glib::Idle->add(\&VolumeChanged, $_[0]) unless $VolumeHasChanged++;
		},
		100000
	) if $::Options{gst_monitor_pa_volume}; 
...
sub bus_message_end {
	...
	if ($self->{continuous}) {
		$self->{sink}->set_locked_state(1);
		$self->{playbin}->set_state('null');
		$self->{sink}->set_locked_state(0);
	} else {
		$self->{playbin}->set_state('null');
	}

	#can't use $msg->parse_error as it doesn't work currently :
	#"FIXME - GI_TYPE_TAG_ERROR" (Glib::Object::Introspection-0.027)
	if ($error) {
		::ErrorPlay(_parse_error($msg));
	} else {
		::end_of_file();
	}
}
...
sub bus_message_missing_plugin {
	...
	GStreamer1::Pbutils::install_plugins_async(
		[ $details ],
		undef,
		sub {
			return unless $_[0] =~ /success/; # success or partial_success

			# FIXME "Applications should assume that the registry
			# update is neither atomic nor thread-safe and should
			# therefore not have any dynamic pipelines running
			# (including the playbin and decodebin elements) and
			# should also not create any elements or access the
			# GStreamer registry while the update is in progress"
			GStreamer1::update_registry(); 
		}
	);
}

# when a song starts playing, notify::volume callbacks are called, which causes
# glib to hang (see https://bugzilla.gnome.org/show_bug.cgi?id=620099#c11) if a
# skip is done at the same moment using freeze_notify until the skip is done
# mostly avoid the problem setting state to paused until the skip is done also
# mostly avoid the problem but the hang still happens in some cases, in
# particular when using the scroll wheel to change the position in the song,
# and also very rarely when starting a song
my $StateChanged;

# used to wait for the right state to do the skip
sub bus_message_state_changed {
	...
	$self->{playbin}->freeze_notify unless $self->{notify_frozen};
	$self->{notify_frozen} = 1; # freeze notify until skip is done
	Glib::Idle->add(
		sub {
			# this will only skip if the state is right, else will wait for another state change
			$self->SkipTo($self->{skip}) if $self->{skip}; 

			#if skip is done, unfreeze
			unless ($self->{skip}) {

				$self->{playbin}->thaw_notify if delete $self->{notify_frozen};
			}
...
sub init_sink {
	...
	my $sink = $self->{sink};
	unless ($sink) {
		::ErrorPlay(::__x("Can't create sink '{sink}'", sink => $::Options{gst_sink}));
		return;
	}
	...
	if ($sink->{RG}) {
		my ($rgv, $rgl, $ac, $ar) = map GStreamer1::ElementFactory::make($_ => $_), qw/rgvolume rglimiter audioconvert audioresample/;
	...
	if (my $custom = $::Options{gst_custom}) {
		# make a Bin by default instead of a pipeline
		$custom = "( $custom )" if $custom =~ m/^\s*\w/ && $custom =~ m/!/;
		...
		if ($elem && $elem->isa('GStreamer1::Bin')) {
			...
			$elem->add_pad(
			    GStreamer1::GhostPad->new('sink', $last->get_static_pad('sink'))
			);
			$elem->add_pad(
			    GStreamer1::GhostPad->new('src', $first->get_static_pad('src'))
			);
		}
...
sub SkipTo {
	...
	return if $result eq 'async';

	$playbin->seek(1, 'time', 'flush', 'set', $skip * 1_000_000_000, 'none', 0);
	if (my $new = delete $self->{state_after_skip}) {
		$playbin->set_state($new);
	}
	delete $self->{skip};
}
...
sub SetVolume {
	...
	# use a cubic volume scale
	$pb->set(volume => (($::Mute || $::Volume) / 100)**3, mute => !!$::Mute) if $pb;
	$self->{volume_busy} = 0;
...
# return false if option to sync is disabled
sub _throwaway_equalizer {
...
sub EQ_Import_Presets {
	...
	for my $name (@{$equalizer->get_preset_names}) {
		# ignore if one by that name already exist
		next if $::Options{equalizer_presets}{$name};
...
sub EQ_Get_Range {
	...
	{
		my $equalizer = $self->{playbin}->get_by_name('equalizer') || GStreamer1::ElementFactory::make('equalizer-10bands' => 'equalizer');
...
sub add_visuals {
	...
	unshift @{$self->{visual_windows}}, $window;

	::weaken($self->{visual_windows}[0]);
	...
	$window->signal_connect(
		unrealize => sub {
			...
			$self->remove_visuals($_[0]) if $self;
		}
	);
	...
	$window->signal_connect(
		expose_event => sub {
			...
			if ($self && $_[0]{visuals_on} && defined $::TogPlay) {
				$self->{playbin}->expose;
			} else {
				# not connected or stopped -> draw black background
				my ($widget, $event) = @_;
...
sub remove_visuals {
	...
	# re-weaken references (the grep above made them strong again)
	::weaken($_) for @$wlist;
...
sub set_visual {
	...
	if ($visual eq '+') {
		# choose next visual in the list
		...
		$i = 0 if $i > $#l;
		$visual = $l[$i];
	} elsif (!(grep $_ eq $visual, @l)) {
		# if visual not found in list
		$visual = undef;
	}
...
my $RGA_songmenu = {
	...
	notmode => 'P',
	test => sub {
		$Play_GST::GST_RGA_ok && $::Options{gst_rg_songmenu};
	},
	submenu => [
		{
...
sub Analyse_full {
	...
	my ($again, $apeak, $ids) = Songs::BuildHash(
		'album',
		$IDs_in_album,
		undef,
		'replaygain_album_gain:same',
		'replaygain_album_peak:same',
		'id:list'
	);
...
sub Analyse {
	...
	unless ($RGA_pipeline) {
		...
		my ($src, $decodebin, $ac, $ar, $rganalysis, $fakesink) = map GStreamer1::ElementFactory::make($_ => $_), qw/filesrc decodebin audioconvert audioresample rganalysis fakesink/;
		...
		# (Glib::Object::Introspection-0.027)
		$bus->signal_connect(
			'message::error' => sub {
				warn "ReplayGain analysis error : " . join(":\n ", Play_GST::_parse_error($_[1])) . "\n";
			}
		);
	...
	process_next() unless $::Progress{replaygain}{current};
}
...
sub process_next {
	# called from callback => one file has been scanned => increment
	::Progress('replaygain', inc => 1) if $_[0];
	...
	if (defined $ID) {
		...
		if ($rganalysis->is_locked_state) {
			# for album mode: unlock $rganalysis

			# For some reason, GStreamer 1.0's rganalysis element
			# produces an error here unless a flush has been
			# performed
...
sub bus_message_tag {
	...
	return unless $count && $count == $taglist->n_tags;
...
sub Stop {
	...
	unless ($partialstop) {
		...
		for (keys %$sockets) {
			$sockets->{$_}[1] = 0;
			$self->{stream}->signal_emit(remove => $_);
		}
...
sub create_sink {
	...
	$stream->signal_connect(
		'client-fd-removed' => sub {
			my $sockets = $_[0]{sockets};
			close $sockets->{$_[1]}[0];
			delete $sockets->{$_[1]};
			::QHasChanged('connections');
		}
	);
...
sub Connection {
	...
	my $answer =
		...
		. "x-audiocast-name: jukebox stream"                     . EOL
	...
	Glib::IO->add_watch(
		...
		sub {
			...
			my $stream = $_[2];
			$stream->{sockets}{$fileno}[1] = 0;

			::HasChanged('connections');

			$stream->signal_emit(remove => $fileno);
