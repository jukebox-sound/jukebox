...
sub init {
	...
	# FIXME RG should be 0 if replaygain tags are disabled or if not $SoftVolume
...
sub VolInit {
	...
	$SoftVolume = !system($mplayer, qw/-really-quiet -softvol -volume 100/)
		unless defined $SoftVolume;
...
sub launch_mplayer {
	...
	# -nocache because when using the cache option it spawns a child
	# process, which makes monitoring the mplayer process much harder
	#
	# -hr-mp3-seek to fix wrong time with some mp3s
	#
	@cmd_and_args = (
		$mplayer,
		qw#-nocache -idle -slave -novideo -nolirc -hr-mp3-seek -msglevel all=1:statusline=5:global=6 -input nodefault-bindings:conf=/dev/null#
	);
...
sub set_equalizer {
	...
	# should be able to do either af_add or af_cmdline, but for some reason
	# af_add doesn't set the equalizer, so do either af_add+af_cmdline or
	# af_cmdline
...
sub _remotemsg {
	for (<$OUTPUTfh>) {
		if ($playcounter == 1 && m#^A:\s*(\d+)\.(\d) #) {
			::UpdateTime($1 + ($2 >= 5 ? 1 : 0));
		...
		if (m#^(Could not open/initialize audio device)# || m#^(Failed to open .+)# || m#^(Failed to recognize file format)#) {
			handle_error($1);
...
sub _Kill_timeout {
	...
	# checks to see which ones are still there
...
sub AdvancedOptions {
	...
	my $opt = ::NewPrefEntry('mplayeroptions', "mplayer options :", sizeg1 => $sg1);
	my $cmd = ::NewPrefEntry(
		'mplayer_cmd',
		"mplayer executable :",
		cb => \&init,
...
# convert a linear volume to cubic volume scale and apply $GainFactor
sub convertvolume {
	my $vol = $_[0];
	$vol = 100 * ($vol / 100)**3;
	$vol *= $GainFactor;
