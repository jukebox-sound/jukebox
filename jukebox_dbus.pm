...
sub Set {
	...
	# return false on error, and true if ok
	Songs::SetTagValue(@$array);
}
...
# initialize once the main gmb init is finished
Glib::Idle->add(\&init);
...
sub simple_call {
	# $service_path can be service_and_path separated by space, or the
	# object
	...
	my $return = eval {
		...
		$reply &&=
			  $reply eq 'async'   ? dbus_call_async
			: $reply eq 'noreply' ? dbus_call_noreply
			:                       undef;
...
sub DBus_mainloop_hack {
	# use Net::DBus internals to connect it to the Glib mainloop, though
	# unlikely, it may break with future version of Net::DBus
