...
sub new {
	...
	# start of flac stream (in case an id3v2 tag is at the beginning of the
	# file)
	$self->{startaudio} = 0;
	...
	$self->{info}{bitrate} = $self->{info}{seconds}
...
sub write_file {
	...
	if ($inplace) {
		...
		print $tmpfh $towrite . $pictures . $header . $$newcom_packref . $padding
			or warn $!;
...
sub _ReadInfo {
	...
	#
	# B <16>  The maximum block size (in samples) used in the stream.
	# (Minimum blocksize == maximum blocksize) implies a fixed-blocksize
	# stream.
	#
	# C <24>  The minimum frame size (in bytes) used in the stream.  May be
	# 0 to imply the value is not known.
	#
	# D <24>  The maximum frame size (in bytes) used in the stream. May be
	# 0 to imply the value is not known.
	#
	# E <20>  Sample rate in Hz. Though 20 bits are available, the maximum
	# sample rate is limited by the structure of frame headers to
	# 1048570Hz. Also, a value of 0 is invalid.
	#
	# F <3>   (number of channels)-1. FLAC supports from 1 to 8 channels
	#
	# G <5>   (bits per sample)-1. FLAC supports from 4 to 32 bits per
	# sample. Currently the reference encoder and decoders only support up
	# to 24 bits per sample
	#
	# H <36>  Total samples in stream. 'Samples' means inter-channel
	# sample, i.e. one second of 44.1Khz audio will have 44100 samples
...
sub _ReadPicture {
	...
	eval {
		my ($type, $mime, $desc, undef, undef, undef, undef, $data) = unpack 'N N/a N/a NNNN N/a', $$packref;
...
sub _UnpackComments {
	...
	my ($vstring, @comlist) = eval {
		unpack 'V/a V/(V/a)', $$packref;
	};
	if ($@) {
