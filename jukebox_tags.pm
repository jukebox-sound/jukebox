...
sub Read {
	...
	warn "Reading tags for $file"
		. ($findlength ? " findlength=$findlength" : '')
	...
	# filelength == 1 --> may return estimated length (mp3 only)
	my $filetag = eval { $package->new($file, $findlength); };
	unless ($filetag) {
		warn $@ if $@;
		warn "Can't read tags for $file\n";
	...
	if (my $info = $filetag->{info}) {
		#audio properties
		if ($findlength != 1 && $info->{estimated}) {
			delete $info->{$_} for qw/seconds bitrate estimated/;
		}
	...
	for my $field (@fields) {
		for (my $i = 0; $i < $#taglist; $i += 2) {
			#$id is type of tag: id3v1 id3v2 ape vorbis lyrics3v2 ilst
			my $id = $taglist[$i];
			my $tag = $taglist[$i + 1];
			my $value;
			my $def = $Songs::Def{$field};

			if (defined(my $keys = $def->{$id})) {
				# generic cases
				...
				for my $key (split /\s*[|&]\s*/, $keys) {
					...
					# for tag-specific postread function
					my $func = 'postread';
					$func .= ":$1" if $key =~ s/^(\w+)\(\s*([^)]+?)\s*\)$/$2/;

					my $fpms_id;
					$fpms_id = $1 if $key =~ m/FMPS_/ && $key =~ s/::(.+)$//;
					my @v = $tag->get_values($key);
					next unless @v;

					if (defined $fpms_id) {
						@v = (FMPS_hash_read($v[0], $fpms_id));
						next unless @v;
					}
...
sub Write {
	my ($file, $modif, $errorsub) = @_;

	warn "FileTag::Write($file,[@$modif],$errorsub)\n" if $::debug;

	if (!-f $file) {
		warn "FileTag::Write: can't find file '$file'\n";
		return;
	}

	my ($format) = $file =~ m/\.([^.]*)$/;
	unless ($format and $format = $FileTag::FORMATS{lc $format}) {
		warn "FileTag::Write: unknown file extension for '$file'\n";
	...
	unless ($tag) {
		warn "FileTag::Write: can't read tags for '$file'\n";
	...
	while (@taglist) {
		...
		while ($i < $#$modif) {
			...
			# these keys will be updated (first one and ones separated by &)
			push @keys, split /\s*&\s*/, $wkey;
			...
			for my $key (@keys) {
				...
				# for tag-specific prewrite function  "function( TAG )"
				my $func = 'prewrite';
				$func .= ":$1" if $key =~ s/^(\w+)\(\s*([^)]+?)\s*\)$/$2/;
				...
				# FMPS list field such as FMPS_Rating_User
				if ($key =~ m/FMPS_/ && $key =~ s/::(.+)$//) {
...
sub PixFromMusicFile {
	...
	# index can be specified as argument or in the filename
	if ($file =~ s/:(\w+)$//) {
		$nb = $1;
	}
...
sub WriteLyrics {
	my ($ID, $lyrics) = @_;
	Write(
		Songs::GetFullFilename($ID),
		[ embedded_lyrics => $lyrics ],
		sub {
...
INIT {
	@Tools = (
...

		{
			...
			for_all => sub {
				join '', map ucfirst lc, split /(\W+)/, $_[0];
			},
		},
	);
...
sub new {
    ...
    # folder name at the top
    {
        ...
        my $text =
          ::__n("%d file in {folder}", "%d files in {folder}", scalar @IDs);
        $text = ::__x($text,
            folder =>
              ::MarkupFormat('<small>%s</small>', $displaysub->($folder)));
    ...
    for my $field (Songs::EditFields('many')) {
        ...
        $check->signal_connect(
            toggled => sub {
                my $check = shift;
                $check->{widget}->set_sensitive($check->get_active);
            }
        );
        my ($row, $col) = $widget->{noexpand} ? ($row2++, 2) : ($row1++, 0);
        $table->attach($check, $col++, $col, $row, $row + 1, 'fill', 'shrink',
            3, 1);
        $table->attach($widget, $col++, $col, $row, $row + 1,
            ['fill', 'expand'],
            'shrink', 3, 1);
    }
...
# do not add per-file part if LOTS of songs, building the GUI would be too long anyway
    ...
    $self->set_size_request(-1, 400)
      ; #to allow resizing the window to a small height in spite of the height request of $sw
    return $self;
}
...
sub add_per_file_part {
    ...
    my $BSelFields = Gtk2::Button->new("Select fields");
    {
        ...
        for my $f (Songs::EditFields('per_id')) {
            my $item =
              Gtk2::CheckMenuItem->new_with_label(Songs::FieldName($f));
    ...
    #add filename column
    $perfile_table->attach(
        Gtk2::Label->new(Songs::FieldName('file')),
        $lastcol, $lastcol + 1,
        $row, $row + 1, 'fill', 'shrink', 1, 1
    );
    for my $ID (@$IDs) {
        $row++;
    ...
    $expander->signal_connect(
        activate => sub {
            my $on = !$_[0]->get_expanded;
            $_->set_visible($on) for $sw, $BSelFields;
        }
    );

    $self->{vpaned}
      ->pack2(::Vpack('compact', [$expander, $BSelFields], '_', $sw),
        TRUE, FALSE);
    my $vsizegroup = Gtk2::SizeGroup->new('vertical');
    $vsizegroup->add_widget($_)
      for $exp_label, $BSelFields;    # so that they are aligned
    $sw->set_size_request(-1, $exp_label->size_request->height)
      ;                               # so that the expander is always visible
    ...
    $hbox->pack_start($_, FALSE, FALSE, 0)
      for Gtk2::VSeparator->new, $Bautofill, $BClear, $checkOBlank, $Btools,
...
sub add_column {
    ...
    for my $ID (@{$self->{IDs}}) {
        ...
        $table->attach($widget, $col, $col + 1, $row, $row + 1, $p, 'shrink',
            1, 1);
        $widget->show_all;
        push @entries, $widget;
    }
    ...
    {    #$_->set_alignment(1) for @entries;
        ...
        my $button =
          ::NewIconButton('gtk-go-down', undef, $autosub, 'none', $tip);
        $button->set_border_width(0);
        $button->set_size_request();
        $check->signal_connect(
            toggled => sub { $button->set_sensitive($_[0]->get_active) });
    ...
    for my $ID (@{$self->{IDs}}) {
        ...
        $check->signal_connect(
            toggled => sub {
                ...
                $self->{pf_widgets}{$_}[$i]->set_sensitive($active)
                  for keys %{$self->{pf_widgets}};
            },
            $i
        );

        #$widget->signal_connect(focus_in_event=> \&scroll_to_entry);
        $table->attach($check, $col, $col + 1, $row, $row + 1, 'fill',
            'shrink', 1, 1);
        $check->show_all;
        push @toggles, $check;
        $i++;
...
sub autofill_check {
	...
	for my $ref (@FORMATS_user, @FORMATS) {
		...
		# ignore patterns that match less than half of the filenames
		next if @files / 2 > (grep m/$re/, @files);
	...
	$store->set(
		$store->append,
		0,
		::PangoEsc("Edit auto-fill formats ..."),
		1,
		\&GMB::Edit::Autofill_formats::new
	);
...
sub autofill_user_formats {
	...
	for my $format (sort keys %$h) {
		...
		my $qr = eval { qr/$re/i; };
		if ($@) {
			warn "Error compiling regular expression for '$format' : $re\n$@";
...
sub autofill_cb {
    ...
    if (ref $ref eq 'CODE') {
        $ref->($self);
        return;
    }    # for edition of filename formats
    my ($format, $pattern) = @$ref;
    my @fields = GMB::Edit::Autofill_formats::find_fields($format);
    $_ eq 'album_artist' and $_ = 'album_artist_raw'
      for @fields;    #FIXME find a more generic way to do that
    ...
    for my $f (@fields) {
        ...
        for my $e (@$entries) {
            ...
            next
              if $OBlank
              && !($e->can('is_blank') ? $e->is_blank : $e->get_text eq '');
...
sub save {
    ...
    Songs::Set(
        ...
        callback_finish => $finishsub,
        window          => $self->get_toplevel
    );
}

######################################################################
# GMB::Edit::Autofill_formats                                        #
######################################################################
...
sub new {
    my $ID = $_[0]{IDs}[0];
    if ($Instance) {
        $Instance->force_present;
        $Instance->{ID} = $ID;
        $Instance->preview_update;
        return;
    }
    my $self = Gtk2::Dialog->new("Custom auto-fill filename formats",
        undef, [], 'gtk-close' => 'none');
    ...
    my $table = ::MakeReplaceTable('taAlCyndgL',
        A => Songs::FieldName('album_artist_raw'));    #AutoFillFields
    my $hbox = ::Vpack(
        [$label_format, '_', $entry_format],
        $table, $check_re, [$label_re, '_', $entry_re],
        $error, $preview, '-', $bbox
    );
    ...
    $check_re->signal_connect(
        toggled => sub {
            $entry_re->set_sensitive(!$_[0]->get_active);
            entry_changed($_[0]);
        }
    );
    ...
    $self->show_all;
    $self->signal_connect(
        response => sub { $_[0]->destroy; $Instance = undef; });
}
...
sub make_format_name {
	...
	$format =~ s#(\$\w+|%[a-zA-Z]|\$\{\w+\})|([%\$])\2#
		$2 || do {
			my $f= $::ReplaceFields{ $Override{$1}||$1 };
...
sub find_fields {
	...
	my @fields = map $::ReplaceFields{ $Override{$_} || $_ },
			grep defined, $format =~ m/ %% | \$\$ | ( \$\w+ | %[a-zA-Z] | \$\{\w+\} ) /gx;
...
sub new {
	...
	# request 4 lines of height
	$textview->set_size_request(
		100,
		($textview->create_pango_layout("X")->get_pixel_size)[1] * 4
	);
	...
	if (ref $IDs) {
		...
		# sort values by their frequency
		my @l = sort { $values->{$b} <=> $values->{$a} } keys %$values;

		$val            = $l[0];
		$self->{IDs}    = $IDs;
		$self->{field}  = $field;
		$self->{append} = my $append = Gtk2::CheckButton->new("Append (only if not already present)");
		$self->pack_end($append, 0, 0, 0);
	} else {
		$val = Songs::Get($IDs, $field);
	}
...
sub get_text {
	...
	if ($self->{append} && $self->{append}->get_active) {
		# append
		...
		for my $orig (@orig) {
			...
			if ($orig eq '') {
				...
				# don't append if the line(s) already exists
				next if index("$orig\n", "$text\n") != -1; 
				$orig .= "\n" . $text;
			}
		}
...
sub new {
    my ($class, $field, $IDs, %opt) =
      @_;    #possible options in %opt : signed digits min max mode
    my $mode   = $opt{mode}   || '';
    ...
    if (ref $IDs) {
        my $values = Songs::BuildHash($field, $IDs);
        my @l      = sort { $values->{$b} <=> $values->{$a} }
          keys %$values;    #sort values by their frequency
    ...
    if ($mode) {
        ...
        elsif ($mode eq 'allow_empty'
          ) # non-numeric values are replaced with "" which is treated as different than 0
        {
            $self->signal_connect(
                input => sub {
                    my $v = Gtk2::Entry::get_text($_[0]);
                    $_[0]{null} = $v !~ /\d/;
                    return 0;
                }
            );
            ...
            $self->signal_connect(
                value_changed => sub {
                    my $v = $_[0]->get_value;
                    $_[0]->set_value((localtime)[5] + 1900)
                      if $v == 1 || $v >= $max;
                }
            );
            $self->signal_connect(output => \&output_nozero);
    ...
    if ($mode eq 'allow_empty' && !length $val) {
        $self->{null} = 1;
        $self->set_text('');
    }
...
sub new {
    ...
    @l = @{Songs::Gid_to_Get($field, \@l)}
      if Songs::Field_property($field, 'gid_to_get');
    if ($listall) {
        my $cb = sub {
            ::PopupAA(
                Songs::MainField($field),
                noalt => 1,
                cb    => sub {
                    $entry->set_text(Songs::Gid_to_Get($field, $_[0]{key}));
                }
            );
        };
...
sub new {
    ...
    #$self->{modif}=0;
    $adj->signal_connect(
        value_changed => sub { $self->update_cb($_[0]->get_value) });
    $check->signal_connect(
        toggled => sub {
            update_cb($_[0],
                ($_[0]->get_active ? '' : $::Options{DefaultRating}));
        }
    );
...
sub new {
    ...
    $button->signal_connect(button_press_event =>
          sub { popup_menu_cb($_[0]); $_[0]->grab_focus; 1; });
...
sub update {
    my $self = $_[0];
    my $h    = $self->{selected};
    my $text = join '<b>, </b>', map ::PangoEsc($_),
      ::superlc_sort(grep $h->{$_}, keys %$h);
...
{
    my ($self, $val) = @_;
    my @vals = grep $_ ne '', split /\s*[;,]\s*/,
      $val;     # currently split on ; or ,
...
######################################################################
# GMB::TagEdit::EntryMassList                                        #
######################################################################

# For mass-editing fields with multiple values.

package GMB::TagEdit::EntryMassList;
use base 'Gtk2::Box';
...
sub new {
    ...
    my %selected;
    $selected{Songs::Gid_to_Get($field, $_)} =
      $valueshash->{$_} == @$IDs ? 1 : 0
      for keys %$valueshash;
    delete $selected{''};
    $self->{selected} = \%selected;
    $self->{all} =
      [keys %selected];    #all values that are set for at least one song
    $self->update;
    return $self;
}
...
sub update                 #update the text and tooltips of buttons
{
    ...
    for my $key (qw/toadd toremove/) {
        ...
        my $h     = $self->{selected};
        my $text  = join '<b>, </b>', map ::PangoEsc($_),
          ::superlc_sort(grep $h->{$_} == $mode, keys %$h);
...
sub popup_menu_cb {
    ...
    my $menu  = Gtk2::Menu->new;
    my $cb    = sub {
        $self->{selected}{$_[1]} = $_[0]->get_active ? $mode : 0;
        $self->update;
    };
...
sub fill {
    ...
    for my $field (Songs::EditFields('single')) {
        ...
        else                                    #first time
        {
            ...
            $table->attach(
                $label,   $col, $col + 1, $row, $row + 1, 'fill',
                'shrink', 2,    2
            );
        }
...
sub new {
    ...
    $treeview->get_selection->signal_connect(
        changed => \&selection_changed_cb,
        $self
    );

    my $view = $self->{view} = Layout::PictureBrowser::View->new(
        xalign      => .5,
        yalign      => .5,
        scroll_zoom => 1,
    );
    ...
    my $hbox = ::Hpack(
        '_',
        ['_', ::new_scrolledwindow($treeview), $button_new],
        [$combo_type, $entry_desc, $button_set, $button_del]
    );
...
sub load {
    ...
    if ($file =~ m/\.(?:m4a|m4b)$/i) {
        $self->{m4a_mode} =
          1;    #only 1 picture, type "front cover", no description
...
sub fill {
    ...
    for my $nb (0 .. $#$pix) {
        ...
        if (defined $select && $select == $nb) {
            $select_path = $store->get_path($iter);
        }
    }
    if ($self->{m4a_mode}) {
...
sub new_picture_cb {
    ...
    my $type = 0;
    $type = 3
      unless grep $_ && $_->[1] == 3, @{$self->{pix}}
      ;    # default to 3 (front cover) if no other picture of that type
...
sub remove_selected_cb {
    ...
    $self->{pix}[$nb] = undef;
    ($nb) = grep $self->{pix}[$_],
      reverse 0 .. $nb - 1;    #select previous entry if any
    $self->fill($nb);
}
...
sub key_press_cb {
	...
	if (::WordIn($key, 'Insert KP_Insert')) {
		$self->new_picture_cb;
...
sub get_changes {
	...
	# only keep those that have a picture
	my @apics = grep $_->[3], @{$self->{pix}};

	if ($self->{m4a_mode} && @apics) {
		@apics = ($apics[0][3]);
	}
...
sub new {
    ...
    for my $t (split / /, $format->[2]) {
        if ($t eq 'vorbis' || $t eq 'ilst') { push @tags, $filetag; }
            ...
            if ($filetag->{ID3v2}) {
                push @tags, $filetag->{ID3v2};
                push @tags, @{$filetag->{ID3v2s}} if $filetag->{ID3v2s};
            }
...
INIT {
    ...
    %tagprop = (
        ID3v2 => {
            addlist => [
                qw/COMM TPOS TIT3 TCON TXXX TOPE WOAR WXXX USLT APIC POPM PCNT GEOB/
            ],
        ...
        OGG => {
            addlist => [
                qw/description genre discnumber author metadata_block_picture/,
                ''
            ],
            default =>
              [qw/title artist album tracknumber date description genre/],
            name   => 'vorbis comment',
            types  => $vorbis_types,
            lckeys => 1,
        },
        APE => {
            addlist => [
                qw/Title Subtitle Artist Album Genre Publisher Conductor Track Composer Comment Copyright Publicationright Year/,
                'Debut Album'
            ],
        ...
        Lyrics3v2 => {
            addlist => [qw/EAL EAR ETT INF AUT LYR/],
            default => [qw/EAL EAR ETT INF/],
            name    => 'lyrics3v2 tag',
            types   => $lyrics3v2_types,
        },
        M4A => {
            addlist => ["\xA9cmt", "\xA9wrt", qw/disk aART cpil ----/],
            default => [
                "\xA9nam", "\xA9ART", "\xA9alb", 'trkn',
                "\xA9day", "\xA9cmt", "\xA9gen"
            ],
...
sub new {
    ...
    my $tagtype = ref $tag;
    $tagtype =~ s/^Tag:://i;
    unless ($tagprop{$tagtype}) {
        warn "unknown tag '$tagtype'\n";
        return undef;
    }
    ...
    if ($lc) {
        my %lc;
        $lc{lc()} = 1 for keys %toadd;
        @default = grep !$lc{lc()}, @default;
    }
    $toadd{$_} = undef for @default;
    for my $key (
        sort {
            ($tagtype->{types}{($lc ? lc $a : $a)}[TAGORDER] || 100)
              <=> ($tagtype->{types}{($lc ? lc $b : $b)}[TAGORDER] || 100)
        } keys %toadd
      )
    {
        my $nb = 0;
...
sub addrow {
    ...
    if ($tagtype->{infosub}) {
        (my $type0, $realkey, my $fallbackname, my @extra) =
          $tagtype->{infosub}($self->{tag}, $key, $nb);
        $type ||= $type0;
    ...
    if (length($type) > 1)    #frame with sub-frames
    {
        ...
        $table->attach($widget, 1, 3, $row, $row + 1, ['fill', 'expand'],
            'shrink', 1, 1);
    }
    else                      #simple case : 1 label -> 1 value
    {
        ...
        if ($key eq '') { ($widget, $label) = EntryDouble->new($value); }
            ...
            $widget = $type->new($value, $param);
            $label  = Gtk2::Label->new($name);
            $label->set_tooltip_text($key);
        }
        $table->attach($label, 1, 2, $row, $row + 1, 'shrink', 'shrink', 1, 1);
        $table->attach($widget, 2, 3, $row, $row + 1, ['fill', 'expand'],
            'shrink', 1, 1);
        @Todel = ($label);
    }
    push @Todel, $widget;
...
sub new {
	...
	$table->attach(Gtk2::Label->new("Genre"), 0, 1, $row, $row + 1, 'shrink', 'shrink', 1, 1);
...
sub new {
	...
	my $self = bless Gtk2::SpinButton->new(
		Gtk2::Adjustment->new($init || 0, 0, $max || 10000000, 1, 10, 0),
		10,
		0), $class;
...
# For id3v2 frames containing multiple fields.
...
INIT {
    ...
    $SUBTAGPROP{metadata_block_picture} =
      $SUBTAGPROP{APIC};    #for vorbis pictures
}
...
sub new {
    ...
    for my $t (split //, $type) {
        ...
        if ($name ne '') {
            ...
            $table->attach(
                $label,    $cols - 1, $cols,    $frow,
                $frow + 1, 'shrink',  'shrink', 1,
                1
            );
        }
        $widget = $widget->new($val, $param);
        push @{$self->{widgets}}, $widget;
        $table->attach($widget, $cols, $cole, $frow, $frow + 1,
            ['fill', 'expand'],
            'shrink', 1, 1);
    }
    if ($key eq 'APIC') {
        $self->{widgets}[3]->set_mime_entry($self->{widgets}[0]);
    }
...
# used only if $Gtk2::VERSION < 1.092
sub _identify_pictype {
...
sub edit {
	...
	if ($self->{dialog}) {
		$self->{dialog}->force_present;
		return;
	}

	$self->{dialog} = ::EditLyricsDialog(
		$self->get_toplevel,
		$self->{value},
		undef,
		sub {
