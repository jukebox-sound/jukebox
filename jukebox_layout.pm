...
our @MenuQueue = (
...

	{
		include => sub {
			...
			::BuildChoiceMenu(
				...
				ordered_hash => 1,
				'reverse'    => 1,
				check        => sub { $::QueueAction          },
				code         => sub { ::EnqueueAction($_[1]); },
			);
		},
	},
...

	{
		...
		submenu => sub {
			my $m = ::max(1, $::Options{MaxAutoFill} - 5);
			return [$m .. $m + 10];
		},
...
our %Widgets = (
	Prev => {
		...
		nbsongs  => 10,
		click3   => sub {
			::ChooseSongs([ ::GetPrevSongs($_[0]{nbsongs}) ]);
		},
	},
	...
	Next => {
		...
		nbsongs  => 10,
		click3   => sub {
			::ChooseSongs([ ::GetNextSongs($_[0]{nbsongs}) ]);
		},
	},
	...
	OpenBrowser => {
		...
		activate => sub {
			::OpenSpecialWindow('Browser', $_[0]{toggle});
		},
		click3   => sub {
	...
	OpenContext => {
		...
		activate => sub {
			::OpenSpecialWindow('Context', $_[0]{toggle});
		},
		click3   => sub {
	...
	OpenQueue => {
		...
		options  => 'toggle',
		activate => sub {
			::OpenSpecialWindow('Queue', $_[0]{toggle});
		},
	},
	...
	Pref => {
		...
		click3   => sub {
			Layout::Window->new($::Options{Layout});
		},
	...
	Lock => {
		...
		# default field to make sure it's defined
		field   => 'fullfilename',

		state   => sub {
			($::TogLock && $::TogLock eq $_[0]{field}) ? 'on' : 'off'
		},
		stock   => {
			on => 'gmb-lock',
			off => '. gmb-locklight'
	...
	Filter => {
		...
		size   => SIZE_FLAGS,
		state  => sub {
			  defined $::ListMode          ?  'list'
			: $::SelectedFilter->is_empty  ?  'library'
			:                                 'filter'
			;
		},
		stock  => {
	...
	Queue => {
		...
		tip    => sub {
			if ($::NextAction) {
				return $::QActions{$::NextAction}{long_next}
			}

			::CalcListLength(
				$::Queue, 'queue') . (
					$::QueueAction
						? "\n" . ::__x("then {action}", action => $::QActions{$::QueueAction}{short})
						: ''
				);
		},
		text   => "Queue",
		...
		#FIXME replace with 3 gmb commands once new command system is done
		click3 => sub {
			::EnqueueAction('');
			::SetNextAction('');
			::ClearQueue();
	...
	Pos => {
		...
		# longest string that will be displayed
		initsize => ::__n("%d song in queue", "%d songs in queue", 99999),
		click1 => sub {
			::ChooseSongs([::GetNeighbourSongs(5)]) unless $::RandomMode || @$::Queue;
		},
		update => sub {
			my $t =
				  (@$::ListPlay == 0)   ?  ''
	...
	Album => {
		...
		click1  => sub {
			...
			::PopupAA('album', from => Songs::Get_gid($ID, 'artists')) if defined $ID;
		},
		click3 => sub {
			my $ID = ::GetSelID($_[0]);
			::PopupAAContextMenu(
	...
	PlayingTime => {
		...
		event          => 'Time',
		click1         => sub {
			$_[0]{remaining} = !$_[0]{remaining};
			$_[0]->update_time;
		},
		update         => sub {
			$_[0]->update_time unless $_[0]{busy};
		},
	},
	...
	TimeBar => {
		...
		event   => 'Time',
		update  => sub {
			$_[0]->set_val($::PlayTime);
		},
		fields  => 'length',
		schange => sub {
			$_[0]->set_max(
				defined $_[1]
					? Songs::Get($_[1], 'length')
					: 0
			);
		},
		set    => sub {
	...
	ChooseRandAlbum => {
		...
		activate => sub {
			...
			if (my $ac = $_[0]{action}) {
				::DoActionForList($ac, $list);
			} else {
				my $ID = ::FindFirstInListPlay($list);
	...
	Visuals => {
		New => sub {
			...
			return $darea unless $::Play_package->{visuals};

			$::Play_package->add_visuals($darea);
		...
		click1 => sub {
			# select next visual
			$::Play_package->set_visual('+') if $::Play_package->{visuals};
		},

		# FIXME use a fullscreen layout instead
		click2 => \&ToggleFullscreen,
	...
	Connections => {
		class  => 'Layout::Label',
		update => sub {
			unless ($::Play_package->can('get_connections')) {
				$_[0]->hide;
				$_[0]->set_no_show_all(1);
	...
	Repeat => {
		...
		event  => 'Repeat Sort',
		update => sub {
			if ($_[0]->get_active xor $::Options{Repeat}) {
				$_[0]->set_active($::Options{Repeat});
			}

			$_[0]->set_sensitive(!$::RandomMode);
		},
	},
...
sub InitLayouts {
	undef %Layouts;
	my @files = ::FileList(
		qr/\.layout$|(?:$::QSLASH)layouts$/o,
		$::DATADIR . ::SLASH . 'layouts',
		$::HomeDir . 'layouts',
		$::CmdLine{searchpath}
	);
...
sub ReadLayoutFile {
	...
	while (1) {
		...
		if ($first) {
			if ($first =~ m#^\[#) {
				ParseLayout(\@lines, $file, $linefirst);
			} else {
				ParseSongTreeSkin(\@lines);
			}
		}
...
sub ParseLayout {
	...
	if ($first =~ m/^\[([^]=]+)\](?:\s*based on (.+))?$/) {
		...
		if (defined $2) {
			%{$Layouts{$name}} = %{$Layouts{$2}};
			delete $Layouts{$name}{Name};
		} else {
			delete $Layouts{$name};
		}
	} else {
	...
	for (@$lines) {
		# translation, escaping the " so it is not picked up as a
		# translatable string. Replace any " in translations because
		# they would cause trouble
		s#_\"([^"]+)"#my $tr=$1; $tr=~y/"/'/; qq/"$tr"/#ge;

		# continuation of previous line if doesn't begin with "word="
		unless (m/^(\w+)\s*=\s*(.*)$/) {
		...
		if ($2 eq '') {
			delete $Layouts{$name}{$currentkey};
			next;
		}
	...
	for my $key (qw/Name Category Title/) {
		# remove quotes from layout name and category
		$Layouts{$name}{$key} =~ s/^"(.*)"$/$1/ if $Layouts{$name}{$key};
	}
...
sub ParseSongTreeSkin {
	...
	for (@$lines) {
		...
		if ($e eq '=') {
			if ($key eq 'elems' || $key eq 'options') {
				warn "Can't use reserved keyword $key in SongTreee column $name\n";

				next;
			}

			# translation, escaping the " so it is not picked up as a translatable string
			$string = $1 if $string =~ m/_\"([^"]+)"/;
...
# Save options for this layout by collecting options of its widgets
sub SaveWidgetOptions {
	my @widgets = @_;
	my %states;
	for my $widget (@widgets) {
		...
		unless ($key) {
			warn "Error: no name for widget $widget\n";
		...
		if (!ref $opt) {
			warn "invalid options returned from $key\n";
...
sub InitLayout {
	...
	$mainwidget ||= do {
		my $l = Gtk2::Label->new("Error : empty layout");
...
sub CreateWidgets {
	...
	if ($self->{layoutdepth} && $self->{layoutdepth} > 10) {
		warn "Too many imbricated layouts\n";
	...
	while (@boxlist) {
		...
		for my $name (@names) {
			...
			# create widget if doesn't exist yet (only boxes have already been created)
			if (!$widget) {
			...
			if ($widget) {
				if ($widget->parent) {
					warn "layout error: $name already has a parent -> can't put it in $key\n";
	...
	for my $key (grep m/^[HV]Size/, keys %$boxes) {
		...
		for my $n (@names) {
			if (my $w = $widgets->{$n}) {
				...
				warn "Can't add unknown widget '$n' to sizegroup\n"
			}
		}
	}

	if (my $l = $boxes->{VolumeScroll}) {
		$widgets->{$_}->signal_connect(scroll_event => \&::ChangeVol) for grep $widgets->{$_}, split /\s+/, $l;
	}

	$self->signal_connect(key_press_event => \&KeyPressed, 0);
	$self->signal_connect_after(key_press_event => \&KeyPressed, 1);

	for my $widget (values %$widgets) {
		my $postinit = delete $widget->{PostInit};
		$postinit->($widget) if $postinit;
	}
	...
	if (@noparentboxes == 0) {
		warn "layout empty ('$self->{layout}')\n";

		return;
	} elsif (@noparentboxes != 1) {
...
# Turn { 'key1/key2' => value } into { key1 => { key2 => value } }
sub Hash_to_HoH {
	my $hash = shift;
	...
	# the hash ref hasn't changed, but can be handy to return it anyway
	return $hash;
}
...
sub NewWidget {
	...
	# FIXME make undef group means parent's group?
	my $group = $options{group};

	my $defaultgroup = $options{default_group} || 'default_group';

	# group local to window unless it begins with uppercase
	$options{group} = $defaultgroup . ($group =~ m/^\w/ ? '-' : '') . $group  unless $group =~ m/^[A-Z]/;
		...
		$widget->signal_connect(
			realize => sub {
				...
				if ($widget->isa('Gtk2::EventBox') && !$widget->get_visible_window) {
					# for eventbox using an input-only
					# gdkwindow, $widget->window is
					# actually the parent's gdkwin,
					# the only way to get to the input-only
					# gdkwin is looking at all the children
					# of its parent :(
...
# apply some options common to both boxes and other widgets
sub ApplyCommonOptions {
	my ($widget, $opt) = @_;
	...
	# only works with widgets/boxes that have their own gdkwindow (put it
	# into a WB box otherwise)
	if ($opt->{hover_layout}) {
...
sub RegisterWidget {
	...
	my $action;
	if ($hash) {
		if ($Widgets{$name} && $Widgets{$name} != $hash) {
			warn "Widget $name already registered\n";
...
sub ToggleFullscreen {
	...
	if ($win->{fullscreen}) {
		if ($::FullscreenWindow && $win == $::FullscreenWindow) {
			$win->close_window;
		} else {
			$win->unfullscreen;
		}
...
sub KeyPressed {
	...
	} elsif ($after && $self->{fullscreen} && $key eq 'Escape') {
		$cmd = 'ToggleFullscreen';
	}

	return 0 unless $cmd;

	# try to find the focused widget (gmb widget, not gtk one), so that the cmd can act on it
	if ($self->isa('Gtk2::Window')) {
		my $widget = $self->get_focus;

		while ($widget) {
			last if exists $widget->{group};
...
sub SetFocusOn {
	...
	# if name contains slashes, divide it into parent and child, where
	# parent can be an Embedded layout or a TabbedLists/Context/NB
	while ($name =~ s#^([^/]+)/##) {
		$self = $self->{widgets}{$1};
		return unless $self;
	}
...
# change the current page of all parent notebook so that widget is on it
sub TurnPagesToWidget {
	my $parent = $_[0];
...
sub SensitiveIfMoreOneSong {
	my $songarray = ::GetSongArray($_[0]);

	$_[0]->set_sensitive($songarray && @$songarray > 1);
}

sub SensitiveIfMoreZeroSong {
	my $songarray = ::GetSongArray($_[0]);

	$_[0]->set_sensitive($songarray && @$songarray > 0);
}
...
sub PlayOrderComboNew {
	...
	my $store = Gtk2::ListStore->new(('Glib::String') x 3);
	...
	$combo->signal_connect(
		changed => sub {
			...
			if ($sort =~ m/^EDIT (.)$/) {
				# so that the combo doesn't stay on Edit...
				PlayOrderComboUpdate($combo);
...
sub VisualsMenu {
	...
	my $cb = sub {
		$::Play_package->set_visual($_[1]) if $::Play_package->{visuals};
	};
...
# create entry to add a label to the current song
sub AddLabelEntry {
...
sub new {
	...
	if (my $opt = $layout =~ m/^[^(]+\(.*=/) {
		# separate layout id and options
		($layout, $opt0) = $layout =~ m/^([^(]+)\((.*)\)$/;
	...
	unless (exists $Layout::Layouts{$layout}) {
		...
		# FIXME if not a player window
		$layout = $fallback;

		# create an error layout if fallback not found
		$Layout::Layouts{$layout} ||= {
			VBmain => 'Label(text="Error : fallback layout not found")'
		};
	}
	...
	if (my $mode = $options{ifexist}) {
		...
		if ($window) {
			if ($mode eq 'toggle' && !$window->{quitonclose}) {
				$window->close_window;

				return;
			} elsif ($mode eq 'replace' && !$window->{quitonclose}) {
				# destroying previous window make it save its
				# settings, then restart new() from the start
				# with new $opt2 but the same original
				# arguments, add ifexists=>0 to make sure it
				# doesn't loop

				$window->close_window;

				return Layout::Window::new(@original_args, ifexists => 0);
			} elsif ($mode eq 'present') {
...
=cut
	...
	if (my $skin = $Layout::Layouts{$layout}{Skin}) {
		$self->set_background_skin($skin);
	}
...
sub init {
	...
	if ($self->{options}{transparent}) {
		if ($::CairoOK) {
			...
			warn "no Cairo perl module => can't make the window transparent\n";
		}
	}
...
sub close_window {
	...
	unless ($self->{quitonclose}) {
		$_->destroy for values %{$self->{widgets}};
...
sub SaveOptions {
	...
	my $opt = Layout::SaveWidgetOptions(
		$self,
		values %{$self->{widgets}},
...
sub Resize {
	...
	# if using default layout size, reserve some space for potential panels
	# and decorations
	# FIXME use gdk_screen_get_monitor_workarea once ported to gtk3
...
sub Position {
	...
	# format : 100x100    50%x100%   -100x-100   500-100% x 500-50%  1@50%x100%
	return unless $pos;
	...
	if (defined $monitor) {
		$monitor = undef if $monitor >= $screen->get_n_monitors;
	}
	...
	$x = 0                   if $x < 0;
	$x = $monitorwidth - $w  if $x + $w > $monitorwidth;
	$y = 0                   if $y < 0;
	$y = $monitorheight - $h if $y + $h > $monitorheight;
	$x += $xmin;
	$y += $ymin;
...
# only works with Gnome2::Wnck
sub set_workspace {
	my ($self, $workspace) = @_;

	eval { require Gnome2::Wnck };
	if ($@) {
		warn "Setting workspace : error loading Gnome2::Wnck : $@\n";
...
sub set_background_skin {
	...
	# FIXME find the cause of these signals, seems related to stock icons
	$self->signal_connect(
		style_set => sub {
			warn "style set : @_" if $::debug;
...
# FIXME needs to add a delay to better deal with a burst of resize events
sub resize_skin_cb {
	my ($self, $alloc) = @_;
...
sub new {
	...
	my $self = Layout::Window::new(
		$class,
		$layout,
		wintype     => 'popup',
		'pos'       => undef,
		size        => undef,
		fallback    => 'full with buttons',
		popped_from => $widget
	);

	#XXX warning: widget can be a Gtk2::StatusIcon
	if ($widget) {
...
sub init {
	...
	$self->signal_connect(
		leave_notify_event => sub {
			$_[0]->CheckCursor if $_[1]->detail ne 'inferior';
...
# StartDestroy if popup is not ancestor of widget under cursor and cursor isn't grabbed (menu)
sub CheckCursor {
	my $self = shift;
	$self->{check_timeout} ||= Glib::Timeout->add(800, \&CheckCursor, $self);

	# to prevent destroying while a menu is open
	return 1 if $self->get_display->pointer_is_grabbed;

	if (my $sicon = $self->{popped_from}) {
		# check if pointer above statusicon
		return 1 if $sicon->isa('Gtk2::StatusIcon') && OnStatusIcon($sicon);
	}
	...
	while ($widget) {
		...
		# don't destroy if cursor is over child of self
		return 1 if $widget == $self;

		# parent popup
		$widget = $widget->{popped_from};
	}
...
# return true if pointer is above sicon
sub OnStatusIcon {
	my $sicon = shift;
...
sub _compute_pos {
	...
	# so that decimal separator is the dot
...
sub HoverPopup {
	...
	# for statusicon, don't popup if no longer above icon
	return 0 if $widget->isa('Gtk2::StatusIcon')  &&  !OnStatusIcon($widget);
...
sub Popup {
	...
	# don't add timeout if there wasn't already one
	$addtimeout = 0 if $self  &&  !$self->{destroy_timeout};
...
sub PreparePopup {
	...
	if (!$widget->{PoppedUpWindow}) {
		...
		if (my $t = delete $widget->{hover_timeout}) {
...
sub CancelDestroy {
	...
	if (my $t = delete $self->{destroy_timeout}) {
		Glib::Source->remove($t);
	}

	if (my $t = delete $self->{check_timeout}) {
		Glib::Source->remove($t);
	}
}
...
our %Boxes = (
	...
	SM => {
		# submenu
		New => sub {
			my $item = Gtk2::MenuItem->new($_[0]{label});
			my $menu = Gtk2::Menu->new;
	...
	EB => {
		New => sub {
			my $self = Gtk2::Expander->new($_[0]{label});

			$self->set_expanded($_[0]{expand});
			$self->{SaveOptions} = sub { expand => $_[0]->get_expanded; };
	...
	FR => {
		New => sub {
			my $f = Gtk2::Frame->new($_[0]{label});

			$f->set_shadow_type($_[0]{shadow}) if $_[0]{shadow};
	...
	AB => {
		New => sub {
			...
			Gtk2::Alignment->new(@opt{qw/xalign yalign xscale yscale/});
		},
		Pack => \&SimpleAdd,
	},
...
sub NewTB {
	...
	#$nb->signal_connect(
	#	button_press_event => sub {
	#		return ! ::IsEventInNotebookTabs(@_);
	#	}
	#);
...
sub SBoxPack {
	...
	if ($end) {
		$box->pack_end($wg, $exp, $fill, $pad);
	} else {
		$box->pack_start($wg, $exp, $fill, $pad);
	}

	# to work around memory leak (gnome bug #498334)
	if ($Gtk2::VERSION < 1.163 || $Gtk2::VERSION == 1.170) {
		$wg->{SBOX_packoptions} = [ $exp, $fill, $pad, ($end ? 'end' : 'start') ];
	}
}
...
sub PanedPack {
	...
	if (!$paned->child1) {
		$paned->pack1($wg, $expand, $shrink);
	} elsif (!$paned->child2) {
		$paned->pack2($wg, $expand, $shrink);
	} else {
		warn "layout error : trying to pack more than 2 widgets in a paned container\n";
	}
}
...
sub PanedNew {
	...
	# +0 to make the conversion to numeric while LC_NUMERIC is set to C
	($self->{size1}, $self->{size2}) = map $_ + 0, split /-|_/, $opt->{size} if defined $opt->{size};
	...
	if (defined $self->{size1}) {
		...
		# in case $self->{size1}==0 'position-set' is not set to true if
		# child1's size is 0 (which is the case here as child1 doesn't
		# exist yet)
		$self->set('position-set', 1);
	}

	$self->{SaveOptions} = sub {
		::setlocale(::LC_NUMERIC, 'C');

		my $s = $_[0]{size1};
		$s .= '-' . $_[0]{size2} if $_[0]{size2};
	...
	# needed to correctly save/restore the handle position
	$self->signal_connect(size_allocate => \&Paned_size_cb);
...
sub Paned_size_cb {
	...
	if (defined $size1 && defined $size2 && abs($max - $size1 - $size2) > 5 || $self->{need_resize}) {
		...
		if ($self->child1_resize && !$self->child2_resize) {
			$size1 = ::max($max - $size2, 0);
			$not_enough = $size2 > $max;
		} elsif ($self->child2_resize && !$self->child1_resize) {
			$size1 = $max if $not_enough = $size1 > $max;
		} else {
			$size1 = $max * $size1 / ($size1 + $size2);
		}

		# don't change the saved value if couldn't restore the size properly
		if ($not_enough) {
			# => will retry in a later size_allocate event unless the position is set manually
			$self->{need_resize} = 1;
		} else {
...
sub Fixed_pack {
	...
	if (my ($x, $y, $w, $h) = $opt =~ m/^(-?\.?\d+),(-?\.?\d+)(?:,(\.?\d+),(\.?\d+))?/) {
		...
		warn "Invalid position '$opt' for widget $wg\n";
	}
}
...
use Glib::Object::Subclass Gtk2::Fixed::, signals => { size_allocate => \&size_allocate, };
...
our @contextmenu = (
...

	{
		...
		submenu => sub {
			my %h;
			$h{$_->{array}->GetName} = 1 for grep $_->{name} =~ m/^EditList\d*$/, $_[0]{self}->get_children;
...
sub new {
	...
	# option begining with list_ / context_ / layout_ will be passed to children of this class
	for my $class (qw/list context layout/) {
		my @opt1;

		# no need for a prefix for these options
		if (my $optkeys = $opt->{'options_for_' . $class}) {
	...
	# Gtk2::Notebook::set_action_widget requires gtk+ >= 2.20 and perl-Gtk2 >= 1.23
	$opt->{newbutton} = 0 unless *Gtk2::Notebook::set_action_widget{CODE};

	if ($opt->{typesubmenu}  &&  $opt->{newbutton}  &&  $opt->{newbutton} ne 'none') {
		# add a button next to the tabs to show new-tab menu
...
sub EndInit {
	...
	if (my $pages = delete $self->{pages}) {
		...
		# recreate dynamic pages (page name begin with +)
		$pagewidget{"+$_"} = $self->newtab($_) for map m/^\+(.+)$/, @pagelist;

		# reorder pages to the saved order
		my $i = 0;
		$self->reorder_child($_, $i++) for grep $_, map $pagewidget{$_}, @pagelist;
	}

	if (my $name = delete $self->{page}) {
...
sub newtab {
	...
	# used to save default options of SongTree/SongList before creating a new one
	$self->SaveOptions if $setpage;
	...
	# give songlist/songtree their own group
	$group = 'Global(' . ::refaddr($self) . '-' . $self->{groupcount}++ . ')' if $wclass eq 'list';

	if ($opt2) {
		# new widget => use a new name not already used
		my $n = 0;
		$n++ while $self->{widgets}{$name . $n} || $self->{widgets_opt}{$name . $n};
		$name .= $n;
	} else {
		$opt2 = $self->{widgets_opt}{$name};
	}
	...
	$self->set_current_page($self->get_n_pages - 1) if $setpage;
...
sub Pack {
	...
	#FIXME ? what to do if no tabtitle given
	if (!defined $label) {
		$label = $wg->{name};
	} elsif (ref $label eq 'CODE') {
		$label = $label->($wg);
	} elsif ($wg->can('DynamicTitle')) {
		$label = $wg->DynamicTitle($label);
	}
	...
	# for some reason (gtk bug ?) the setting is reverted when the tab is dragged, so this re-set it
	$label->signal_connect(
		hierarchy_changed => sub {
			$_[0]->get_pango_context->set_base_gravity('auto');
		}
	);
	...
	if ($wg->{tabcanclose} && $self->{closebuttons}) {
		...
		# Use a custom tab icon if available, as the default
		# `gtk-close` button is too large.  If a custom icon is not
		# available, use the `gtk-close` icon without resizing it,
		# since resized icons may appear distorted with custom themes.
...
sub close_tab {
	...
	if ($manual && $self->{match} && $Layout::Widgets{$name} && $Layout::Widgets{$name}{autoadd_type}) {
		$self->{blacklist}{$name} = undef;
	}
...
# remove EditList tab if corresponding list has been deleted
sub SavedLists_changed {
...
sub button_press_event_cb {
	...
	# to make right-click on tab arrows work
	return 0 unless ::IsEventInNotebookTabs($self, $event);
	...
	my @opt = (
		...
		page     => $page,
		type     => $self->{typesubmenu},
		'close'  => $page->{tabcanclose},
		'rename' => $page->{tabrename},
	);
	...
	if ($self->{tablist} && !$self->{chooser_mode}) {
		...
		for my $page ($self->get_children) {
			# append page list to menu

			my $label =
				  $page->{tab_page_label}
				? $page->{tab_page_label}->get_text
				: $page->{tabtitle}
				;
...
sub pagerename_cb {
	...
	# trigger the focus-out event
	$entry->signal_connect(activate => sub { $_[0]->set_sensitive(0) });
	...
	$entry->signal_connect(
		focus_out_event => sub {
			...
			# prevent error when context menu of the entry pops-up
			return 0 if $entry->get_display->pointer_is_grabbed && $popupmenu && $popupmenu->mapped;
			...
			if ($new ne '') {
				# user has entered new name -> do the renaming
				$renamesub->($label, $new);
			}
...
sub create_chooser_page {
	...
	#remove a previous version of this page
	$self->remove($_) for $self->get_children;
...
#new or removed widgets => check if a widget should be added or removed
...
sub SwitchedPage {
	...
	if (my $songlist = $SongList::Common::Register{$group}) {
		...
		::Watch(
			...
			'Selection_' . $group,
			sub {
				::HasChanged('Selection_' . $_[0]->{group});
			}
		);
	...
	::Watch(
		...
		sub {
			...
			::HasChangedSelID($self->{group}, $ID) if defined $ID;
		}
	);
...
sub new {
	...
	# used to create children at creation time for some placeholders
	if (my $init = $type->{init}) {
		$init->($ph);
	}
...
sub AddWidget {
	...
	my $widget = Layout::NewWidget(
		$name,
		$ph->{opt1},
		$ph->{widgets_opt}{$name},
		{
			default_group => $ph->{group}
		}
	);
	return unless $widget;
...
sub new {
	...
	if (!ref $stock && $ref->{'state'}) {
		...
		# make a copy of the default setting if it is a hash
		my %hash;
		%hash = %$default if ref $default eq 'HASH';

		# extract icon(s) for each state using format : "state1: icon1 facultative_icon2 state2: icon3"
		$hash{$1} = $2 while $stock =~ s/(\w+) *: *([^:]+?) *$//;
		$stock = \%hash;

		# if default setting is a function, use a function that look in
		# the hash, and fallback to the default function (this is the
		# case for Queue and VolumeIcon widgets)
	...
	if ($opt->{skin}) {
		...
		$self->set_app_paintable(1); # XXX needed ?

		# XXX mess up button-press cb TESTME
		if (0 && !$isbutton && $opt->{shape}) {
			$self->{shape} = 1;
		}
	} elsif ($stock) {
		$self->{stock} = $stock;
		...
		# so that it always request the same size, even when no icon
		$img->set_size_request(Gtk2::IconSize->lookup($self->{size}));
...
sub UpdateStock {
	...
	if ($stock =~ m/ /) {
		...
		$stock = '' if $stock eq '.'; # XXX needed ? the result is the same : no icon
...
sub new {
	...
	#		$_[0]->set_markup('<u>'.$_[0]->child->get_label.'</u>')
	#	}
	#);

	#$self->signal_connect(
	#	leave_notify_event => sub {
	#		my $m = $_[0]->child->get_label;
	#		$m =~ s#^<u>##;
	#		$m =~ s#</u>$##;
	#
	#		$_[0]->set_markup($m)
...
sub update_text {
	...
	if ($self->{markup}) {
		my $markup =
			  defined $ID                    ?  ::ReplaceFieldsAndEsc($ID, $self->{markup})
...
# extend the requested size so that the string fit in initsize mode (in case the initsize string is not wide enough)
sub checksize {
	my $self = $_[0];
...
# only used for autoscroll
sub restart_scrollcheck {
	my $self = shift;
...
sub update_time {
	...
	if (defined $time) {
		...
		# update_time() can be called before $self->{markup_stopped} is set, ignore
...
sub new {
	...
	my $orientation =
		  $opt->{vertical}   ? 'bottom-to-top'
		: $opt->{horizontal} ? 'left-to-right'
		:                      $opt->{orientation} || 'left-to-right'
		;

	$self->set_orientation($orientation);

	if ($opt->{skin} && $opt->{handle_skin}) {
		$self = Layout::Bar::skin->new($opt) || $self; # XXX warning : replace $self
	}
...
sub button_press_cb {
	...
	my ($x, $w) =
		$self->{vertical}
		? ($event->y, $self->allocation->height)
...
sub update_preview_Time {
	...
	# get list of Layout::Label::Time widgets in the layouts
	my @labels = grep $_->isa('Layout::Label::Time'), values %$h;
...
sub new {
	...
	$self->set_size_request(
		$left + $right + $hskin->{minwidth},
		$top + $bottom + $hskin->{minheight}
	);
...
sub button_press_cb {
	...
	if (!$self->{step_mode}) {
		# short-circuit normal Gtk2::Scale click behaviour
...
sub new {
	...
	if (my $o = $opt->{overlay}) {
		{
			...
			unless (defined $file) {
				warn "Invalid picture-overlay string : '$o' (format: XxY:WIDTHxHEIGHT:FILE)\n";
	...
	if ($opt->{forceratio}) {
		# not sure it's still needed with the natural_size mode
		$self->{forceratio} = $ratio;
	} else {
...
sub set {
	...
	# default picture
	if (!@files && (my $file = $self->{'default'})) {
		@files = ($file);
	}

	if (@files) {
		# use only the first file if not in multiple mode
		if (@files > 1 && !$self->{multiple}) {
			$#files = 0;
		}
	...
	$self->signal_connect(
		# undo the temporary settings set in size_allocate_cb for the natural_size mode
...
sub LoadImg {
	...
	my $pix = shift @pix;
	if (@pix) {
		$pix = collage($self->{multiple}, $pix, @pix);
	}
...
sub size_allocate_cb {
	...
	if (delete $self->{natural_size}) {
		# set temporary settings for natural_size mode
		# FIXME should be simpler
	...
	if (!defined $self->{size}) {
		unless ($self->{pixbuf} || $::ToDo{'8_LoadImg' . $self}) {
			$self->hide;

			return;
		}
	} elsif ($self->{size} eq $size) {
		return;
	}
...
sub add_overlay {
	...
	$overlay->composite(
		...
		0,
		0,
		$wo / $overlay->get_width,
		$ho / $overlay->get_height,
		'bilinear',
		255
	);
...
sub toggled_cb {
	...
	if (my $tg = $self->{togglegroup}) {
		...
		# togglegroup mode, click on a pressed button just press it again, doesn't un-pressed it
		unless ($show) {
			$show = 1;
		...
		my @togbuttons =
			# get list of widgets of the same togglegroup
			grep $_->{togglegroup}  &&  $_ != $self  &&  $_->{togglegroup} eq $tg, values %{$layw->{widgets}};
	...
	if (my $w = $self->{widget}) {
		$layw->ShowHide($w, $self->{resize}, $show);
	}
}
...
sub new {
	...
	if ($opt->{button} && $opt->{updatemenu}) {
		return Layout::ButtonMenu->new($opt);
	}
	...
	if ($opt->{togglewidget}) {
		$self = Gtk2::CheckMenuItem->new($label);
	} elsif ($opt->{icon}) {
		$self = Gtk2::ImageMenuItem->new($label);
		$self->set_image(Gtk2::Image->new_from_stock($opt->{icon}, 'menu'));
	} else {
		$self = Gtk2::MenuItem->new($label);
	}
	...
	if ($opt->{updatemenu}) {
		...
		# (delayed) initial filling of the menu, not needed but makes
		# the menu work better with gnome2-globalmenu
		::IdleDo('9_UpdateSubMenu_' . $self, undef, \&UpdateSubMenu, $self);
	}
...
sub new {
	...
	$self->{menu}->signal_connect(
		deactivate => sub {
			my $self = shift;

			$self->get_attach_widget->set_active(0);
		}
	);
...
sub new {
	...
	$self->signal_connect(
		size_allocate => sub {
			::IdleDo("resize_$self", 1000, \&update_columns, $_[0]);
		}
	);
...
sub new {
	...
	if ($opt->{ID}) {
		# for use in SongProperties window
		::Watch($self, SongsChanged => \&update); # could check which ID changed
	} else {
		# use group option to find ID
...
sub update {
	...
	# for unclear reasons (bug in gtk2 ?), labels that were hidden when the
	# widget is was last shown, and then are shown while the widget is
	# hidden, have a row height of 0 (and thus hidden) when the widget is
	# shown, forcing a queue resize fixes it
	$self->queue_resize;
}
...
our @toolbar = (
	...
	#{
	#	stockicon=> 'gtk-go-back',
	#	label => "Previous picture",
	#	cb => sub { $_[0]{self}->change_file(-1); },
	#},

	#{
	#	stockicon=> 'gtk-go-forward',
	#	label => "Next picture",
	#	cb => sub { $_[0]{self}->change_file(1); },
	#},
...
our @ContextMenu = (
...

	{
		...
		code => sub {
			Songs::Picture($_[0]{gid}, $_[0]{field}, 'set', $_[0]{file});
		},
		test => sub {
			$_[0]{file} ne (Songs::Picture($_[0]{gid}, $_[0]{field}, 'get') || '');
		},
		istrue => 'file gid',
		mode   => 'V',
...

	{
		...
		test  => sub {
			...
			$_[0]{clip} = $_[0]{self}->get_clipboard(Gtk2::Gdk::Atom->new('PRIMARY', 1))->wait_for_text;
			$_[0]{clip} && $_[0]{clip} =~ m#^\s*\w+://#;
		},
		code => sub {
			$_[0]{self}->drop_uris(uris => [ grep m#^\s*\w+://#, split /[\n\r]+/, $_[0]{clip} ]);
		},
	},

	{
		label     => sub { $_[0]{view}{fullwin} ? "Exit full screen" : "Full screen" },
...
sub new {
	...
	$treeview1->insert_column_with_attributes(
		-1,
		'Dir icon',
		Gtk2::CellRendererPixbuf->new,
		stock_id => 2
	) unless $opt->{no_folder_icons};
	...
	$treeview2->insert_column_with_attributes(
		-1,
		'File name',
		$renderer,
		text     => 0,
	...
	# draw "Embedded pictures"... text over the correct row (avoid
	# stretching the first column for nothing and emphasize that this is
	# not a regular row) and draw progress line if needed
	$treeview2->signal_connect_after(
		expose_event => sub {
			my ($tv, $event) = @_;
	...
	# disable selection of rows without a filename
	$treeview2->get_selection->set_select_function(
		sub {
			my ($selection, $store, $treepath) = @_;

			return $store->get($store->get_iter($treepath), 1) ne '';
		}
	);
		...
		dest => [
			...
			sub {
				...
				if ($type == ::DRAG_FILE) {
					$self->drop_uris(
						uris    => \@values,
						is_move => $view->{dragdest_suggested_action}
					);
				} elsif ($type == ::DRAG_ID) {
	...
	::set_drag(
		...
		source => [
			...
			sub {
				...
				return $file
					? (::DRAG_FILE, 'file://' . ::url_escape($file))
					: ();
			}
		]
	);
	...
	# updating of the file/folder list is disabled whe hidden, so needs to update it when shown
	$vpaned->signal_connect(
		show => sub {
			my $self = ::find_ancestor($_[0], __PACKAGE__);
	...
	if (my $file = $opt->{set_file}) {
		$self->set_file($file);
	} elsif (my $pb = $opt->{set_pixbuf}) {
...
sub SaveOptions {
	...
	# The SaveOptions function of Layout::Boxes::PanedNew returns (size=>$value),
	$opt{hpos} = ($vpaned->parent->{SaveOptions}($vpaned->parent))[1];
...
sub view_in_new_window {
	...
	Layout::Window->new(
		'PictureBrowser',
		pos => undef,
		'PictureBrowser/follow' => 0,
		'PictureBrowser/set_file' => $file,
		'PictureBrowser/embedded_mode' => $self->{embedded_mode}
	);
}
...
sub delete_selected {
	...
	@files = ::uniq(@files);
	my $text = @files == 1
		? ::filename_to_utf8displayname(::basename($files[0]))
...
sub key_press_cb {
	...
	if (::WordIn($key, 'Delete KP_Delete')) {
		$self->delete_selected;
	} elsif (lc $key eq 'l') {
		$self->{show_list} ^= 1;
		$self->update_showhide;
		$self->{view}->grab_focus unless $self->{show_list};
	} elsif (lc $key eq 'n') {
		$self->view_in_new_window;
	} elsif ($key eq 'F5') {
		$self->refresh_treeviews;
	} else {
		# propagate event to the view widget
		return $self->{view}->key_press_cb($event);
	}
	#else {return 0}
...
sub folder_button_press_cb {
	...
	# ignore double or triple clicks
	return 1 if $event->type ne 'button-press';
...
sub drop_uris {
	...
	$self->{drop_job} ||= GMB::DropURI->new(
		toplevel => $self->get_toplevel,
		cb       => sub { $self->file_dropped($_[0]) },
		cb_end   => sub { $self->drop_end }
	);

	$self->{drop_job}->Add_URI(
		uris     => $args{uris},
		is_move  => $args{is_move},
		destpath => $self->{current_path}
	);
...
sub file_dropped {
	...
	# if selection hasn't changed since drop, select the new file
	if ($self->{select_drop}) {
		# do not select the file if not a picture/pdf
		if ($file !~ m/$::Image_ext_re/ && $file !~ m/\.pdf$/i) {
			$file = undef;
		}
...
sub update {
	...
	if ($self->{mode} eq 'song') {
		...
		@files = grep { my $f = $_; $f =~ s/:\w+$//; -f $f } @{$self->{filelist}};
	}
	...
	for my $path (@paths) {
		...
		for my $file (map $path . ::SLASH . $_, ::sort_number_aware(grep !m#^\.#, readdir $dh)) {
			...
			if ($file =~ m/$::Image_ext_re/) {
				push @files, $file;
			} elsif ($file =~ m/\.pdf$/i && $pdfok) {
				push @files, $file, map "$file:$_", 1 .. GMB::Picture::pdf_pages($file) - 1;
			} else {
				if ($emb_ok && $self->{mode} eq 'path') {
					push @emb_files, $file;
				}
	...
	if ($emb_ok) {
		...
		# do not scan embedded files in an idle if selected files is one of them
		my $now = $file && (grep $file eq $_, @emb_files);
	...
	# make sure the path is in @paths
	if ($file) {
		my $fpath = ::dirname($file);
		$path = $fpath if grep $fpath eq $_, @paths;
	}
...
sub refresh_treeviews {
	...
	# folder has changed, reset scrollbars
	unless ($path && $oldpath && $oldpath eq $path) {
	...
	for my $file (::sort_number_aware(grep !m#^\.#, readdir $dh)) {
		if (-d $path . $file) {
			...
			$dirstore->set(
				$iter,
				0, ::filename_to_utf8displayname($file),
				1, Songs::filename_escape($path . $file),
				2, 'gtk-directory'
			);

			# select and center on previous folder if there
			if ($oldpath  &&  $oldpath eq $path . $file) {
		...
		if ($file =~ m/\.pdf$/i && $pdfok) {
			@pages = (1 .. GMB::Picture::pdf_pages($path . $file) - 1);
			$show_expanders = 1;
		} elsif ($file !~ m/$::Image_ext_re/ && !$self->{all_mode}) {
			next;
		}
	...
	if ($self->{embfiles}) {
		my $text =
			...
			: $self->{mode} eq 'song' && $self->{field} eq 'album' ?  "Embedded pictures for this album"
			:                                                         "Embedded pictures"
			;
		...
		# empty row where the text will be written
		$filestore->set($iter, 0, '', 1, '');
		...
		# if scan of embedded files was made immediately
		$self->scan_embedded_pictures_update_tv unless $self->{embfiles}{updatetv};
	}

	$filetv->set_show_expanders($show_expanders);
	$filetv->expand_all;

	# needs to be done once the list is filled
	$foldertv->scroll_to_cell($folder_center_treepath, undef, ::TRUE, .5, 0) if $folder_center_treepath;
}
...
sub scan_embedded_pictures {
	...
	if ($now) {
		$now = $self->scan_embedded_pictures_idle_cb while $now;
	} else {
		$self->{embfiles_idle} ||= Glib::Idle->add(\&scan_embedded_pictures_idle_cb, $self);
	}
}
...
sub scan_embedded_pictures_idle_cb {
	...
	for my $p (0 .. $#p) {
		...
		# check if already seen that picture
		my $i = ::first {$p[$p] eq $pix->[$_][1]} 0 .. $#$pix;
...
sub scan_embedded_pictures_update_tv {
	...
	my $filestore = $self->{filestore};
	$filestore->set(
		$filestore->append(undef),
		0, ' ' . sprintf("in %d/%d files", scalar(@{$_->[2]}), $count),
		1, Songs::filename_escape($_->[0]),
		3, $_->[1]
	) for @$pix;
}
...
sub update_file {
	...
	my $pixbuf = $file && GMB::Picture::pixbuf($file, undef, undef, 'anim_ok'); # XXX disable cache ?
	my %info;
	if ($file) {
		my $realfile = $file;
	...
	if ($file && ::dirname($file) ne ($self->{loaded_path} || '')) {
		$self->refresh_treeviews;
	}
...
sub update_selection {
	...
	while ($iter) {
		if ($store->get($iter, 1) eq $file) {
			if ($page) {
				$iter = $store->iter_nth_child($iter, $page - 1);
			}
...
sub change_file {
	...
	if ($file) {
		$i = ::first { $list->[$_] eq $file } 0 .. $#$list;
	}

	if (defined $i) {
		if ($self->{nowrap}) {
			$i = ::Clamp($i + $direction, 0, $#$list);
		} else {
			# wrap-around
			$i = ($i + $direction) % @$list;
		}
...
sub set_file {
	...
	if ($file && !(grep $file eq $_, @{$self->{filelist}})) {
		$self->set_path(::dirname($file));
	} else {
		$self->update_file;
	}
}
...
sub new {
	...
	if (my $c = $opt{bgcolor}) {
		$self->modify_bg('normal', Gtk2::Gdk::Color->parse($c));
	}
...
sub expose_cb {
	...
	# if drawing a transparent image, fill with white first
	...
	# display a message when dragging a file/link/song above the picture
	if (my $msg = $self->{dnd_message}) {
		$cr->restore;
...
sub resize {
	...
	if ($self->{fit}) {
		$scale = $fit_scale;
	} elsif ($self->{fit_scale}) {
		# multiply by new image ratio and divide by old image ratio

		# to keep the final size constant when changing image
		$scale *= $fit_scale / $self->{fit_scale};
	}
...
sub key_press_cb {
	...
	if (::WordIn($key, 'plus KP_Add Home')) {
		$self->change_zoom('+');
	} elsif (::WordIn($key, 'minus KP_Subtract End')) {
		$self->change_zoom('-');
	} elsif (::WordIn($key, 'equal 1 KP_1')) {
		$self->change_zoom(1);
	} elsif (::WordIn($key, 'Return KP_Enter')) {
		$self->set_zoom_fit;
	} elsif ($key eq 'Left') {
		$self->rotate(-1);
	} elsif ($key eq 'Right') {
		...
		# shortcuts before this point must only change zoom/orientation
		$self->get_toplevel->close_window;
	} elsif (lc $key eq 'f') {
		$self->set_fullscreen;
	} elsif ($key eq 'Escape' && $self->{fullwin}) {
		$self->set_fullscreen(0);
	} elsif (::WordIn($key, 'Down space Page_Up')) {
		$self->change_picture(1);
	} elsif (::WordIn($key, 'Up BackSpace Page_Down')) {
		$self->change_picture(-1);
	} else {
		return 0;
	}
...
sub button_press_cb {
	...
	if ($button == 3) {
		...
		while ($parent = $parent->get_parent) {
			last if $parent->{view} && $parent->{view} == $self;
		}
		...
		$self->change_picture(-1);
	} elsif ($button != 1 && $event->type eq '2button-press') {
		$self->set_fullscreen;
	} elsif (!$self->{pressed}) {
...
sub motion_notify_cb {
	...
	if ($button == 1 && ($self->{max_x} || $self->{max_y})) {
		...
		# could be optimized, copy parts, not sure it's worth it
		$self->invalidate_gdkwin;

	} elsif ($button != 1 && !$self->{prevnext}) {
		# zoom or prev/next with other button # only prev/next once by button press
...
sub scroll_cb {
	...
	# ctrl:zoom, shift:prev/next, else depend on scroll_zoom option (button1 inverts it)
	if ($updown && !$ctrl && ($shift || (!$self->{scroll_zoom} xor $button1))) {
		$updown = 0;
		$d *= -1;
	}

	if ($updown) {
		$self->change_zoom(($d > 0 ? '+' : '-'), $event->x, $event->y);
	} else {
		$self->change_picture($d);
	}
}
...
sub change_zoom {
	...
	if (defined $x) {
		...
		$x = $x - $self->{x1};
		$x = $self->{x2} if $x > $self->{x2};
		$y = $y - $self->{y1};
		$y = $self->{y2} if $y > $self->{y2};
	} else {
		# no zoom coordinates => zoom on center
		$x = $self->{x2} / 2;
		$y = $self->{y2} / 2;
	}
...
sub set_fullscreen {
	...
	if ($fullscreen) {
		...
		$gdkwin->set_user_data(0); # XXX needed ?
		$gdkwin->destroy;
	}

	# reset mouse state, in particular when using double middle button to
	# fullscreen, the mouse could stay in middle-button-pressed mode
	$self->{pressed} = undef;
...
sub new_follow_toolitem {
	...
	my $follow_text = $self->{group} eq 'Play'
		? "Follow playing song"
		: "Follow selected song"
		;
...
sub new_layout_widget {
	...
	if (Songs::FieldType($field) ne 'rating') {
		warn "Stars: invalid field '$field'\n";
...
sub createmenu {
	...
	if (Songs::FieldType($field) ne 'rating') {
		warn "Stars::createmenu : invalid field '$field'\n";
...
sub update {
	...
	unless ($prop) {
		# task finished => remove widgets
		my $widgets = delete $self->{pids}{$pid};

		$self->remove($widgets->[0]) if $widgets;
		if ($self->{lastclose} && !($self->get_children)) {
			$self->get_toplevel->close_window;
		}
...
######################################################################
# Layout::EqualizerPresets                                           #
######################################################################

package Layout::EqualizerPresets;
use base 'Gtk2::Box';

# must not be a possible name of a preset, use "  " because EqualizerPresets
# won't let you create names that contain only spaces
...
sub new {
	...
	$combo->set_row_separator_func(
		sub {
			my $text = $_[0]->get_value($_[1]);

			defined $text && $text eq SEPARATOR;
		}
	);
...
sub combo_changed_cb {
	...
	if ($index > $self->{lastpreset}) {
		...
		# only execute actions if clicked with the mouse, or choose in
		# the popup with the keyboard, not by scrolling
		if (!$action) {
			$self->update('preset'); # reset the combobox to the current preset
		} elsif ($self->{open} == $index) {
			::OpenSpecialWindow('Equalizer');
			$self->update;
		} elsif ($self->{turnoff} == $index) {
...
sub update {
	...
	# first time or package changed or preset_list changed
	my $full = !$event || $event eq 'package' || $event eq 'presetlist';
	...
	if ($full) {
		...
		$combo->set_active($self->{presets}{$set});
	}
...
sub new {
	...
	if ($opt->{preamp}) {
		...
		$adj->signal_connect(
			value_changed => sub {
				::SetEqualizer(preamp => $_[0]->get_value) unless $_[0]{busy};
			}
		);
		if ($self->{labels}) {
			my $vbox   = Gtk2::VBox->new;
			...
			$label1->set_markup_with_format(qq(<span size="%s">%s</span>), $self->{labels}, "pre-amp");
	...
	for my $i (0 .. 9) {
		...
		$adj->signal_connect(
			value_changed => sub {
				::SetEqualizer($_[1], $_[0]->get_value) unless $_[0]{busy};
			},
			$i
		);
		if ($self->{labels}) {
...
sub update {
	...
	if ($doall && $self->{labels}) {
		my ($min, $max, $unit) = $::Play_package->{EQ}
			? $::Play_package->EQ_Get_Range
			: (-12, 12, '')
			;
...
# SVBox and SHBox are Gtk2::VBox and Gtk2::HBox with a smarter size_allocate
# function : the normal boxes divide the extra space equally among children
# with the expand mode. With these boxes the extra space can be allocated to
# children up to a ratio of their other dimension (expand_to_ratio), or
# according to a weight (expand_weight)

package SVBox;
use Glib::Object::Subclass Gtk2::VBox::, signals => { size_allocate => \&SHBox::size_allocate, };

package SHBox;
use Glib::Object::Subclass Gtk2::HBox::, signals => { size_allocate => \&size_allocate, };
...
sub size_allocate {
	...
	for my $child ($self->get_children) {
		...
		my $xreq = $vertical
			? $child->size_request->height
			: $child->size_request->width
			;

		my ($expand, $fill, $pad, $type) =
			# work around memory leak in query_child_packing (gnome bug #498334)
			($Gtk2::VERSION < 1.163 || $Gtk2::VERSION == 1.170)
			? @{$child->{SBOX_packoptions}}
			: $self->query_child_packing($child)
			;
		...
		if ($end) {
			# to keep the same order as a HBox
			unshift @children, \@attrib;
		} else {
			push @children, \@attrib;
		}
	}
	...
	if (@children > 1) {
		...
		if ($bwidth < $total_spacing) {
			$total_spacing = $bwidth;
			$spacing = $total_spacing / $#children;
		}
	...
	if ($self->get_homogeneous && @children) {
	...
	} elsif ($total_xreq < $bwidth && $ecount) {
		# if enough room for all, and some have expand attribute
		...
		while ($ecount && $w > $ecount) {
			...
			for my $child (grep $_->[1], @children) {
				# children that want to expand
		...
		if ($w > 0 && $ecount) {
			# less than one pixel by widget left

			for my $child (sort { $b->[1] <=> $a->[1] } grep $_->[1], @children) {
			...
			# note that weightsum, ecount, max and expand are not
			# updated here as they are no longer used
		}
	} elsif ($total_xreq > $bwidth && @children) {
		# not enough room for requested width

		my $w = $bwidth - $total_spacing;

		# [5] is request [3] is padding
		# sort from smallest to largest
		my @tofit = sort { $a->[5] + $a->[3] * 2 <=> $b->[5] + $b->[3] * 2 } @children;
		...
		while (my $child = shift @tofit) {
			...
			if ($give >= $needed) {
				...
				# space available more than padding

				$child->[5] = $give - $padding; # reduce request to fit
			} else {
				# not even enough space for padding
...
sub draw {
	...
	unless ($h) {
		...
		# x and y are relative to the parent window, so are only useful if the widget use the parent window
		$x = $y = 0 unless $widget->no_window;
	}
	...
	if ($widget->{shape}) {
		...
		#my (undef, $mask) = $pb->render_pixmap_and_mask(1); # XXX leaks X memory for Gtk2 <1.146 or <1.153
...
sub _resize {
	...
	if ($opt eq 'ratio') {
		...
		return $src->scale_simple($width, $height, 'hyper'); # XXX or bilinear ?
	}
	...
	my $wi  = $w - ($left ||= 0) - ($right ||= 0);
	my $dwi = $width - $left - $right;
	my $hi  = $h - ($top ||= 0) - ($bottom ||= 0);
	my $dhi = $height - $top - $bottom;
