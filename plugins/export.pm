...
sub prefbox {
	...
	my $check1 = ::NewPrefCheckButton(
		OPT . 'topath',
		"Copy to mounted portable player",
		cb => \&updatemenu,
...
sub ToCSV {
	my $IDs = $_[0]{IDs} || $_[0]{filter}->filter;
	my $check = ::NewPrefCheckButton(OPT . 'toCSV_notitlerow', "Do not add a title row");

	my $file = ::ChooseSaveFile(undef, "Write filenames to ...", undef, 'songs.csv', $check);
	return unless defined $file;

	# make sure file and path are in position 0 and 1
	my @fields = (qw/file path/, sort grep !m/^file$|^path$/, (Songs::PropertyFields()));
...

	{
		...
		# print a title row
		unless ($::Options{OPT . 'toCSV_notitlerow'}) {
