    ...
    for my $event (keys %events) {
        ...
        ::Watch(
            ...
            $event => sub {
                ...
                ::IdleDo('2_MPRIS2_propchanged', 500, \&PropertiesChanged,
                    $self);
            }
        );
    }
...
sub PropertiesChanged {
    ...
    %PropChanged = ();
    $self->emit_signal(
        PropertiesChanged => 'org.mpris.MediaPlayer2.Player',
        \%changed, []
    );
}
...
sub SupportedMimeTypes {
    return [
        qw(application/ogg audio/flac audio/mpeg audio/ogg audio/x-flac audio/x-m4a audio/x-musepack)
    ];
}    #FIXME
...
dbus_method('Pause', [], [], 'org.mpris.MediaPlayer2.Player',
    {no_return => 1});
...
dbus_property('PlaybackStatus', 'string', 'read',
    'org.mpris.MediaPlayer2.Player');

sub PlaybackStatus {
    my $status =
      $::TogPlay ? 'Playing' : defined $::TogPlay ? 'Paused' : 'Stopped';
    return dbus_string($status);
}
dbus_property('LoopStatus', 'string', 'readwrite',
    'org.mpris.MediaPlayer2.Player');
...
sub LoopStatus {
    if (defined $_[1]) {
        ...
        if ($m eq 'None') { ::SetRepeat(0); $notrack = 1; }
            ...
            ::SetRepeat(1);
            ::ToggleLock('fullfilename', 1);
        }
        elsif ($m eq 'Playlist') { ::SetRepeat(1); $notrack = 1; }
        if ($notrack && $::TogLock && $::TogLock eq 'fullfilename') {
        ...
        my $r =
            !$::Options{Repeat}                          ? 'None'
          : ($::TogLock && $::TogLock eq 'fullfilename') ? 'Track'
          :                                                'Playlist';
...
dbus_property('MinimumRate', 'double', 'read',
    'org.mpris.MediaPlayer2.Player');
sub MinimumRate { dbus_double(1) }
dbus_property('MaximumRate', 'double', 'read',
    'org.mpris.MediaPlayer2.Player');
...
dbus_property('Volume', 'double', 'readwrite',
    'org.mpris.MediaPlayer2.Player');
...
sub CanGoNext {
    ...
    return dbus_boolean(0)
      if !$::Options{Repeat} && $::Position == $#$::ListPlay;
    return dbus_boolean(1);
}
dbus_property('CanGoPrevious', 'bool', 'read',
    'org.mpris.MediaPlayer2.Player');
...
sub CanSeek {
    return dbus_boolean(defined $::SongID)
      ;    #will need to check if stream when supported
}
...
#my ($type)= Net::DBus::Binding::Introspector->_convert(['dict','string',['variant']]); #works too, not sure which one is best
    ...
    $h{$_} = Songs::Get($ID, $_)
      for
      qw/title album artist comment length track disc year album_artist uri album_picture rating bitrate samprate genre playcount/,
      grep Songs::FieldEnabled($_), qw/composer lyricist bpm/;
    my %r =    #return values
      ( 'mpris:length'  => dbus_int64($h{'length'} * 1_000_000),
        'mpris:trackid' => dbus_object_path("/Song/$ID")
        , #FIXME should contain a string that uniquely identifies the track within the scope of the playlist
        'xesam:album'       => dbus_string($h{album}),
        'xesam:albumArtist' => dbus_array([$h{album_artist}]),
        'xesam:artist'      => dbus_array([$h{artist}]),
        'xesam:comment' =>
          ($h{comment} ne '' ? dbus_array([$h{comment}]) : undef),
        'xesam:contentCreated' => ($h{year} ? dbus_string($h{year}) : undef)
        ,    #   ."-01-01T00:00Z" ?
        'xesam:discNumber' => ($h{disc} ? dbus_int32($h{disc}) : undef),
        'xesam:genre',
        => dbus_array([split /\x00/, $h{genre}]),
        'xesam:lastUsed',
        => ($h{lastplay}
            ? dbus_string(::strftime("%FT%RZ", gmtime($h{lastplay})))
            : undef
        ),
        'xesam:title',
        => dbus_string($h{title}),
        ...
        'xesam:composer' =>
          ($h{composer} || '' ne '' ? dbus_array([$h{composer}]) : undef),
        'xesam:lyricist',
        => ($h{lyricist} || '' ne '' ? dbus_array([$h{lyricist}]) : undef),
      );
    my $rating = $h{rating};
    if (defined $rating && length $rating) {
        $r{'xesam:userRating'} = dbus_double($rating / 100);
    }
    unlink $TEMPCOVERFILE;
    if (my $pic =
        $h{album_picture})    #FIXME use ~album.picture.uri when available
    {
        if ($pic =~ m/$::EmbImage_ext_re(?::\w+)?$/i)    #embedded pictures
        {
            my $ok;
            {
                last
                  ...
                  && $ID
                  == $::SongID; #only support embedded picture for current song
                my $data = FileTag::PixFromMusicFile($pic);
                last unless $data;
                my $fh;
                open($fh, '>', $TEMPCOVERFILE)
                  && (print $fh $data)
                  && close($fh)
                  && ($ok = 1);
                warn "mpris2 plugin, error writing temporary file '"
                  . $TEMPCOVERFILE
                  . "' for embedded cover: $!"
                  unless $ok;
            }
        ...
        $r{'mpris:artUrl'} = dbus_string('file://' . ::url_escape($pic))
          if $pic;
    }
...
### patch : fix method name, which fix return type
    $self->_introspector->encode($reply, "methods", "GetAll", "returns",
        \%values);
