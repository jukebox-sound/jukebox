...
my %sites = (
	...
	albums => [
		"$API_URL/?method=artist.getTopAlbums&artist=%a&api_key=$API_KEY&autocorrect=1&format=json",
		"Albums",
		"Show artist's albums"
	],

	similar => [
		"$API_URL/?method=artist.getsimilar&artist=%a&api_key=$API_KEY&autocorrect=1&limit=%l",
		"Similar",
...
my %menuitem = (
	...
	submenu => sub {
		CreateSearchMenu(
			Songs::Gid_to_Get('artist', $_[0]{gid})
		);
	},

	# The menu item is displayed if returns true
	test => sub {
		$_[0]{mainfield} eq 'artist'
	},
);
...
my $artistinfowidget = {
	...
	# no icon by that name by default (yet)
	tabicon       => 'plugin-artistinfo',
...
sub new {
	...
	delete $self->{site} if $self->{site} && !$sites{$self->{site}}[SITEURL];

	# biography is the default site
	$self->{site} ||= 'biography';
	...
	::Watch(
		$artistpic,
		plugin_artistinfo_option_pic => $artistpic_create
	);
	...
	my $tc_similar = Gtk2::TreeViewColumn->new_with_attributes(
		"%",
		$renderer,
		text => 1
	);
	...
	foreach my $key (sort keys %sites) {
		...
		# trying to make the radiobuttons overflowable, but no shared
		# groups for radiobuttons and radiomenuitems (group doesn't
		# seem to work for radiomenuitem at all)
		#
	...
	)
	{
		...
		$menuitem->set_image(
			Gtk2::Image->new_from_stock($stock, 'menu')
		);
		...
		if ($key eq 'save') {
			...
			my $update = sub {
				$_[0]->set_visible(!$::Options{OPT . 'AutoSave'});
			};
...
sub prefbox {
	...
	my $autosave = ::NewPrefCheckButton(
		...
		cb => sub {
			::HasChanged('plugin_artistinfo_option_save');
		}
	);
	...
	my $picsize = ::NewPrefSpinButton(
		...
		cb => sub {
			::HasChanged('plugin_artistinfo_option_pic');
		}
	);
...
=pod
	my $eventformat = ::NewPrefEntry(
		...
		expand => 1,
		tip => "Use tags from last.fm's XML event pages with a leading % (e.g. %headliner), "
			. "furthermore linebreaks '<br>' and any text you'd like to have in between. "
			. "E.g. '%title taking place at %startDate<br>in %city, %country<br><br>'",
...
=cut
	...
	my $similar_exclude_seed = ::NewPrefCheckButton(
		...
		tip => "The artists similar to the 'seed'-artist will be used to populate "
			. "the queue, but you can decide to exclude the 'seed'-artist him/herself."
	);

	my $artistinfo = ::NewIconButton(
		'plugin-artistinfo',
		undef,
		sub {
			::main::openurl("http://www.last.fm/music/");
		},
		'none',
		"Open last.fm website in your browser"
	);
...
sub CreateSearchMenu {
	...
	for my $item (@External) {
		...
		if ($key eq "lastfm" && $artistinfo_url) {
			$url = $artistinfo_url;
		} else {
			$url =~ s/%a/$artist/;
		}
...
sub ArtistChanged {
	...
	$self->{Ltitle}->set_markup(AA::ReplaceFields($aID, "<b>%a</b>", "artist", 1));
...
sub loaded {
    ...
    if ($self->{site} eq "biography") {
        ...
        for my $albumID (@{ $albumIDS }) {
            my $album = Songs::Gid_to_Get(
                album => Songs::Get_gid($albumID, 'album')
            );
            $local_album{$album} = 1;
        }
...
=cut
        ...
        for my $s_artist (split /<\/artist>/, $data) {
            ...
            # otherwise the last </artist> is also treated like an
            # artist
