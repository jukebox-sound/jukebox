...
sub Handshake {
	...
	Send(
		\&response_cb,
		'http://post.audioscrobbler.com/?hs=true&p=1.2&c=' . CLIENTID . '&v=' . VERSION . "&u=$user&t=$time&a=$auth"
	);
}
...
sub Log {
	...
	if (my $iter = $Log->iter_nth_child(undef, 50)) {
		$Log->remove($iter);
	}
}
