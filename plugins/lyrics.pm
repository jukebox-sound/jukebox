...
my @ContextMenuAppend = (
...

	{
		...
		code => sub {
			$_[0]{self}{textview}->set_justification($_[0]{self}{justification} = $_[1]);
		},
	},
);
...
my %Sites = (
	...
	#	function
	# ]
	# if the function return 1 => lyrics can be saved
	...
	#	undef,
	#	sub {
	#		my $ok = $_[0] =~ m#<div id="lyrics">.*\w\n.*\w.*</div>#s;
	#		$_[0] =~ s/<div id="admin".*$//s if $ok;
	#		return $ok;
	#	}
	#],
	...
	lyricssongs => [
		...
		sub {
			::ReplaceFields(
				...
				sub { ::url_escapeall(::superlc($_[0])); }
			);
		},
		undef,
		sub {
			my $is_suggestion = $_[0] =~ m#<h3>Provável música</h3>#i;
			my $l             = html_extract($_[0], div => 'letra');

			# remove header with title and artist links
			$l =~ s#<div id="cabecalho">.*?</div>##s if $l;
	...
	musixmatch => [
		...
		sub {
			::ReplaceFields(
				$_[0],
				"http://www.musixmatch.com/lyrics/%a/%t",
				sub {
	...
	# special mode that search multiple sources
	AUTO => [ "Auto", ],
);

# for versions <1.1.6
$::Options{OPT . 'Font'} ||= delete $::Options{OPT . 'FontSize'};

if (my $site = $::Options{OPT . 'LyricSite'}) {
	# reset selected site if no longer defined
	delete $::Options{OPT . 'LyricSite'} unless exists $Sites{$site};
}

::SetDefaultOptions(OPT,
	Font                 => 10,
	PathFile             => "~/.lyrics/%a/%t.lyric",
	LyricSite            => 'lyricssongs',
	PreferEmbeddedLyrics => 0
);
...
sub new {
	...
	for my $aref (
		# key       stock icon     callback      label   tip
	...
	$self->{buffer}->signal_connect(
		modified_changed => sub { $_[1]->set_sensitive($_[0]->get_modified); },
		$self->{saveb}
	);
	...
	$zoom_spin->signal_connect(
		value_changed => sub {
			my $self = ::find_ancestor($_[0], __PACKAGE__);
			$self->SetFont($_[0]->get_value);
		}
	);
...
sub prefbox {
	...
	my $autosave = ::NewPrefCheckButton(
		...
		tip => "only works with some lyrics source and when the lyrics tab is active"
	);
...
sub SongChanged {
	...
	# cancel any lyrics operation in progress on an a previous song
	$self->cancel;
...
sub load_from_web {
	...
	if ($next) {
		...
		if ($site eq 'AUTO') {
			...
			($site, @{$self->{trynext}}) = qw/lyricssongs/;
		}
	}
	...
	for my $val ($url, $post) {
		...
		if (ref $val) {
			...
			$val = ::ReplaceFields($ID, $val, \&::url_escapeall);
		}
	}
...
# _very_ crude html to gtktextview renderer
sub loaded {
	...
	unless ($data) {
		$data = "Loading failed." . qq( <a href="$self->{url}">) . "retry" . '</a>';
		$type = "text/html";
		$encoding = 0;
	}
	...
	if ($type && $type !~ m#^text/html#) {
		if ($type =~ m#^text/#) {
			...
			if (my $p = $loader->get_pixbuf) {
				$buffer->insert_pixbuf(0, $p);
			}
		}
	...
	if (my $check = $self->{check}) {
		...
		if ($redirect) {
			$self->load_url($redirect, undef, $check);
			return;
		}

		if ($self->{trynext} && !($oklyrics && $oklyrics > 0)) {
			$self->load_from_web('trynext');
			return;
		}
	}
	...
	for ($data) {
		...
		# added to remove warnings from lyrc.com.ar, maybe should be restricted to lyrc.com.ar?
	...
	while (defined($_ = shift @l)) {
		...
		if (m/^[^<]/) {
		...
		} elsif (m#^<(script|style)[ >]#i) {
			shift @l while @l && $l[0] ne "</$1>";
		} elsif (m#^<ul[ >]#i) {
			$buffer->insert($iter, "\n");
			$ul++;
		} elsif (m#^<li[ >]#i) {
			...
			if ($tag eq 'a') {
				if (exists $p{href}) {
					push @urls, $iter->get_offset, $p{href};
				} elsif (exists $p{name}) {
					$namedanchors{$p{name}} = $iter->get_offset;
				}
			} elsif ($tag eq 'font' && 0) {
				if (my $s = $p{size}) {
					if ($s =~ m/^\d+$/) {
			...
			if (exists $p{id}) {
				$namedanchors{$p{id}} = $iter->get_offset;
			}
		} elsif (m#^</(\w+)>$#) {
			if ($1 eq 'a') {
				push @urls, $iter->get_offset if @urls % 3 == 2;
			} elsif ($1 eq 'u') {
				delete $prop{underline};
			} elsif ($1 eq 'b') {
				delete $prop{weight};
			} elsif ($1 eq 'i') {
				delete $prop{style};
...
sub load_pixbuf {
	...
	$self->{waiting} = Simple_http::get_with_cb(
		...
		cb => sub {
			...
			# load next
			::IdleDo('8_FetchPix' . $self, 100, \&load_pixbuf, $self);
		}
	);
...
# old method, more crude :)
sub loaded_old {
	my $self = shift;
	my $buffer = $self->{buffer};
	unless ($_[0]) {
		$buffer->delete($buffer->get_bounds);
		$buffer->set_text(_"Loading failed.");
		return;
	}
	local $_ = $_[0];
	s/[\r\n]//g;
...
sub url_at_coords {
	...
	for my $tag ($iter->get_tags) {
		...
		# lyrc specific
		if ($url =~ m#^http://www\.lyrc\.com\.ar/en/add/add\.php\?#) {
		    ::openurl($url);
		    return;
		}
