...
sub Start {
	Glib::Idle->add(
		sub {
			CreateWindow($_) for sort keys %$DWlist;
...
sub prefbox {
	...
	$LayoutCombo = TextCombo::Tree->new(
		sub {
			Layout::get_layout_list('D')
		},
		undef,
		undef,
		event => 'Layouts',
	...
	my $add = ::NewIconButton(
		'gtk-add',
		"Add",
		sub {
			New($LayoutCombo->get_value);
		}
	);
	...
	$Treeview->set_size_request(
		100,

		#request 5.5 lines of height (not counting row spacing)
		($Treeview->create_pango_layout("X")->get_pixel_size)[1] * 5.5
	);
...
sub FillOptions {
	...
	my $remove = ::NewIconButton('gtk-remove', "Remove this widget", sub { Remove($key) });
	if ($Layout::Layouts{$layout}) {
		...
		$label->set_markup_with_format("<b>%s</b>", "The layout for this desktop widget is missing.");
	...
	$textcolor->signal_connect(
		color_set => sub {
			$opt->{DefaultFontColor} = $_[0]->get_color->to_string;
			CreateWindow($key);
		}
	);
	...
	$textfont->signal_connect(
		font_set => sub {
			$opt->{DefaultFont} = $_[0]->get_font_name;
			CreateWindow($key);
		}
	);
	...
	$adjo->signal_connect(
		value_changed => sub {
			$opt->{opacity} = $_[0]->get_value / 100;
			my $win = $Displayed{$key};
			$win->set_opacity($opt->{opacity}) if $win;
		}
	);

	my $ontop = Gtk2::CheckButton->new("On top of other windows instead of below");
	$ontop->set_active($opt->{ontop});
	$ontop->signal_connect(
		toggled => sub {
			my $on = $_[0]->get_active;
			$opt->{ontop} = $on;
			$opt->{below} = !$on;
			CreateWindow($key);
		}
	...
	my $adjw = Gtk2::Adjustment->new(
		$opt->{w},
		1,
		::max(2000, $opt->{w}, Gtk2::Gdk::Screen->get_default->get_width),
		10,
		50,
		0);

	my $adjh = Gtk2::Adjustment->new(
		$opt->{h}, 1,
		::max(2000, $opt->{h}, Gtk2::Gdk::Screen->get_default->get_height),
		10,
		50,
		0);
	...
	my $vbox = ::Vpack(
		...
		[
			Gtk2::Label->new("Default text color"),
			$textcolor
		],

		[
			Gtk2::Label->new("Default text font"),
			$textfont
		],
		...
		[
			Gtk2::Label->new("Monitor"),
			$spin_mon,
		],

		[
			Gtk2::Label->new("Minimum size"),
			$spinw,
			Gtk2::Label->new('x'),
			$spinh
		],
...
sub CreateWindow {
	...
	$Displayed{$key} = Layout::Window->new(
		$opt->{layout},
		%$opt,
		'pos'        => $pos,
		size         => $size,
...
sub ResizeWindow {
	...
	# better, but doesn't work well with Cover widget:
	#$win->resize($DWlist->{$key}{w},$DWlist->{$key}{h});
}
