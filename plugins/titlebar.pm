...
sub prefbox {
	...
	my $set_font = ::NewPrefCheckButton(
		OPT . 'set_textfont',
		"Change default text font and size",
		cb         => \&init,
		widget     => $font,
		horizontal => 1
	);
...
sub move {
	...
	# hide it if there is a window above
	if (1) {
		# go through windows from top to bottom
		for my $win (reverse $Screen->get_windows_stacked) {

			# until the active window
			last if $ActiveWindow == $win;

			next unless $win->is_visible_on_workspace($ActiveWindow->get_workspace);

			my ($x0, $y0, $w0, $h0) = $win->get_geometry;

			if ($x + $pw > $x0 && $x0 + $w0 > $x && $y + $ph > $y0 && $y0 + $h0 > $y) {
				$Popupwin->hide;
