...
# Needed to avoid progress bar overflow in save_fields when called from
# mass_download.
my @towrite;

# Needed to avoid progress bar overflow in save_fields when called from
# mass_download.
my $save_fields_lock = 0;
...
sub prefbox {
	...
	#my $btn_amg = ::NewIconButton(
	#	'plugin-artistinfo-allmusic',
	#	undef,
	#	sub {
	#		::main::openurl("http://www.allmusic.com/");
	#	},
	#	'none',
	#	_"Open allmusic.com in your web browser"
	#);
	#
	...
	my $entry_path = ::NewPrefEntry(
		OPT . 'PathFile' => "Save album info in:",
		width => 40
	);
	...
	my $chk_join = ::NewPrefCheckButton(
		...
		tip => "Allmusic uses both Genres and Styles to describe albums.  If you use only Genres, you may want to include Styles in allmusic's list of Genres."
	);
	...
	for my $field (qw(genre mood style theme)) {
		push(
			...
			::NewPrefCheckButton(
				...
				tip => "Note: inactive fields must be enabled by the user in the 'Fields' tab in Settings"
			)
		);
	...
	my ($radio_add, $radio_rpl) = ::NewPrefRadio(
		OPT . 'ReplaceFields',
		[
	...
	my $chk_saveflds = ::NewPrefCheckButton(
		...
		tip => "Save selected fields for all tracks on the same album whenever album data is loaded from allmusic or from file."
	);
	...
	$btn_download->set_tooltip_text("Fields will be saved according to the settings above. Albuminfo files will be re-read if there are fields to be saved and you choose 'albums missing reviews' in the combo box.");

	my $cmb_download = ::NewPrefCombo(
		OPT . 'mass_download',
		{
			all => "entire collection",
			missing => "albums missing reviews"
		},
...
sub mass_download {
	# the context widget was meant to be used as self in methods of this
	# plugin, but when downloading all there is no gtk widget to use. Using
	# a blessed hash instead should work as well as long as no gtk2
...
sub download_one {
	...
	# Need a track (any track) from the album: pick the first in the list.
	my $ID  = $IDs->[0];
	...
	if ($::Options{OPT . 'mass_download'} ne 'all' && $file && -r $file) {
		...
		$self->{waiting} = Simple_http::get_with_cb(
			url => $url,
			cache => 1,
			cb => sub { $self->load_search_results($ID, 1, \&download_one, @_) }
		);
	}
}
...
sub new {
					...
					::PopupAAContextMenu(
						{
							...
							field => 'album',
							ID => $::SongID,
							gid => Songs::Get_gid($::SongID, 'album'),
							mode => 'P'
						}
					);
				}
	...
	my $refreshbutton = ::NewIconButton(
		'gtk-refresh',
		undef,
	...
	# Year is a 'Glib::String' to avoid printing "0" when year is missing.
	# Caveat: will give wrong sort order for albums released before year
	# 1000 or after year 9999 :)
	#
	# Album, Artist, Label, Year, URL, Sort order.
	my $store = Gtk2::ListStore->new(('Glib::String') x 5, 'Glib::UInt');
	...
	for my $col (split(/\s+/, $::Options{OPT . 'Columns'} || ''), qw/album artist genre year/) {
		...
		my $column = Gtk2::TreeViewColumn->new_with_attributes(
			...
			text => $coldef->{storecol}
		);
		$column->{key} = $col;
		$column->set_sort_column_id($coldef->{storecol});
		$column->set_expand(1);
		...
		my $label = Gtk2::Label->new($coldef->{name});
		$column->set_widget($label);
		$label->show;

		# The header label is attached to a button by Gtk
		my $button = $label->get_ancestor('Gtk2::Button');
		$button->signal_connect(
			button_press_event => \&treeview_click_cb,
			$col
		) if $button;
	}
	...
	$searchview->add(
		::Vpack(
			['_', $search,  $Bsearch], '_', $scrwin, '-',
			['-', $Bcancel, $Bok]
		)
	);

	# Pressing Enter in the search entry.
	$search->signal_connect(activate => \&new_search);
	...
	$scrwin->signal_connect(
		key_press_event => sub {
			entry_selected_cb($_[0]) if $_[1]->keyval == $Gtk2::Gdk::Keysyms{Return};
		}
	);
	$scrwin->signal_connect(
		key_press_event => sub {
			song_changed($_[0]) if $_[1]->keyval == $Gtk2::Gdk::Keysyms{Escape};
		}
	);

	# Must call it once now before $searchview->set_no_show_all(1) disables it.
	$searchview->show_all();

	# GMB sometimes calls $plugin->show_all(). We then want only infoview to show.
	$searchview->set_no_show_all(1);
...
sub treeview_click_cb {
	...
	if ($event->button == 1) {
		...
		if ($sortid == $storecol && $order eq 'descending') {
			# After third click on column header: return to AMG sort order (default).
			$treeview->get_model->set_sort_column_id(5, 'ascending');
		...
		my $menu = ::BuildChoiceMenu(
			{
				# hash of colkey => name
				map { ($_ => $Columns{$_}{name}) } keys %Columns 
			},
			...
			check => sub {
				...
				[
					map $_->{key}, grep $_->get_visible, $_[0]{treeview}->get_columns
				];
			},
			...
			code => sub {
				...
				# make sure one column is visible
				$cols[0]->set_visible(1) unless grep $_->get_visible, @cols;
			},
		);
		$menu->show_all;
...
sub button_release_cb {
	...
	for my $tag ($iter->get_tags) {
		if ($tag->{url}) {
			...
			# prepend + for multi-value fields : Genre, Mood, Style, Theme
			my $field = $tag->{field} eq 'year'
				? 'year'
				: '+' . $tag->{field}
				;
...
sub cover_popup {
	...
	# black bg for the cover-popup
	$menu->modify_bg('GTK_STATE_NORMAL', Gtk2::Gdk::Color->parse('black'));
...
sub print_review {
	...
	for my $f (@showfields) {
		if ($fields->{$f->{short}} && $::Options{OPT . 'Show' . $f->{short}} && $f->{active}) {
			...
			if ($f->{multi}) {
				# genres, moods, styles and themes.
				...
				for my $val (@amg) {
					if (grep { lc($_) eq lc($val) } @old) {
						...
						# val doesn't exist in local db => create link to save it.
				...
				if (defined $1 && $1 != Songs::Get(::GetSelID($self), 'year')) {
					# AMG year differs from local year => create link to correct.
					...
					$tag->{val}   = $1;
					$tag->{tip} = ::__x(
						"Set {year} as year for all tracks on this album.",
						year => $1
					);
...
sub new_search {
	# $_[0] can be a button or gtk-entry. Ancestor is an albuminfo object.
	my $self = ::find_ancestor($_[0], __PACKAGE__);

	my $album = $self->{search}->get_text();
	$album =~ s|^\s+||;
	$album =~ s|\s+$||; # remove leading and trailing spaces
...
sub print_results {
	...
	# result is a ref to an array of hash refs
	my $result = parse_amg_search_results($html, $prop{type});
...
sub entry_selected_cb {
	# $_[0] may be the TreeView or the 'OK' button. Ancestor is an albuminfo object.
	my $self = ::find_ancestor($_[0], __PACKAGE__);

	my ($path, $column) = $self->{treeview}->get_cursor();

	# The user may click OK before selecting an album
	unless (defined $path) {
	...
	$self->{waiting} = Simple_http::get_with_cb(
		cb => sub {
			$self->{searchview}->hide();
			$self->{infoview}->show();
			$self->load_review(::GetSelID($self), 0, undef, $url, @_);
		},
		url => $url,
		cache => 1
	);
...
sub song_changed {
	...
	# Check if album has changed or a forced update is required.
	if (!$self->{aID} || $aID != $self->{aID} || $force) {
		$self->{aID}   = $aID;
		$self->{album} = Songs::Gid_to_Get("album", $aID);
		$self->album_changed($ID, $aID, $force);
	} else {
		# happens for example when song properties are edited, so we need to repaint the widget.

		::IdleDo(
			'9_refresh_albuminfo' . $self,
			undef,
			sub {
...
sub album_changed {
	...
	warn "Albuminfo: fetching search results from $url\n" if $::debug;
	$self->{waiting} = Simple_http::get_with_cb(
		cb => sub { $self->load_search_results($ID, 0, undef, @_) },
		url => $url,
		cache => 1
	);
}
...
sub load_search_results {
	# $md = 1 if mass_download, 0 otherwise.
	# $cb = callback function if mass_download, undef otherwise.
	my ($self, $ID, $md, $cb, $html, %prop) = @_;

	delete $self->{waiting};

	# $result[$i] = {url, album, artist, genres, year}
	my $result = parse_amg_search_results($html, $prop{type});
	...
	if ($url) {
		...
		$self->{waiting} = Simple_http::get_with_cb(
			cb => sub { $self->load_review($ID, $md, $cb, $url, @_) },
			url => $url,
			cache => 1
		);
	} else {
...
sub parse_amg_album_page {
	...
	if ($::Options{OPT . 'StyleAsGenre'} && $result{style}) {
		@{$result{genre}} = ::uniq(@{$result{style}}, @{$result{genre}});
	}
...
sub decode {
	...
	$encoding = 'cp1252' if ($encoding && $encoding eq 'iso-8859-1');
...
sub load_file {
	...
	if (open(my $fh, '<', $file)) {
		...
		my $text = <$fh>;
		if (my $utf8 = Encode::decode_utf8($text)) {
			$text = $utf8;
		}
		my (@tmp) = $text =~ m|<(.*?)>(.*)</\1>|gs;
		...
		close $fh;
		if ($::Options{OPT . 'StyleAsGenre'} && $self->{fields}->{style}) {
			@{$self->{fields}->{genre}} = ::uniq(@{$self->{fields}->{style}}, @{$self->{fields}->{genre}});
		}
		$self->print_review() unless $md;

		# We may not have saved when first downloaded.
		if ($::Options{OPT . 'SaveFields'}) {
			push(@towrite, [$ID, %{$self->{fields}}]);
			save_fields();
		}
...
sub save_review {
	...
	unless ($path && $file) {
		::ErrorMessage("Error: invalid filename pattern" . " : $format", $::MainWindow);

		return;
	}
	return unless ::CreateDir($path, $::MainWindow, "Error saving review") eq 'ok';
...
sub save_fields {
	...
	warn "Albuminfo: Saving tracks on $album ("
		. scalar(@towrite)
		. " album"
		. (scalar(@towrite) != 1 ? "s" : "")
		. " in queue).\n"
	...
	# Songs on this album
	my $IDs = Songs::MakeFilterFromGID('album', Songs::Get_gid($ID, 'album'))->filter();
