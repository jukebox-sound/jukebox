...
BEGIN {
	{
		...
		# this segfault when mozembed doesn't find its libs
		system(q(GNOME_DISABLE_CRASH_DIALOG=1 perl -e 'use Gtk2 "-init"; use Gtk2::MozEmbed;$w=Gtk2::Window->new;my $e=Gtk2::MozEmbed->new; $w->add($e);$w->show_all; exit 0'));    

		#if (($? & 127) ==11) {
		#	die "Error : mozembed libraries not found. You need to add the mozilla path in /etc/ld.so.conf and run ldconfig (as root) or add the mozilla libraries path to the LD_LIBRARY_PATH environment variable.\n"
		#}

		# crash or fail to load
		if ($?) {
			warn "Gtk2::MozEmbed found but not working.\n";
			$CrashMoz = 1;
			last;
		}
...
######################################################################
# GMB::Plugin::WebContext::MozEmbed                                  #
######################################################################
...
sub init {
	...
	if ($Gtk2::MozEmbed::VERSION >= 0.06) {
		Gtk2::MozEmbed->push_startup;
	} else {
		# needed to keep a Gtk2::MozEmbed to prevent xpcom from shutting down with the last gtkmozembed
		$Embed ||= Gtk2::MozEmbed->new;
	} 
}
...
# called before loading a new uri,
sub about_to_load_cb {
	my ($embed, $uri) = @_;
...
# FIXME use print version of the wikipedia page instead?
sub set_stripped_wiki {
	my $stripped = $_[0];
	my $content  = '';
...
sub new_embed {
	...
	$embed->signal_connect(
		navigation_policy_decision_requested => sub {
			# middle-click on a link
			if ($_[3]->get_button == 2) {
				# only works if inside a NB/TabbedLists/Context widget
				my $nb = ::find_ancestor($_[0], 'Layout::NoteBook');

				# open the link in a new tab
				$nb->newtab('PluginWebPage', 1, {url => $_[2]->get_uri}) if $nb;
...
# ignored unless overridden by the class
sub set_title { }
...
sub loaded {
	...
	# FIXME doesn't check type ($prop{type})
	$embed->load_html_string($data, $self->{url});
}
...
######################################################################
# GMB::Plugin::WebContext                                            #
######################################################################

package GMB::Plugin::WebContext;
our @ISA;
BEGIN {
	push @ISA, 'GMB::Context';
}
use base 'Gtk2::VBox';
use constant {OPT => 'PLUGIN_WebContext_',};

our %Predefined = (
	google => {
		tabtitle => 'google',
		baseurl  => 'http://www.google.com/search?q="%a"+"%t"',
	},

	amgartist => {
		tabtitle => 'amg artist',
		baseurl  => 'http://www.allmusic.com/search/artist/%a',
	},

	amgalbum => {
		tabtitle => 'amg album',
		baseurl  => 'http://www.allmusic.com/search/album/%l',
	},

	lastfm => {
		tabtitle => 'last.fm',
		baseurl => 'http://www.last.fm/music/%a',
	},

	discogs => {
		tabtitle => 'discogs',
		baseurl => 'http://www.discogs.com/artist/%a',
	},

	youtube => {
		tabtitle => 'youtube',
		baseurl  => 'http://www.youtube.com/results?search_query="%a"',
	},

	pollstar => {
		tabtitle => 'pollstar',
		baseurl  => 'http://www.pollstar.com/eventSearch.aspx?SearchBy=%a',
	},

	songfacts => {
		tabtitle => 'songfacts',
		baseurl  => 'http://www.songfacts.com/search_fact.php?title=%t',
	},

	rateyourmusic => {
		tabtitle => 'rateyourmusic',
		baseurl => 'http://rateyourmusic.com/search?searchterm=%a&searchtype=a',
	},
);
...
our %Widgets = (
	...
	PluginWebPage => {
		...
		# load/save the title because page is only loaded when mapped, so
		# we can ask it its title until tab is selected
		options     => 'url title',
	},
);
...
::SetDefaultOptions(
	...
	StrippedWiki => 1,
	Custom => {
		map { $_ => {%{$Predefined{$_}}} } qw/lastfm amgartist youtube/
	}
);
UpdateBackend();
...
sub UpdateBackend {
	...
	$backend = '' if !$OKMoz && $backend eq 'MozEmbed' || !$OKWebKit && $backend eq 'WebKit';
	unless ($backend) {
		if ($OKWebKit) {
			$backend = 'WebKit';
		} elsif ($OKMoz) {
	...
	if ($OKMoz && $backend eq 'MozEmbed') {
		require Gtk2::MozEmbed;
		Gtk2::MozEmbed->import;
...
sub UpdateCustom {
	...
	if (!defined $id) {
		...
		# find a new name
		::IncSuffix($id) while $Widgets{'PluginWebCustom_' . $id . '_'};
	} elsif ($active) {
		Layout::RegisterWidget('PluginWebCustom_' . $id . '_' => undef);
	}

	if ($hash) {
		# edit existing custom page
	...
	my %widget = (
		%{$Widgets{PluginWebCustom}},
		autoadd_type => 'context page custom html',
		%$hash
	);
	$widget{tabtitle} ||= "Untitled";

	# '_' is appended because gmb widget names cannot end with numbers
	my $name = 'PluginWebCustom_' . $id . '_';

	$Widgets{$name} = \%widget;
	if ($active) {
		Layout::RegisterWidget($name => $Widgets{$name});
	}
...
sub new {
	...
	$entry->signal_connect(
		activate => sub {
			::find_ancestor($_[0], __PACKAGE__)->load_url($_[0]->get_text);
		}
	);
	...
	$open->signal_connect(clicked => sub {
			my $url = ::find_ancestor($_[0], __PACKAGE__)->get_location;
			::openurl($url) if $url =~ m#^https?://#;
			}
	);
...
# default method, overridden by packages that add extra items to the toolbar
sub addtoolbar {
	return ();
}
...
sub prefbox {
	...
	my $label_moz = $OKMoz ? '' : $CrashMoz ? "Found but not working" : "Not found";
...
# FIXME
sub set_stripped_wiki {
	GMB::Plugin::WebContext::MozEmbed::set_stripped_wiki($::Options{OPT . 'StrippedWiki'});
}

sub popup_toolbar_menu {
	my ($toolbar, $x, $y, $button) = @_;
	my $args = {
		self => ::find_ancestor($toolbar, __PACKAGE__),
	};
...
# called by Layout::NoteBook when tab is created
sub DynamicTitle {
	my ($self, $default) = @_;
...
use constant {
	# FIXME
	OPT => GMB::Plugin::WebContext::OPT,
};

my %sites = (
	lyrc => [
		'lyrc',
		'http://lyrc.com.ar/en/tema1en.php?artist=%a&songname=%s'
	],

	#leoslyrics => [
	#	'leolyrics',
	#	'http://api.leoslyrics.com/api_search.php?artist=%a&songtitle=%s'
	#],

	google => [
		'google',
		'http://www.google.com/search?q="%a"+"%s"'
	],

	lyriki => [
		'lyriki',
		'http://lyriki.com/index.php?title=%a:%s'
	],

	lyricwiki => [
		'lyricwiki',
		'http://lyrics.wikia.com/%a:%s'
	],

	lyricsplugin => [
		'lyricsplugin',
		'http://www.lyricsplugin.com/winamp03/plugin/?title=%s&artist=%a'
	],

	lyricscom => [
		'lyrics.com',
		'http://www.lyrics.com/search.php?keyword=%s+%a&what=all'
	],
);
...
######################################################################
# GMB::Plugin::WebContext::Wikipedia                                 #
######################################################################
...
sub SongChanged {
	...
	# FIXME add a way to choose artist ?
	my $artist = Songs::Get($ID, 'first_artist');
...
sub SongChanged {
	...
	unless ($url) {
		warn "no baseurl defined for custom webcontext $self->{name}\n";
...
######################################################################
# GMB::Plugin::WebContext::Custom::Edition                           #
######################################################################
...
# if $id => fill entries with existing properties, if $hash => fill entries
# with content of hash, if neither => empty entries
sub fill_editbox {
	my ($self, $id, $hash) = @_;
	...
	my $preview = Label::Preview->new(
		...
		preview => sub {
			my $url = shift;
			defined $::SongID && $url
				? ::ReplaceFields($::SongID, $url, \&::url_escapeall)
				: undef;
		},
		wrap => 1,
	);
...
sub save_cb {
	...
	my $editbox = $self->{editbox};
	my $hash = {
		tabtitle => $editbox->{entry_title}->get_text,
		baseurl  => $editbox->{entry_url}->get_text,
	};
	my $id = $editbox->{id};
	# if $id is undef, create a new page, else edit existing one
	GMB::Plugin::WebContext::UpdateCustom($id => $hash);
	$editbox->{busy} = 1;
	fill_list($self->{store});
	$editbox->{busy} = 0;
...
sub preset_menu_cb {
	...
	my $menu_cb = sub {
		my $preid = $_[1];
		$self->fill_editbox(undef, $predef->{$preid});
		$self->update_selection;
	};
