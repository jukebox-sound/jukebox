...
# is not required, else could do just: use Net::DBus::GLib;
...
sub GetMetadata_from {
	...
	my %h;
	$h{$_} = Songs::Get($ID, $_) for qw/title album artist comment length track disc year
					album_artist uri album_picture rating bitrate samprate/;
	...
	# FIXME use ~album.picture.uri when available
	if (my $pic = delete $h{album_picture}) {
		# XXX ignore picture embedded in mp3/flac files?
		$h{arturl} = 'file://' . ::url_escape($pic);
	}
...
sub GetCurrentTrack {
	...
	# random mode or song not in playlist
	$p = ::FindPositionSong($::SongID, $::ListPlay) unless defined $p;

	# fallback to 0, not really a good idea, but don't know what else to
	# return if current song is not in playlist
...
sub AddTrack {
	...
	# Uris_to_IDs split spaces
	# FIXME Uris_to_IDs shouldn't do that
	$uri =~ s/ /%20/g;
