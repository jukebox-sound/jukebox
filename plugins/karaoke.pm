...
sub load_file {
	...
	for my $l (split /\012|\015|\015\012/, $text) {
		...
		if ($l =~ m/^\[offset:([+-]?\d+)\]/) {
			...
			# look for format : [mm:ss.xx]lyrics line
			my $i = 0; # loop for cases : [mm:ss.xx][mm:ss.xx][mm:ss.xx]repeated line
			while ($l =~ s/^\[(\d?\d):(\d\d)(?:\.(\d\d)\d?)?\]//) {
				push @$seconds, $offset + $1 * 60 + $2 + ($3 || 0) / 100;
				$i++;
			}
			push @$lines, ($l) x $i;
			$needsort = 1 if $i > 1;
	...
	# for repeated lines, as they are not properly ordered
	if ($needsort) {
