...
use constant {
	...
	# google checks to see if the browser can handle the "standard" image
	# search version, instead of the "basic" version. And as of the end of 2013
	# the "basic" version doesn't include direct url of the images, so we need
	# to use the "standard" version
	GOOGLE_USER_AGENT => 'Mozilla/5.0 Gecko/20100101 Firefox/26.0',

	# for some reason bing returns weird results with the default gmb user agent (Mozilla/5.0)
	BING_USER_AGENT   => ' ',
};
...
my %Sites = (
	artist => {
		...
		lastfm => [
			'last.fm',
			"http://www.last.fm/music/%a/+images",
			\&parse_lastfm
		],

		#discogs => [
		#	'discogs.com',
		#	"http://api.discogs.com/search?f=xml&type=artists&q=%a",
		#	\&parse_discogs
		#],

		bing => [
			'bing',
			"http://www.bing.com/images/async?q=%s",
			\&parse_bing,
			BING_USER_AGENT
		],
		...
		yahoo => [
			'yahoo',
			"http://images.search.yahoo.com/search/images?p=%s&o=js",
			\&parse_yahoo
		],

		ddg => [
			"DuckDuckGo",
			"https://duckduckgo.com/?q=%s&iax=1&ia=images",
			\&parse_ddg
		],
	},
	...
	album => {
		...
		bing => [
			'bing',
			"http://www.bing.com/images/async?q=%s&qft=+filterui:aspect-square",
			\&parse_bing,
			BING_USER_AGENT
		],
		...
		slothradio => [
			'slothradio',
			"http://www.slothradio.com/covers/?artist=%a&album=%l",
			\&parse_sloth
		],

		# could add /Music+CD but then we'd lose /Soundtrack
		# XXX API doesn't work anymore
		#freecovers => [
		#	'freecovers.net',
		#	"http://www.freecovers.net/api/search/%s",
		#	\&parse_freecovers
		#],

		# XXX urls results in "403 Forbidden"
		#rateyourmusic=> [
		#	'rateyourmusic.com',
		#	"http://rateyourmusic.com/search?searchterm=%s&searchtype=l",
		#	\&parse_rateyourmusic
		#],

		# XXX not sure it should be include, request too many big files from the server
		#discogs => [
		#	'discogs.com',
		#	"http://api.discogs.com/search?f=xml&type=releases&q=%s",
		#	\&parse_discogs
		#],
	},
);
...
my %menuitem = (
	...
	code => sub {
		Fetch($_[0]{mainfield}, $_[0]{gid}, $_[0]{ID});
	},

	# the menu item is displayed if returns true
	test => sub {
		$_[0]{mainfield} eq 'album' || $_[0]{mainfield} eq 'artist'
	},
);
...
my %fpane_menuitem = (
	...
	# menu item is hidden if more than one album/artist is selected
	onlyone => 'gidlist',

	# menu item is hidden for non artist/album (aa) FPanes
	istrue => 'aa',
);
...
sub prefbox {
	my $check1 = ::NewPrefCheckButton(OPT . 'ASK',    "Ask confirmation only if file already exists");
	my $check2 = ::NewPrefCheckButton(OPT . 'UNIQUE', "Find a unique filename if file already exists");
	my $entry1 = ::NewPrefEntry(OPT . 'COVERPATH');
	my $entry2 = ::NewPrefEntry(OPT . 'COVERFILE');
	my ($radio1a, $radio1b) = ::NewPrefRadio(OPT . 'USEPATH', ["use song folder", 0, "use :", 1]);
	my ($radio2a, $radio2b) = ::NewPrefRadio(OPT . 'USEFILE', ["use album name", 0, "use :", 1]);
...
sub Fetch {
	...
	::set_drag(
		...
		dest => [
			::DRAG_ID,
			sub {
				$_[0]->get_toplevel->SearchID(undef, $_[2]);
			}
		],
	);
...
sub SearchID {
	...
	if (!defined $ID) {
		...
		# for field like artists return an array of values, use first value
...
# FIXME could use a XML module	#can provide backcover and more too
sub parse_freecovers {
	my $result = $_[0];
	my @list;
	...
	while ($result =~ m#<title>(.+?)</title>#gs) {
		...
		# FIXME not sure of the encoding
		$res{desc} = ::decode_html(Encode::decode('cp1252', $1)) if $res =~ m#<name>([^<]+)</name>#;
...
sub parse_googlei {
	...
	for my $res (split /<div class="rg_meta[^"]*"[^>]*>/, $result) {
		...
		# escape \" to make extraction simpler, not perfect
		$res =~ s/(?<!\\)\\"/\\u0022/g;
		...
		for ($url, $desc, $ref, $preview) {
			# FIXME maybe use proper json decoding library
			s/\\u([0-9A-F]{4})/chr(hex($1))/eig;
		}
		push @list, {
			url        => $url,
			previewurl => $preview,
...
# to get more results than the 35 on the first page, it uses the first= url
# argument, but it behaves strangely, in particular it includes results from
# previous pages, so these are ignored. The final results are not exactly those
# you get from the web page, but it seems good enough
sub parse_bing {
	my ($result, $pageurl, $searchcontext) = @_;
	$searchcontext->{baseurl} ||= $pageurl;
	...
	while ($result =~ m/\s+m="([^"]+)"/g) {
		...
		# if ($seen->{$url}) {
		#	warn "result #".(++$searchcontext->{count})." was already found as #".$seen->{$url}."\n"
		#} #DEBUG
...
sub parse_yahoo {
	...
	# with the o=js parameter the result html is in a js file -> un-escape (the data is smaller with o=js)
	if ($result =~ m/^{"html":/) {
		$result =~ s#\\(.)#$1#g;
	}
	...
	while ($result =~ m/<li class="ld ?"([^>]+)><a +(?:target="[^"]*" +)?href=([^>]+)>(?:<img src=['"]([^'"]+)['"])?/g) {
		...
		if ($href =~ m/aria-label=["']([^"']+)["']/) {
			...
			# remove escaped bold markup around matched strings
			$desc =~ s#&lt;/?b&gt;##g;
	...
	# no idea what the parameters mean, they don't match the number of results, but it works ...
...
sub parse_ddg {
	...
	unless ($searchcontext->{vqd}) {
		...
		my $q   = $result =~ m/\?q=([^&"]+)[&"]/ ? $1 : 0;
		my $url = $vqd && $q
			? "https://duckduckgo.com/i.js?o=json&q=$q&vqd=$vqd&p=1"
			: undef;
		$searchcontext->{vqd} = $vqd;

		# third return parameter true means get next url even though no results in this query
		return [], $url, !!$url;
	}
	...
	for my $res (split /}\s*,[^{]*{/, $result) {
		...
		#if ($seen->{$url}) {
		#	warn "result #".($searchcontext->{count}+1)." : found previous result $seen->{$url}\n";
		#}
...
sub searchresults_cb {
	...
	unless ($ignore0 || @{$self->{results}}) {
		stop($self, "no matches found, you might want to remove some search terms.");
...
sub set_cover {
	...
	my $text;
	if ($self->{mainfield} eq 'album') {
		$text = ::__x("Use this picture as cover for album '{album}'", album => $name);
	} else {
		$text = ::__x("Use this picture for artist '{artist}'", artist => $name);
	}
	...
	if ($::Options{OPT . 'UNIQUE'}) {
		# find a unique name
		while (-e $default_dir . ::SLASH . $default_file) {
			last unless $default_file =~ s/(?:_(\d+))?\.(\w+)$/'_'.($1? $1+1 : 1).".$2"/e;
		}
	}
	my $file = $default_dir . ::SLASH . $default_file;
	if (!$::Options{OPT . 'ASK'} || -e $file) {
		$file = ::ChooseSaveFile($self, "Save picture as", $default_dir, $default_file, $check);
	}
	return unless $file;
