...
# The plugin file must have the following block before the first non-comment
# line, it must be of the format:
# =for gmbplugin PID
# name		short name
...
sub Start {
	# The handle to the Watch function must be a hash ref, if it is a
	# Gtk2::Widget, UnWatch will be called when the widget is destroyed
	$handle = {}; 
...
sub prefbox {
	...
	my $entry = ::NewPrefEntry(
		OPT . 'CMD',
		"Command when playing song changed :",
		expand => 1,
		sizeg1 => $sg1
	);
...
sub command_preview {
	...
	splice @cmd, $_, 0, ::MarkupFormat("\n<i>%s</i>", ::__x("argument {n} :", n => $_)) for reverse 1 .. $#cmd;
...
sub PlayStop {
	# TogPlay is undef when Stopped, 0 when Paused, 1 when Playing
	return if defined $::TogPlay;
...
# FIXME should simply call run_system_cmd($cmd, [$::SongID], 0);
# but the SENDSTDINPUT option make that difficult
sub Changed {
	my $ID  = $::SongID;
	my $cmd = $::Options{OPT . 'CMD'};
