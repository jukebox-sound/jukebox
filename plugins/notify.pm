...
sub Start {
	...
	# XXX is there a standard category for that?
	#$notify->set_category('music');
...
sub prefbox {
	...
	my $size = ::NewPrefSpinButton(
		...
		text   => "Picture size : %d",
		sizeg1 => $sg1,
		tip    => "Note that some notification daemons resize the displayed picture"
	);
	...
	for ($custom_actions, $default_action) {
		...
		$_->set_tooltip_text(
			"Actions are not supported by current notification daemon : " . $Daemon_name
		) unless $can_actions;
	}
	...
	$body->set_tooltip_text(
		"Body text is not supported by current notification daemon : " . $Daemon_name
	) unless $can_body;

	my $whenhidden = ::NewPrefCheckButton(
		OPT . 'onlywhenhidden',
