...
# and then runnning an external command with mandriva's gtk2
#$SIG{CHLD} = 'IGNORE';
...
sub check_version {
	...
	if ($version =~ m/^v?(\d+)\.(\d+)(\S+)?$/i) {
		$ok = $1 > $major || $2 >= $minor;
	} elsif ($version =~ m/^git-[[:xdigit:]]+$/i) {
		# assume git version is ok
		$ok = 1;
	} else {
		warn "mpv: error looking up mpv version\n";
	}
...
sub _remotemsg {
	...
	while (my $line = <$sockfh>) {
		# use JSON::PP->new->decode instead of decode_json (equivalent
		# to JSON::PP->new->utf8->decode) because decode_json converts
		# to utf8, which gives error with invalid utf8 filenames (only
		# happens for mpv >0.9.0)
		...
		if (my $error = $msg->{error}) {
			...
			# doesn't happen when previous file is same as new file
			if ($event eq 'property-change' && $msg->{name} eq 'path') {
				$mpv_file = $msg->{data} || "";
				...
				# only do eof now to catch log-message that are only
				# sent after end-file and start-file
				last if $eof;
			} elsif ($mpv_file ne $gmb_file) {
				# ignore all other events unless file is current
			} elsif ($event eq 'property-change' && $msg->{name} eq 'playback-time' && defined $msg->{data}) {
				::UpdateTime($msg->{data});
			} elsif ($event eq 'end-file') {
				# ignore EOF signal on user-initiated track
				# change as $mpv_file and $gmb_file aren't
				# equal in those cases
				$eof = 1;
...
sub convertvolume {
	...
	# XXX convert a linear volume to cubic volume scale doesn't seem to be
	# needed in mpv will be sent to mpv as string, make sure it use a dot
...
sub EQ_Get_Hz {
	...
	# differently: mplayer docs list band center frequences, GST reports
	# band start freqs.  Using GST values here for consistency.
	my @bands = (qw(
		29Hz 59Hz 119Hz 237Hz 474Hz 947Hz 1.9kHz 3.8kHz 7.5kHz 15.0kHz
		));
