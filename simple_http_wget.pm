...
sub get_with_cb {
	...
	# FIXME: not sure if we shouldn't escape something
	push @cmd_and_args, '--post-data=' . $post if $post;
	...
	if ($enc) {
		if ($enc eq 'gzip' && $gzip_ok) {
			...
			IO::Uncompress::Gunzip::gunzip(\$gzipped, \$self->{content})
				or do {
					warn "simple_http_wget : gunzip failed: $IO::Uncompress::Gunzip::GunzipError\n";
