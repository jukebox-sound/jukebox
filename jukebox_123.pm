...
# To make sure there are no zombies cause crash after displaying a file dialog
# and then running an external command with mandriva's gtk2.
#$SIG{CHLD} = 'IGNORE';
...
our %Commands = (
	...
	mpg123 => {
		type    => 'mp3',
		devices => sub {
			return grep $_ ne 'dummy', map m/^(\w+)\s+output*/g, qx(mpg123 --list-modules);
		},
		remote => {
			PAUSE   => 'P',
			RESUME  => 'P',
	...
	flac123 => {
		...
		# makes it higher priority than ogg123 (because ogg123 can't
		# seek in flac files)
		priority => 1,
	},
);
...
sub VolInit {
	# use amixer for volume
	return Play_amixer::init();
}
...
# mpg321 needs a frame number
sub mp3_sec_to_frame {
	my $sec = $_[0];
...
sub ogg123_cmdline {
	...
	if ($out) {
		...
		$out =~ s/^alsa/alsa09/ if (defined $alsa09 ? $alsa09 : $alsa09 = qx(ogg123) =~ m/alsa09/);
...
sub AdvancedOptions {
	...
	$ext{$_} = undef
		for map split(/ /, $Commands{$_}{type}), keys %Commands;

	my @ext = sort keys %ext;
	for my $e (@ext) {
		$ext{$e} = join '/', $e, sort grep $::Alias_ext{$_} eq $e, keys %::Alias_ext;
	}
	...
	for my $cmd (sort keys %Commands) {
		...
		# don't try to find dynamic list of devices if command not
		# found, as the function likely requires the command
		$devs = '' if ref $devs  && !$Commands{$cmd}{found};
			...
			if (exists $cando{$ext}) {
				...
				$w->signal_connect(
					toggled => sub {
						...
						$Supported{$ext} = $::Options{'123priority_' . $ext} = $cmd;

						$Supported{$_} = $Supported{$ext}
							for grep $::Alias_ext{$_} eq $ext, keys %::Alias_ext;
					}
				);
			}
			$i++;
		...
		unless ($Commands{$cmd}{found}) {
			$_->set_sensitive(0) for @widgets;
		}
	}
...
sub init {
	...
	unless ($mixer) {
		warn "amixer not found, won't be able to get/set volume through the 123 or mplayer output.\n";
	}
...
sub init_volume {
	...
	if ($h{PCM}) {
		...
		warn "Don't know what mixer to choose among : @list\n";
	}
...
sub GetVolumeError {
	...
	. "Needs amixer (packaged in alsa-utils) to change volume when using this audio backend.";
}
