...
sub write_file {
	...
	# concatenate newly generated comment packet and setup packet from the
	# original file in $data, and compute the segments in @segments
	my $data;
	my @segments;
	...
	my $continued   = 0;
	{
		...
		# make page of max 4095+255 bytes
		while ($size < 4096) {
		...
		my $page = pack('a4aa x8 a4 V x4 C', 'OggS', $version, $continued, $serial, $pagenb++, $nbseg)
			. $segments
			. substr($data, $data_offset, $size);
	...
	if (substr($data, 0, 4) eq 'OggS' && unpack('x18V', $data) eq $pagenb) {
		...
		# read each page
		while (my $pageref = _ReadPage($self)) {
...
sub _ReadComments {
	...
	if (my $packref = _read_packet($self, PACKET_COMMENT)) {
		...
		# upgrade old embedded pictures format to metadata_block_picture
		if (my $covers = $comments{coverart}) {
...
sub remove {
	...
	unless (defined $val) {
		warn "comment to delete not found\n";
...
sub _read_packet {
	...
	do {
		...
		until ($lastseg) {
			...
			unless (@$seg_table) {
				_read_page_header($self) || return undef
			}
	...
	if ($type == $wantedtype && $vorbis eq 'vorbis') {
		return \$packet;
	} else {
		return undef;
	}
...
sub _read_page_header {
	...
	if ($self->{serial} && $self->{serial} ne $sn) {
		warn "corrupted page : serial number doesn't match\n";
...
sub _recompute_page_crc {
	...
	if ($digestcrc) {
		$digestcrc->add($$pageref);

		$crc = $digestcrc->digest;
	} else {
		# pure-perl: SLOW
...
sub _skip_to_last_page {
	...
	while ($buf =~ m/OggS\x00(.)(.{8})(.{4})/gs) {
		...
		# granule == -1 --> no packets finish on this page
		$granule = $2 unless $2 eq "\xff\xff\xff\xff\xff\xff\xff\xff";

		# last page of logical bitstream
		next unless vec $1, 2, 1;
