...
sub _FindTags {
	...
	my $apefooter = substr($id3v1, -32, 8) eq 'APETAGEX'  &&  substr($id3v1, -8) eq ("\x00" x 8);

	# ID3v1 tag
	if (!$apefooter && substr($id3v1, 0, 3) eq 'TAG') {
...
sub write_file {
	...
	warn "startaudio=" . $self->{startaudio} . " copybegin=$copybegin length(towritebefore)="
		. join(' ', map(length $$_, @towritebefore)) . "\n" if $::debug;

	warn "endaudio=" . $self->{endaudio} . " copyend=$copyend length(towriteafter)="
...
sub _FindFirstFrame {
  ...
  SEARCH1ST:
	...
	# Only look in the first 60.000 bytes (after tag)
	while ($pos < 60000) {
...
sub _CountFrames {
	...
	while (read $fh, $_, 4) {
		if (substr($_, 0, 2) eq $byte1_2) {
			...
			#my $s = $cache{substr($_,2,1)} ||= (
			#	(vec $_,17,1)
			#		? $size[ (vec $_,2,8)>>4 ] + $pad
			#		: $size[ (vec $_,2,8)>>4 ]
			#)
			#};# a bit faster, needs a my %cache
	...
	# if a file has been rewrote while reading, mark the info as
	# suspicious
	$info->{estimated} = 1 if $MODIFIEDFILE;
...
sub new_from_string {
	my $string = $_[1];
	my ($title, $artist, $album, $year, $comment, $vers1_1, $track, $genre) = unpack 'x3 Z30 Z30 Z30 Z4 Z28 C C C', $string;
...
# http://www.id3.org/lyrics3.html #untested
sub new_from_file {
	my ($class, $file) = @_;
...
sub new_from_file {
	...
	if ($isfooter) {
		seek $fh, $file->{endaudio} - 32, 0;
	} else {
		seek $fh, $file->{startaudio}, 0;
	}
	...
	if ($flags & 0x20000000) {
		# if $headorfoot is a header

		read $fh, $rawtag, $size - 32;
		# check footer
		return undef unless ($flags & 0x40000000) || $rawtag =~ m/APETAGEX.{24}$/s;
	} else {
		# $headorfoot is a footer -> must seek backward
		seek $fh, -$size, 1;
		read $fh, $rawtag, $size;
	...
	for my $key (values %{$tag->{realkey}}) {
		...
		for my $vt (@$values_types) {
			...
			unless ($type & 0b10) {
				...
				# previous one with same type
				my ($prev) = grep $_->[1] == $type, @towrite;
				if ($prev) {
...
sub new_from_file {
   ...
   # Where yy is less than $FF, xx is the 'flags' byte and zz is less than  $80
    ...
    if ($flags & 0b01000000)    #Extended header	#currently unused & untested
    {
        ...
        if ($v1 == 3) {
            # CHECKME V or N
            my ($f, $padsize, $crc) = unpack 'C2VV', $extheader;
            $tag{crc} = $crc if $f & 0x8000;
            warn "padding $padsize\n" if $::debug;

            # FIXME use remove padding
            # CHECKME find a file who has $padding
            #substr $rawtag,-$padsize,$padsize,'';
        }
        elsif ($v1 == 4) {
    ...
    while ($pos < $maxpos) {
        ...
        # DEBUG
        warn "........padding\n"
            if $::debug && (substr($rawtag, $pos, 1) eq "\x00");

        # reached padding
        last if substr($rawtag, $pos, 1) eq "\x00";

        # v2.2
        if ($v1 == 2) {
            ($frame, $fsize, my @size) =
                unpack 'a3CCC', substr $rawtag, $pos, 6;
            $pos += 6;
        ...
        # v2.3 and v2.4
            ...
            ($frame, $fsize, $f1, $f2) =
                unpack 'a4a4CC', substr $rawtag, $pos, 10;
        ...
        # check if valid frameID
        my $error;
        unless ($frame =~ m/^[A-Z0-9]+$/) {
        ...
        # end of tag
        if (!$error && length($rawtag) < $fsize + $pos) {
        ...
        if ($error) {
            ...
            if ($broken24 < 3) {
                ...
                # keep the best
                if (@{$tag{brokenframesorder}} >= @{$tag{framesorder}}) {
        ...
        if ($v1 == 3) {
            if ($f2 & 0b10000000) {
                ...
                unless ($Zlib) {
                    warn "Compressed frame $frame can't be read because Compress::Zlib is not found.\n";
                    next;
                }
...
# same as add but put it first (of its kind)
sub insert {
	my ($self, $fname, $data) = @_;
...
sub get_values {
	...
	# for multi fields (COMM for example)
	if (@extra && ref $v->[0]) {
		@values = map {
			my $v_ok;
			my $notok;
...
sub make {
	...
	for my $frameid (@{$tag->{framesorder}}) {
		...
		if ($fields[-1] eq '*') {
			...
			if ($v1 == 4) {
				#put all values in the same frame
		...
		for my $t (@fields) {
			...
			if ($t eq 't') {
				...
				$string .= chr($val) . ("\x00" x (3 - length $string));
	...
	# 1-byte padding to avoid false sync
	$tagstring .= "\x00" if substr($tagstring, -1, 1) eq "\xff";
...
# To convert TCON from id3v2.3 (and from id3v2.2) to id3v2.4, and replace
# numerical or CR/RX genres by the corresponding string genre
sub _genreid {
	my ($ref, $version) = @_;

	# id3v2.2/3 format (NUMBER) (CR) or (RX), shouldn't use this format in
	# id3v2.4 but support it anyway
	for my $g (@$ref) {
		if ($g =~ s#^\((\d+|RX|CR)\)##) {
			# possible second genre following (NUMBER)
			push @$ref, $g if length $g;

			$g = $1;
		} elsif ($version != 4) {
			# only un-escape '((' into '(' for id3v2.2/3 tags, not
			# supposed to be used in id3v2.4 tags
			$g =~ s#^\(\(#(#;
		}
