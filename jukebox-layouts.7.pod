...
jukebox' interface is defined by layouts.  Each layout define a window
that contain several containers, each containing widgets.
...
The `hidetabs=1` container option can be used to hide the tabs, this
...
Option: `label` sets the text that will be displayed next to the
expander.
...
preceding the number with a dot (`.`).
...
one of `none`, `in`, `out`, `etched-in` (default), `etched-out`
...
container, using the `hover_layout` option on the HB won't work, but
putting the HB inside a WB and using the `hover_layout` option on the
...
Layouts of type G may optionally be followed a plus sign `+` that will
...
with the `OpenCustom` command or with the `-layout` command-line
option.
