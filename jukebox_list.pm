...
sub ITER_NTH_CHILD {   #my ($self, $parent, $n) = @_; #warn "ITER_NTH_CHILD\n";
...
sub search_equal_func {       #my ($self,$col,$string,$iter)=@_;
...
sub rowinsert {
    ...
    for (1 .. $number) {
        $self->{size}++;
        $self->row_inserted(Gtk2::TreePath->new($row),
            $self->get_iter_from_string($row));
        $row++;
    }
}
...
sub button_press_cb {
    ...
    return 0
      ...
      ;   #ignore click outside the bin_window (for example the column headers)
    ...
    if ($but == 1
      ) # do not clear multi-row selection if button press on a selected row (to allow dragging selected rows)
    {
        {
            last
              if $ctrl_shift
              ;    #don't interfere with default if control or shift is pressed
...
our @cMenu = (
    ...
    },
    {
        ...
        code      => sub {
            ...
            ::ChooseAAPicture(undef, $_[0]{field}, $gid);
        },
        onlyone => 'gidlist',
        test    => sub {
    ...
    },
    {
        ...
        code      => sub {
            my $gid = $_[0]{gidlist}[0];
            Songs::ChooseIcon($_[0]{field}, $gid);
        },
        onlyone => 'gidlist',
        test    => sub {
    ...
    },
    {
        ...
        code         => sub {
            my $fp = $_[0]{filterpane};
            $fp->{notebook}->set_show_tabs(!$fp->{hidetabs});
        },
    },
);
...
sub new {
    ...
    $self->{$_} = $opt->{$_}
      for qw/nb group min hidetabs tabmode/,
      grep(m/^activate\d?$/, keys %$opt);
    $self->{main_opt}{$_} = $opt->{$_}
      for qw/group no_typeahead searchbox rules_hint hscrollbar/
      ;    #options passed to children
    my $nb    = $self->{nb};
    my $group = $self->{group};

    my $spin = Gtk2::SpinButton->new(
        Gtk2::Adjustment->new($self->{min}, 1, 9999, 1, 10, 0),
        10, 0);
    $spin->signal_connect(
        value_changed => sub { $self->update_children($_[0]->get_value); });
    my $ResetB = ::NewIconButton('gtk-clear', undef,
        sub { ::SetFilter($_[0], undef, $nb, $group); });
    ...
    $InvertB->signal_connect(
        toggled => sub { $self->{invert} = $_[0]->get_active; });
    $InterB->signal_connect(
        toggled => sub { $self->{inter} = $_[0]->get_active; });
    ...
    if (my $tabpos = $opt->{tabpos}) {
        ($tabpos, $self->{angle}) =
          $tabpos =~ m/^(left|right|top|bottom)?(90|180|270)?/;
    ...
    $self->AppendPage('album')
      if $notebook->get_n_pages
      == 0;    # fallback in case no pages has been added
    ...
    $notebook->signal_connect(
        switch_page => sub {
            ...
            my $mask =
                $Pages{$pid}                         ? $Pages{$pid}[2]
              : Songs::FilterListProp($pid, 'multi') ? 'oni'
              :                                        'on';
...
sub AppendPage {
    ...
    $n = $notebook->get_n_pages - 1
      ; # $notebook->append_page doesn't returns the page number before Gtk2-Perl 1.080
...
# set base gravity to auto so that rotated tabs handle vertical scripts (asian languages) better
    ...
    $label->signal_connect(hierarchy_changed =>
          sub { $_[0]->get_pango_context->set_base_gravity('auto'); })
      ; # for some reason (gtk bug ?) the setting is reverted when the tab is dragged, so this re-set it

    if ($self->{tabmode} ne 'text') {
        my $icon = "gmb-tab-$pid";
        $img = Gtk2::Image->new_from_stock($icon, 'menu')
          if Gtk2::IconFactory->lookup_default($icon);
        $label = undef if $img && $self->{tabmode} eq 'icon';
    }
    my $tab;
    if ($img && $label) {
        $tab =
          $angle % 180 ? Gtk2::VBox->new(FALSE, 0) : Gtk2::HBox->new(FALSE, 0);
        my @pack =
          $angle % 180
          ? ($label, TRUE, $img, FALSE)
          : ($img, FALSE, $label, TRUE);
...
sub button_press_event_cb {
    ...
    return 0
      unless ::IsEventInNotebookTabs($nb, $event)
...
sub set_mode {
    ...
    my ($child, $view) =
        $mode eq 'cloud'  ? $self->create_cloud
    ...
    $view->signal_connect(
        focus_in_event => sub {
            my $self = ::find_ancestor($_[0], __PACKAGE__);
            $self->{isearchbox}->parent_has_focus;
            0;
        }
    ...
    {
        {
            ...
            my @iters = map $store->get_iter($_),
              $self->{view}->get_selection->get_selected_rows;
            last unless @iters;
            if ($store->get_value($iters[0], 0)
                == GID_ALL)    # assumes "All row" first iter
            {
                my $iter =
                  $store->get_iter_first; # this iter is "All row" -> not added
...
sub drag_cb {
    ...
    my $field = $self->{field}[0];
    if (my $drag =
        Songs::FilterListProp($field, 'drag'))    #return artist or album gids
    {
        if ($self->{mode} eq 'list') {
            my $store = $self->{view}->get_model;
            my @rows  = $self->{view}->get_selection->get_selected_rows;
            unless (grep $_->get_depth > 1, @rows) {
                my @gids = map $store->get_value($store->get_iter($_), 0),
                  @rows;
                warn "dnd : gids=@gids\n";
                if (grep $_ == GID_ALL, @gids) {
                    return ::DRAG_FILTER, '';
                }                                 #there is an "all-row"
                return $drag, @gids;
            }
...
sub row_expanded_cb {
    ...
    for (my $iter = $piter; $iter; $iter = $store->iter_parent($iter)) {
        push @filters,
          Songs::MakeFilterFromGID($self->{type}[$store->iter_depth($iter)],
            $store->get($iter, 0));
    }
    ...
    my $children =
      AA::SortKeys($type, [keys %$h], $self->{'sort'}[$depth + 1]);
    for my $i (0 .. $#$children) {
        my $iter =
          $store->iter_nth_child($piter, $i) || $store->append($piter);
        $store->set($iter, 0, $children->[$i]);
    }
    while (my $iter = $store->iter_nth_child($piter, $#$children + 1)) {
        $store->remove($iter);
    }
...
sub get_fill_data {
    ...
    my $href = $self->{hash}
      ||= Songs::BuildHash($type, $filterpane->{list}, 'gid');
    $self->{valid}     = 1;
    $self->{all_count} = keys %$href
      ;    #used to display how many artists/album/... there is in this filter
    ...
    if (defined $search && $search ne '') {
        @list = @{
            AA::GrepKeys(
                $type, $search,
                $self->{search_is_regexp},
                $self->{search_is_casesens}, \@list
            )
        };
    }
    ...
    if (   $self->{mode} eq 'cloud'
        && $self->{cloud_stat} ne
        'count')                  #FIXME update cloud when used fields change
    {
        $href = Songs::BuildHash($type, $filterpane->{list}, 'gid',
            $self->{cloud_stat});
    }
...
sub Fill {
    ...
    if ($self->{mode} eq 'list') {
        ...
        $self->{busy} = 1;
        $store->clear
          ; #FIXME keep selection ?   FIXME at least when opt is true (ie lmarkup or lpicsize changed)
        ...
        if ($self->{histogram}) {
            $max = ::max(
                $self->{histogram_ignore_none}
                ? (map $href->{$_}, grep $_ != 0, keys %$href)
                : values %$href
            );
        }
        $renderer->set(
            hash        => $href,
            max         => $max,
            ignore_none => $self->{histogram_ignore_none}
        );
        $self->{array_offset} =
          $self->{noall}
          ? 0
          : 1
          ; #row number difference between store and $list, needed by interactive search
        $store->set($store->prepend(undef), 0, $_)
          for
          reverse @$list;  # prepend because filling is a bit faster in reverse
        ...
        if ($self->{field}[1])    # add a children to every row
        {
            ...
            $first = $store->iter_next($first)
              if $first && $store->get($first, 0) == GID_ALL;   #skip "all" row
...
sub new {
    ...
    my $column =
      Gtk2::TreeViewColumn->new_with_attributes(Songs::FieldName($col),
        $renderer);
    $column->set_cell_data_func(
        $renderer,
        sub {
...
sub Fill {
    ...
    my $filterpane = ::find_ancestor($self, 'FilterPane');
    my $href       = $self->{hash}
      ||= BuildTreeRef($filterpane->{list}, $treeview->{expanded},
        $self->{simplify});
    my $min   = $filterpane->{min};
    my $store = $treeview->get_model;
    $self->{busy} = 1;
    ...
    my @toadd;
    my @toexpand;
    push @toadd, $href->{$_}, $_, undef
      for sort grep $href->{$_}[0] >= $min, keys %$href;
    while (my ($ref, $name, $iter) = splice @toadd, 0, 3) {
        my $iter = $store->append($iter);
        $store->set($iter, 0, Songs::filename_escape($name));
        push @toexpand, $store->get_path($iter)
          if ($ref->[2] || 0) & IsExpanded;
        if ($ref->[1])    #sub-folders
        {
            push @toadd, $ref->[1]{$_}, $_, $iter
              for sort grep $ref->[1]{$_}[0] >= $min, keys %{$ref->[1]};
        }
    }
    ...
    if (my $iter = $store->get_iter_first) {
        $iter = $store->iter_children($iter)
          while $store->iter_n_children($iter) == 1;
...
sub BuildTreeRef {
    ...
    while (my ($f, $n) = each %$h) {
        my $ref = \@hier;
        $ref = $ref->[1]{$_} ||= [] and $ref->[0] += $n
          for split /$::QSLASH/o, $f;
        $ref->[2] |= HasSongs;
    }
    ...
    if ($simplify ne 'never') {
        ...
        while (@tosimp) {
            ...
            for my $path (@tofuse) {
                ...
                my @child = keys %{$ref->[1]};
                unless (@child == 1 && !(($ref->[2] || 0) & HasSongs)) {
                    push @tosimp, $ref;
                    next;
                }
...
sub PopupContextMenu {
    ...
    FilterPane::PopupContextMenu(
        ...
        {   self        => $self,
            rawpathlist => \@raw,
            pathlist    => \@paths,
            filter      => _MakeFolderFilter(@paths),
            folderview  => 1,
        }
    );
}
...
sub Fill {
    ...
    my $root     = ::SLASH;
    $root = 'C:'
      if $^O eq 'MSWin32';    #FIXME Win32 find a way to list the drives
...
sub cell_data_func_cb {
    ...
    Glib::Timeout->add(10, \&idle_load, $treeview)
      unless $treeview->{queued_load};
...
sub idle_load {
    ...
    while (my $path = shift @$queue) {
        next
          unless $path->compare($first) >= 0
          && $path->compare($last) <= 0;    # ignore if out of view
        my $self    = ::find_ancestor($treeview, __PACKAGE__);
...
sub refresh_path {
    ...
    if ($self->{in_progress}) {
        if ($full || $self->{in_progress}{folder} ne $folder) {
    ...
    if (!$continue)    # check folder is there and if treeview up-to-date
    {
        ...
        $lastmodif = (stat $dh)[9]
          || 1;        # ||1 to ùake sure it isn't 0, as 0 means not read
...
sub _MakeFolderFilter {
    ...
    my $filter =
      Filter->new('', \@list); #FIXME use a filter on path rather than a list ?
    return $filter;
}
...
sub new {
    ...
    my $store = Gtk2::TreeStore->new(('Glib::String') x 4, 'Glib::Boolean');
    ...
    ::set_drag(
        ...
        dest => [
            ::DRAG_FILTER, ::DRAG_ID,
            sub    #targets are modified in drag_motion callback
            {
...
sub fill_savednames {
    ...
    $store->set($store->append($iter), 0, $_, 1, $type, 4, TRUE)
      for sort keys %{$::Options{$hkey}}
      ; #FIXME use case and accent insensitive sort #should use GetListOfSavedLists() for SavedLists
...
sub PopupContextMenu {
    ...
    FilterPane::PopupContextMenu($self, \%args,
        [@cMenu, {separator => 1}, @FilterPane::cMenu]);
}
...
sub drag_motion_cb {
    ...
    my $status;
    {
        ...
        my $target_filter =
          [$::DRAGTYPES[::DRAG_FILTER][0], [], ::DRAG_FILTER];
        my $lookfor;
        my @targets;
        ...
        if ($lookfor && grep $::DRAGTYPES{$_->name} == $lookfor,
            $context->targets)
        {
            $status = 'copy';
            $treeview->drag_dest_set_target_list(
                Gtk2::TargetList->new(@targets));
        }
    }
...
sub new {
    ...
    $aa = 'album'
      unless $aa eq 'artists';    #FIXME PHASE1 change artist to artists
    $self->{aa}       = $aa;
    ...
    for my $name (qw/Ltitle Lstats/) {
        ...
        if ($name eq 'Ltitle') {
            $l->set_line_wrap(1);
            $l->set_ellipsize('end')
              ;    #FIXME find a better way to deal with long titles
    ...
    $img->signal_connect(size_allocate => \&size_allocate_cb)
      unless $self->{nopic};
    $pixbox->add($img);
    $pixbox->signal_connect(
        button_press_event => \&GMB::Picture::pixbox_button_press_cb,
        1
    );    # 1 : mouse button 1

    my $buttonbox = Gtk2::VBox->new;
    my $Bfilter   = ::NewIconButton(
        'gmb-filter',
        undef,
        sub { my $self = ::find_ancestor($_[0], __PACKAGE__); $self->filter },
        'none'
    );
    ...
    $Bplay->signal_connect(
        button_press_event => sub    #enqueue with middle-click
        {
            my $self = ::find_ancestor($_[0], __PACKAGE__);
    ...
    ::set_drag(
        $self,
        source => [$drgsrc, sub { $drgsrc, $_[0]{Sel}; }],
        dest   => [
            ::DRAG_ID,
            ::DRAG_FILE,
            sub {
                ...
                if ($type == ::DRAG_FILE) {
                    ...
                    my $file = $values[0];
                    if ($file =~ s#^file://##) {
                        AAPicture::SetPicture($self->{aa}, $self->{Sel},
                            ::decode_url($file));
                    }
    ...
    ::Watch($self,
        "Picture_"
          . ($aa eq 'album' ? 'album' : 'artist') => \&AAPicture_Changed);
...
sub new {
    ...
    $self->signal_connect_after(
        activate => sub { ::run_command($_[0], $opt->{activate}); })
      if $opt->{activate};

    #$self->set_width_chars($opt->{width_chars}) if $opt->{width_chars};
    unless ($opt->{noselector}) {
        if (*Gtk2::Entry::set_icon_from_stock{CODE}
          )    # requires gtk>=2.16 && perl-Gtk2 version >=1.211
        {
            ...
            $self->signal_connect(
                icon_press => sub {
                    my ($self, $iconpos) = @_;
                    if   ($iconpos eq 'primary') { $self->PopupSelectorMenu }
                    else                         { $self->ClearFilter }
                }
            );
    ...
    $self->{$_} = $opt->{$_}
      for qw/nb fields group searchfb/, keys %Options, keys %Options2;
    $self->{SaveOptions} = \&SaveOptions;
    ::WatchFilter($self, $self->{group},
        sub { $_[0]->Update_bg(0); $_[0]->UpdateClearButton; })
      unless $opt->{noselector};   #to update background color and clear button
    return $self;
}
    ...
    $item2->signal_connect(
        activate => sub {
            ::EditFilter($self, ::GetFilter($self), undef,
                sub { ::SetFilter($self, $_[0]) if defined $_[0] });
        }
    );
    $menu->append($item2);
    ::PopupMenu($menu);
...
sub EntryChanged_cb {
    ...
    { # 1st timeout : do not filter before this minimum timeout, even if filter is ready
        ...
        $self->{changed_timeout} = Glib::Timeout->add($timeout,
            sub { $self->AutoFilter('time_ready'); 0 });

        # 2nd timeout : filter even if idlefilter not finished
        $timeout = $l < 4 ? 3000 : 2000;
        $self->{idlefilter_timeout} = Glib::Timeout->add($timeout,
            sub { $self->AutoFilter('filter_ready', 'force'); 0 });
    }
    $self->StartIdleFilter
      if $self->{autofilter} || ($l > 2 && !$self->{suggest});
    if ($self->{suggest}) {
        ...
        if ($timeout) {
            $self->{suggest_timeout} =
              Glib::Timeout->add($timeout, \&UpdateSuggestionMenu, $self);
        }
...
sub scroll_event_cb   #increase/decrease numbers when using the wheel over them
{
    ...
    return 0
      unless $event->window
      == $self->get_text_window;    #ignore if pointer outside the text area
    my $text0 = $self->get_text;
    my ($offx, $offy) = $self->get_layout_offsets;
    my $x      = $event->x - $offx;
    my $layout = $self->get_layout;
    my ($index, $trailing) = $layout->xy_to_index(Gtk2::Pango->scale * $x, 0)
      ;                             #y always 0, as only one line
    $index = $x < 0 ? 0 :           # if pointer before the text
      defined $index
      ? $self->layout_index_to_text_index($index)
      : length($text0)
      - 1;    # if pointer after the text, do as if at the end of text
    my $pos  = 0;
    my $text = '';
    my $found;
...
sub UpdateSuggestionMenu {
    my $self = shift;
    if ($self->{matchmenu} && !$self->{matchmenu}->mapped) {
    ...
    my ($x, $y) =
      $self->window->get_origin;  # position of the parent widget on the screen
    my ($dx, $dy) =
      $self->window->get_size;    # width,height of the parent widget
    ...
    for my $field (qw/artists album genre label title/) {
        ...
        if ($field eq 'title') {
            ...
            $filter->add_possible_superset($self->{last_suggestion_filter})
        ...
        $label0->set_markup_with_format(
            "<i> %s : %d</i>",
            Songs::FieldName($field),
            scalar(@$list)
        );
        ...
        if ($field eq 'title') { $item0->set_sensitive(0) }
            ...
            $item0->{list}   = $list;
            $item0->{format} = $format;
            $item0->signal_connect(
                button_press_event => \&SuggestionMenu_field_expand)
        ...
        for my $i (0 .. ::min(4, $#$list)) {
            ...
            my $item;
            if (   $field eq 'artists'
                || $field eq 'album')    #FIXME be more generic
            {
            ...
            elsif ($field eq 'label')    #FIXME be more generic
            {
                if (my $icon = Songs::Picture($val, $field, 'icon')) {
                    $item = Gtk2::ImageMenuItem->new;
                    $item->set_image(
                        Gtk2::Image->new_from_stock($icon, 'menu'));
                }
            }
            $item ||= Gtk2::MenuItem->new;
            my $markup;
            if ($field eq 'title') {
                $markup = ::ReplaceFieldsAndEsc($val, $format);
            }
            else {
            ...
            $item->signal_connect(button_press_event =>
                  sub { $_[0]{middle} = $_[1]->button == 2; });
            $item->signal_connect(
                activate => \&SuggestionMenu_item_activated_cb);
            $item->add($label);
            $item->show_all;
    ...
    if ($menu->mapped) {
        ...
        $menu->signal_connect(
            key_press_event => \&SuggestionMenu_key_press_cb);
        $menu->signal_connect(selection_done =>
              sub { $_[0]->get_attach_widget->CloseSuggestionMenu });
        $menu->popup(
            undef, undef,
            sub {
                my $menu = shift;
                $x, ($above ? $y - $menu->size_request->height : $y + $h);
            },
            undef,
            0,
...
sub new {
    ...
    $entry->signal_connect(
        activate => sub { $_[0]->parent->CloseSuggestionMenu; });
    $entry->signal_connect(
        key_press_event => sub {
            my ($entry, $event) = @_;
            return 0 unless Gtk2::Gdk->keyval_name($event->keyval) eq 'Escape';
    ...
    $entry->signal_connect_after(
        activate => sub { ::run_command($_[0]->parent, $opt->{activate}); })
      if $opt->{activate};
      ...
      )
    {
        ...
        $but->signal_connect(
            expose_event =>
              sub    #prevent the button from beign drawn, but draw its child
            {
                my ($but, $event) = @_;
...
#$but->signal_connect(realize => sub { $_[0]->window->set_cursor(Gtk2::Gdk::Cursor->new('hand2')); });
        ...
        if ($stock eq 'gtk-clear') {
            ...
            $entry->signal_connect(
                changed => sub { $_[0]->parent->UpdateClearButton });
            $but->set_sensitive(0);
        }
    }
    $self->pack_start($entry, 1, 1, 0);
    $entry->set('has-frame', 0);
    $entry->signal_connect($_ => sub { $_[0]->parent->queue_draw })
      for qw/focus_in_event focus_out_event/;
    $self->signal_connect(
        expose_event => sub    #draw an entry frame inside $self
        {
            my ($self, $event) = @_;
            my $entry = $self->{entry};
            if ($entry->state eq 'normal') {
                my $s = $self->{filtered} && !$entry->is_focus;
                $entry->modify_base('normal',
                    ($s ? $entry->style->bg('selected') : undef));
                $entry->modify_text('normal',
                    ($s ? $entry->style->text('selected') : undef));
            }
            $entry->style->paint_flat_box($self->window, $entry->state,
                'none', $event->area, $entry, 'entry_bg',
                $self->allocation->values);
            $entry->style->paint_shadow($self->window, 'normal',
                $entry->get('shadow-type'),
                $event->area, $entry, 'entry', $self->allocation->values);
...
sub new {
    ...
    $treeview->append_column(
        Gtk2::TreeViewColumn->new_with_attributes('', $renderer, gid => 0));
    $self->{activate} = $opt->{activate} || 'queue';
    $treeview->signal_connect(row_activated => \&Activate);

    $self->{field} = $opt->{aa} || 'artists';
    $renderer->set(prop => [[$self->{field}], [1], [32], [0]], depth => 0)
      ;    # (field markup=1 picsize=32 icons=0)
    $self->{drag_type} =
      Songs::FilterListProp($self->{field}, 'drag') || ::DRAG_FILTER;
    ::set_drag(
        $treeview,
        source => [
            $self->{drag_type},
            sub {
                ...
                my @gids = map $store->get_value($store->get_iter($_), 0),
                  @rows;
                if ($self->{drag_type}
                    != ::DRAG_FILTER)    #return artist or album gids
                {
                    return $self->{drag_type}, @gids;
                }
                else {
                    my @f = map Songs::MakeFilterFromGID($self->{field}, $_),
                      @gids;
                    my $filter = Filter->newadd(::FALSE, @f);
                    return (
                        $filter ? (::DRAG_FILTER, $filter->{string}) : undef);
                }
            }
        ]
    );
...
sub RENDER {
    ...
    for my $pb (@pb) {
        $window->draw_pixbuf($widget->style->fg_gc($state),
            $pb, 0, 0, $x, $y, -1, -1, 'none', 0, 0);
        $row++;
        if   ($row < $nb) { $y                         += PAD + $h; }
...
sub RENDER {
    ...
    my ($prop, $gid, $depth, $hash, $max, $ignore_none) =
      $cell->get(qw/prop gid depth hash max ignore_none/);
    ...
    if ($psize && $gid != FilterList::GID_ALL) {
        ...
        my $pixbuf =
            $iconfield
          ? $widget->render_icon(Songs::Picture($gid, $field, 'icon'), 'menu')
          || undef
        ...
        if ($pixbuf)    #pic cached -> draw now
        {
            ...
            defined $pixbuf
          )    #pic exists but not cached -> load and draw in idle
        {
    ...
    if (   $max
        && !$depth
        && !($flags & 'selected')
        && $gid != FilterList::GID_ALL
        && !($gid == 0 && $ignore_none)
      )    #draw histogram only works for depth==0
    { # if parent widget is a scrolledwindow, maxwidth use the visible width instead of the total width of the treeview
        my $maxwidth =
            $widget->parent->isa('Gtk2::ScrolledWindow')
          ? $widget->parent->get_hadjustment->page_size
          : $cell_area->width;
        ...
        $widget->style->paint_flat_box(
            $window,           $state,
            'none',            $expose_area,
            $widget,           'cell_odd_ruled',
            $x + $psize + PAD, $cell_area->y,
            $width,            $cell_area->height
        );
    }

    # draw text
    $widget->get_style->paint_layout(
        $window, $state, 1,                 $cell_area,
        $widget, undef,  $x + $psize + PAD, $y + $offy,
        $layout
    );
...
sub RENDER {
    my ($cell, $window, $widget, $background_area, $cell_area, $expose_area,
        $flags)
      = @_;
    my ($r1, $r2, $row, $gid) = @{$cell->get('ref')}
      ; #come from CellRendererSongsAA::get_value : first_row, last_row, this_row, gid
...
#		$widget->get_style->paint_layout($window, $widget->state, 0, $cell_area, $widget, undef, $x, $y, $layout);
    ...
    $window->draw_rectangle($gc, 1, $background_area->values);
    my ($x, $y, $width, $height) =
      $background_area->values;    #warn "$row $x, $y, $width, $height\n";
...
#	$height-=$ypad*2;
    ...
    if (my $pixbuf = AAPicture::pixbuf($field, $gid, $s)) {
        ...
        my ($tx, $ty) =
          $widget->widget_to_tree_coords($x, $y); #warn "$tx,$ty <= ($x,$y)\n";
...
sub _drawpix {
    my ($widget, $window, $ctx, $cty, $gid, $s, $col) =
      @_;    #warn "$ctx,$cty,$gid,$s\n";
    my ($vx, $vy, $vw, $vh) = $widget->get_visible_rect->values;
    ...
    return unless $pixbuf;
    $window->draw_pixbuf(Gtk2::Gdk::GC->new($window),
        $pixbuf, 0, 0, $x, $y, -1, -1, 'none', 0, 0);
}
...
sub get_value {
    ...
    $r1--
      while $r1 > 0
      && Songs::Get_gid($array->[$r1 - 1], $field)
      == $gid;    #find first row with this gid
    $r2++
      while $r2 < $#$array
      && Songs::Get_gid($array->[$r2 + 1], $field)
      == $gid;    #find last row with this gid
...
sub Fill #FIXME should be called when signals ::style-set and ::direction-changed are received because I keep layout objects
{
    ...
    my $filterlist = ::find_ancestor($self, 'FilterList')
      ;  #FIXME should get its options another way (to keep GMB::Cloud generic)
    ...
    for my $key (@$list) {
        ...
        my $value  = sprintf '%.1f',
          $scalemin + $scalemax * ($href->{$key} - $min) / ($max - $min);

#$layout->set_text($key);
#$layout->get_attributes->insert( Gtk2::Pango::AttrScale->new($value) ); #need recent Gtk2
        my $text = $displaykeysub ? $displaykeysub->($key) : $key;
        $layout->set_markup('<span size="'
              . (10240 * $value) . '"> '
              . ::PangoEsc($text)
              . '</span> ');
        my ($w, $h) = $layout->get_pixel_size;
        my $bl = $self->{baselines}{$h} ||= $layout->get_iter->get_baseline
          / Gtk2::Pango->scale;    #cache not needed for $Gtk2::VERSION>1.161
        if ($x + $w + XPAD > $width) {
        ...
        if (defined $lastkey && $lastkey eq $key) {
            $lastkey = undef;
            $self->{lastclick} = [scalar @lines, scalar @$line];
        }
        if ($inverse) {
            unshift @$line, $width - $x - $w, $width - $x, $bl, $layout, $key;
        }
...
sub expose_cb {
    ...
    $window->draw_rectangle($bgc, ::TRUE, $event->area->values)
      ;    #clear the area with the base bg color
    ...
    {
        {
            ...
            $style->paint_focus(
                $window,
                (exists $self->{selected}{$key} ? $sstate : $state),
                undef,
                $self,
                undef,
                $x1 - XPAD(),
...
sub Fill {
    ...
    if ($width < 2 && !$self->{delayed}) {
        $self->{delayed} = 1;
        ::IdleDo('2_resizemosaic' . $self, 100, \&Fill, $self);
        return;
    }
    delete $self->{delayed};
    ...
    my $list = $self->{list};
    ($list) = $self->{get_fill_data_sub}($self)
      unless $samelist && $samelist eq 'samelist';

    my $filterlist = ::find_ancestor($self, 'FilterList')
      ;    #FIXME should get its options another way
    ...
    $self->{list} = $list;
    $self->set_size_request($self->{hsize} + 2 * XPAD,
        $self->{vsize} + 2 * YPAD);
...
#$style->paint_flat_box( $window,$state,'none',$event->area,$self,'',$event->area->values);
  ...
  #$window->draw_rectangle($style->text_gc($state),1,$x+20,$y+20,24,24); #DEBUG
            my $pixbuf =
              AAPicture::draw($window, $x, $y, $field, $key, $picsize);
            if ($pixbuf) { }
            elsif (defined $pixbuf) {    #warn "add idle\n" unless $self->{idle};
            ...
            {
                my $layout =
                  Gtk2::Pango::Layout->new($self->create_pango_context);
                $layout->set_markup(
                    AA::ReplaceFields($key, "<small>%a</small>", $field, 1));
                $layout->set_wrap('word-char');
                ...
                if ($free_height > 1) {
                    $yoffset = int($free_height / 2);
                }    #center vertically
                $layout->set_ellipsize('end');
                $layout->set_alignment('center');
                $style->paint_layout($window, $state, 1,
                    Gtk2::Gdk::Rectangle->new($x, $y, $hsize, $vsize),
                    $self, undef, $x, $y + $yoffset, $layout);
                next;
            }
            my ($xm, $ym, $align) =
              $self->{markup_pos} eq 'right'
              ? ($x + $picsize + XPAD, $y, 'left')
              : ($x, $y + $picsize + YPAD, 'center');
            my $i = 0;
            for my $markup (@markup) {
                my $layout =
                  Gtk2::Pango::Layout->new($self->create_pango_context);
                $layout->set_markup(
                    AA::ReplaceFields($key, $markup, $field, 1));
                ...
                $style->paint_layout(
                    ...
                    Gtk2::Gdk::Rectangle->new(
                        $xm, $ym, $markup_width, $height
                    ),
                    $self, undef, $xm, $ym, $layout
                );
                $ym += $height;
            }
        }
...
sub key_press_cb {
    ...
    my ($nw, $nh, $nwlast) = @{$self->{dim}};
    my $page = int($self->{vscroll}->get_adjustment->page_size
          / ($self->{vsize} + 2 * YPAD));
    my $i = $pos % $nw;
    my $j = int($pos / $nw);
    if    ($key eq 'Left')  { $i-- }
    ...
    {
        $self->{selected}{$_} = undef for @{$self->{list}};
        $self->queue_draw;
        $self->{selectsub}($self);
        return 1;
    }
...
sub new ##currently the returned widget must be put in ->{isearchbox} of a parent widget, and this parent must have the array to search in ->{array} and have the methods get_cursor_row and set_cursor_to_row. And also select_by_filter for SongList/SongTree
{
    ...
    unless ($type) {
        $self->{fields} = [split /\|/, ($opt->{isearchfields} || 'title')];
    }
...
sub changed {
    ...
    if (!$type)    #search song IDs
    {
        ...
        $self->{searchsub} =
          eval
          'sub { my $array=$_[0]; my $rows=$_[1]; for my $row (@$rows) { local $_=$array->[$row]; return $row if '
          . $code
          . '; } return undef; }';
    ...
    {
        my $code =
          Songs::Code($type, 'gid_to_display', GID => '$array->[$row]');
        $re = $self->{casesens} ? qr/$re/ : qr/$re/i;
        $self->{searchsub} =
          eval
          'sub { my $array=$_[0]; my $rows=$_[1]; for my $row (@$rows) { return $row if ::superlc('
          . $code
          . ')=~m/$re/; } return undef; }';
    }
    if ($@) {
        warn "Error compiling search code : $@\n";
        $self->{searchsub} = undef;
    }
    $self->search(0);
}
...
sub new {
    ...
    #use default options for this songlist type
    my $name = 'songtree_' . $opt->{name};
    $name =~ s/\d+$//;
    my $default = $::Options{"DefaultOptions_$name"} || {};

    %$opt = (@DefaultOptions, %$default, %$opt);
    $self->{$_} = $opt->{$_}
      for qw/headclick songxpad songypad no_typeahead grouping/;
...
#create widgets used to draw the songtree as a treeview, would be nice to do without but it's not possible currently
    ...
    $self->{stylewparent} = Gtk2::VBox->new;
    $self->{stylewparent}->add($self->{stylewidget})
      ; #some style engines (gtk-qt) call ->parent on the parent => Gtk-CRITICAL messages if stylewidget doesn't have a parent. And needs to hold a reference to it or bad things happen
    for my $i (1, 2, 3) {
        my $column = Gtk2::TreeViewColumn->new;
        my $label  = Gtk2::Label->new;
        $column->set_widget($label);
        ...
        $self->{'stylewidget_header' . $i} =
          $button;    #must be a button which has a treeview for parent
    ...
    $self->add($sw);
    $self->{headers} = SongTree::Headers->new($sw->get_hadjustment)
      unless $opt->{headers} eq 'off';
    ...
    $view->can_focus(::TRUE);
    $self->{DefaultFocus} = $view;
    $self->{$_}->signal_connect(
        value_changed => sub { $self->has_scrolled($_[1]) },
        $_
    ) for qw/hadj vadj/;
    $self->signal_connect(scroll_event    => \&scroll_event_cb);
    $self->signal_connect(key_press_event => \&key_press_cb);
    ...
    ::set_drag(
        ...
        source => [
            ...
            sub {
                my $view = $_[0];
                my $self = ::find_ancestor($view, __PACKAGE__);
                return ::DRAG_ID, $self->GetSelectedIDs;
            }
        ],
        dest   => [::DRAG_ID, ::DRAG_FILE, \&drag_received_cb],
        motion => \&drag_motion_cb,
    ...
    $self->{grouping} = ($self->{type} =~ m/[QLA]/ ? '' : 'album|pic')
      unless defined $self->{grouping};
...
sub update_columns {
    ...
    my $savedpos;
    my $songswidth = 0;
    if (my $ew = $self->{events_to_watch}) {
        ::UnWatch($self->{view}, $_) for keys %$ew;
    }
    delete $self->{events_to_watch};
    my $baseline  = 0;
    my $vsizesong = $self->{vsizesong};
    $vsizesong =
      GMB::Cell::init_songs($self, $self->{cells}, $self->{songxpad},
        $self->{songypad})
      if $self->{cols_changed};
    $self->{cols_changed} = undef;
    my %fields_to_watch;
    for my $cell (@{$self->{cells}}) {
        my $colid = $cell->{colid};
        $cell->{width} = $self->{colwidth}{$colid} || $STC{$colid}{width}
          unless exists $cell->{width};
        delete $cell->{last};
        if (my $watch = $cell->{event}) {
            $self->{events_to_watch}{$_} = undef for @$watch;
        }
        if (my $fields = $cell->{watchfields}) {
            $fields_to_watch{$_} = undef for @$fields;
        }
    ...
    if (  !$self->{vsizesong}
        || $self->{vsizesong} != $vsizesong)    #if height of rows has changed
    {
        $self->{vsizesong} = $vsizesong;

        #warn "new vsizesong : $vsizesong\n";
        $savedpos = $self->coord_to_path(0, int($self->{vadj}->page_size / 2))
          unless $nosavepos;
    ...
    $self->{fields_to_watch2} = [keys %fields_to_watch]
      ; #FIXME could call Songs::Depends only once now rather than multiple times for each column in GMB::Expression::parse ????
    if (my $ew = $self->{events_to_watch}) {
        ::Watch($self->{view}, $_, sub { $_[0]->queue_draw; }) for keys %$ew;
    }
...
#my $time=times;
    ...
    for my $depth (0 .. $maxdepth) {
        ...
        my $lastrows_parent =
          $depth == 0 ? [-1, $#$list] : $self->{TREE}{lastrows}[$depth - 1];
        my ($lastrows, $lastchild) =
          @$list
          ? Songs::GroupSub($col)->($list, $lastrows_parent)
          : ([-1], [0]);
...
sub SongArray_changed_cb {
    ...
    if ($action eq 'sort') {
        ...
        $$selected = '';
        vec($$selected, $_, 1) = 1 for @selected;
        ...
        'update') #should only happen when in filter mode, so no duplicates IDs
    {
        ...
        my @selected;
        $selected[$oldarray->[$_]] = vec($$selected, $_, 1)
          for 0 .. $#$oldarray;
        $$selected = '';
        vec($$selected, $_, 1) = 1
          for grep $selected[$songarray->[$_]], 0 .. $#$songarray;

        #translate expstate to new order :
        my @newexp;
        for my $string (@{$self->buildexpstate}) {
            my @exp;
            $exp[$oldarray->[$_]] = substr($string, $_, 1)
              for 0 .. $#$oldarray;
            my $new = '';
        ...
        my ($destrow, $IDs) = @extra;
        vec($$selected, $#$songarray, 1)
          ||= 0;    #make sure $$selected has a value for every row
        ...
        $_ >= $destrow and $_ += @$IDs
          for $self->{lastclick}, $self->{startgrow};
    }
    elsif ($action eq 'move') {
        my (undef, $rows, $destrow) = @extra;
        ...
        for my $exp (@{$self->buildexpstate}) {
            (substr($exp, $_ - 1, 1), substr($exp, $_, 1)) =
              (substr($exp, $_, 1), substr($exp, $_ - 1, 1))
              for @$rows;
        }
    }
    elsif ($action eq 'down') {
        ...
        if (@$songarray) {
            my $rows = $extra[0];
            vec($$selected, @$rows + $#$songarray, 1)
              ||= 0
              ; #make sure $$selected has a value for every row, unlike $songarray $selected is not yet updated, so its last_row= @$rows+$#$songarray
            my $string = unpack 'b*', $$selected;
            for my $s ($string, @{$self->buildexpstate}) {
    ...
    if ($center) {
          ...
          )    #scroll to last selected ID if in the list
        {
            ...
            }    # scroll to row but keep selection
        }
...
sub key_press_cb {
    ...
    {
        vec($self->{selected}, $_, 1) = 1 for 0 .. $#$list;
        $self->UpdateSelection;
    }
    elsif (lc $key eq 'f' && $ctrl) {
        $self->{isearchbox}->begin();
    }                                  #ctrl-f : search
    elsif (lc $key eq 'g' && $ctrl) {
        $self->{isearchbox}->search($shift ? -1 : 1);
    ...
    elsif (!$self->{no_typeahead}
        && $unicode
        && $unicode != 32
        && !$mod)                      # character except space, no modifier
    {
        $self->{isearchbox}->begin(chr $unicode);    #begin typeahead search
    }
    else { return 0 }
...
sub expose_cb {
    ...
    my $style =
      Gtk2::Rc->get_style_by_paths($self->{stylewidget}->get_settings,
        '.GtkTreeView', '.GtkTreeView', 'Gtk2::TreeView')
    ...
    $self->{stylewidget}->has_focus($view->has_focus)
      ;    #themes engine check if the widget has focus
    my $selected  = \$self->{selected};
    my $list      = $self->{array};
        ...
        if ($yend > $exp_y1) {
            ...
            if ($expanded) {
                ...
                else    #songs
                {
                    ...
                    if ($y + $h > $exp_y1) {
                        my $skip = 0;
                        $last -= int(-.5 + ($y + $h - $exp_y2) / $vsizesong)
                          if $y + $h > $exp_y2;
                        if ($y < $exp_y1) {
                        ...
                        for my $row ($first .. $last) {
                            my $ID = $list->[$row];
                            my $state =
                              vec($$selected, $row, 1) ? $sstate : $nstate;
                            my $detail =
                              $odd ? 'cell_odd_ruled' : 'cell_even_ruled';

#detail can have these suffixes (in order) : _ruled _sorted _start|_last|_middle
                            $style->paint_flat_box(
                                $window,              $state,
                                'none',               $expose,
                                $self->{stylewidget}, $detail,
                                $songs_x,             $y,
                                $songs_width,         $vsizesong
                            );
                            my $x = $songs_x;
                            for my $cell (@$songcells) {
                                ...
                                if ($clip) {
                                    if ($cell->{sorted}
                                      ) # if column is sorted, redraw background with '_sorted' hint
                                    {
                                        $style->paint_flat_box(
                                            $window,
                                            $state,
...
sub drag_motion_cb {
    ...
    elsif ($y + $self->{vsizesong} >= $self->{viewwindowsize}[1]) {
        $view->{scroll} = 'down';
    }
    else { delete $view->{context}; delete $view->{scroll} }
    if ($view->{scroll}) {
        $view->{scrolling}
          ||= Glib::Timeout->add(200, \&drag_scrolling_cb, $view);
    ...
    $self->update_row($view->{drag_highlight})
      if defined $view->{drag_highlight};
    $view->{drag_highlight} = $row;
    $self->update_row($row);
...
sub button_press_cb {
    ...
    else                                              # ($but==1)
    {
        ...
        if (defined $depth && $answer->{area} eq 'head'
            || $answer->{area} eq 'collapsed')
        {
...
our %GraphElem = (
    ...
    picture => {
        functions => [
            [   'pic_cached',
                'cached queue = file resize? w? h? xpad ypad crop hide'
            ],
        ...
        defaults =>
          'x=0,y=0,xalign=.5,yalign=.5,resize=0,w=0,h=0,crop=0,xpad=xpad,ypad=ypad,w=___wr+2*___xpad,h=___hr+2*___ypad',
        optional => [
            $optpad,
        ],
    },
    icon => {
        ...
        defaults =>
          'w=___wr+2*___xpad,h=$_h,xpad=xpad,ypad=ypad,xalign=0,yalign=.5,size=\'menu\'',
        optional => [
            $optpad,
        ],
    },
...
#		  },
    ...
    blalign => {
        functions => [['blalign', 'h = y ref', 'y = blp h'],],
        defaults  => 'y=0,ref=0',
    },
    xalign => {
        functions => [['align', 'w = align x ref', 'x = w'],],
        defaults  => 'ref=___align',
    },
    yalign => {
        functions => [['align', 'h = align y ref', 'y = h'],],
        defaults  => 'ref=___align',
    },
    xpack => {
        functions => [['epack', 'w = x pad', 'x = w'],],
        defaults  => 'ref=0,pad=0',
    },
    ypack => {
        functions => [['epack', 'h = y pad', 'y = h'],],
        defaults  => 'ref=0,pad=0',
    },
);
...
sub init_songs {
    ...
    for my $cell (@$cells) {
        ...
        m/^(\w+:)init_(\w+)$/
          and ($dep->{$_}, $dep->{$1 . $2}) = ($dep->{$1 . $2}, $dep->{$_})
          for keys %$dep;    #exchange init_* keys with normal keys
        push @Deps, $dep;
        if ($bl) {
            my @init;
    ...
    if (@blh) {
        my ($h, @y) = blalign(undef, 0, 0, @blh)
          ;    #compute the y of elements aligned with songbl
    ...
    for my $cell (@$cells) {
        ...
        m/^(\w+:)init_(\w+)$/ and $dep->{$1 . $2} = $dep->{$_}
          for keys %$dep;                               #revert init_ keys
...
sub new_group {
    ...
    if (my $ref0 = $SongTree::GroupSkin{$skin}{options}) {
        for my $key (keys %$ref0) {
            $constant->{$key} = "'" . $ref0->{$key}{default} . "'"
              unless exists $constant->{$key};
        }
    }
    ...
    my @init = map $_ . '=' . ($def->{$_} || 0),
      qw/head tail left right vmin vcollapse/;
...
sub createdep {
    ...
    for my $elem (@$elems) {
        ...
        if ($elem && $GraphElem{$elem} && $GraphElem{$elem}{defaults}) {
            ...
            GMB::Expression::parse($default, $context, \%update, \%default,
                $constant);
        }
    }
...
our %vars2 = (
    song => {    #ufile #REMOVED PHASE1 fix the doc
        ...
        progress => [
            '$arg->{ID}==$::SongID ? $::PlayTime/Songs::Get($arg->{ID},"length") : 0',
            'length',
            'CurSong Time'
        ],
        queued => [
            'do {my $i;my $f;for (@$::Queue) {$i++; $f=$i,last if $arg->{ID}==$_};$f}',
            undef,
            'Queue'
        ],
        playing  => ['$arg->{ID}==$::SongID', undef, 'CurSong'],
        playicon => [
            '::Get_PPSQ_Icon($arg->{ID},!$arg->{currentsong})', undef,
            'Playing Queue CurSong'
        ],
        labelicons =>
          ['[Songs::Get_icon_list("label",$arg->{ID})]', 'label', 'Icons'],
        ids => ['$arg->{ID}'],
    },
    group => {
        ids    => ['$arg->{groupsongs}'],
        ...
        album_artist =>
          ['groupartist("album_artist",$arg->{groupsongs})', 'album_artist'],
        album_artistid =>
          ['groupartistid("album_artist",$arg->{groupsongs})', 'album_artist'],
        ...
        gid      => ['Songs::Get_gid($arg->{groupsongs}[0],$arg->{grouptype})']
        ,    #FIXME PHASE1
        title => [
            '($arg->{groupsongs} ? Songs::Get_grouptitle($arg->{grouptype},$arg->{groupsongs}) : "")'
          ]
        , #FIXME should the init case ($arg->{groupsongs}==undef) be treated here ?
        rating_avrg => [
            'do {my $sum; $sum+= $_ for Songs::Map(ratingnumber=>$arg->{groupsongs}); $sum/@{$arg->{groupsongs}}; }',
            'rating'
        ],    #FIXME round, int ?
        'length' => [
            'do {my (undef,$v)=Songs::ListLength($arg->{groupsongs}); sprintf "%d:%02d",$v/60,$v%60;}',
            'length'
        ],
...
sub parse {
    ...
    while ((my $key, local $_) = each %$hash) {
        my (@f_end, @pcount, $pcount, %var1used);
        my $r     = '';
        my $depth = 0;
        while (1) {
            ...
            if (   m#\G\s*([!=<>]=|[-+.,%*/<>]|&&|\|\|)#gc
                || m#\G\s*((?:x|eq|lt|gt|cmp|le|ge|ne|or|xor|and)\s)#gc)
            {
                $r .= $1;
            }
            else {last}
...
sub MakeMake {
    ...
    if ($inqueue) {
        $sub
          .= 'while (my $if=shift @queuedif) { if ($var{$if}) {last} else {my $sub=shift @queued; &$sub} }';
    ...
    {    #warn "\ntodo :",(join ',',keys %todo),"\n";
        ...
        {    #warn "key=$key -- $dep->{$key} --- $dep0->{$key}\n";
            ...
            for (@deps) {
                ...
                next
                  if exists $done->{$d}
                  || !exists $dep->{$d} && ($opt || !exists $dep0->{$d});

                #warn " -> todo $d\n";
                $todo{$d} = undef;
                $notnow = 1;
            }
            unless ($notnow) {    #warn "$key ---found in ($code,@deps)\n";
                if (ref $code) {
