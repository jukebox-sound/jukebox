...
BEGIN {
	...
	# work-around to keep the register from being finalized in
	# gstreamer < 0.10.4
	# http://bugzilla.gnome.org/show_bug.cgi?id=324818
	$Play_GST::reg_keep = $reg if GStreamer->CHECK_VERSION(0, 10, 4);

	$reg->lookup_feature('playbin')
		or die "gstreamer plugin 'playbin' not found.\nYou need to install at least gst-plugins-base.\n";
}
...
BEGIN {
	...
	if ($reg->lookup_feature('equalizer-10bands')) {
		...
		warn "gstreamer plugin 'equalizer-10bands' not found -> equalizer not available\n";
	}

	if ($reg->lookup_feature('rglimiter') && $reg->lookup_feature('rgvolume')) {
		$GST_RG_ok = 1;
	} else {
		warn "gstreamer plugins 'rglimiter' and/or 'rgvolume' not found -> replaygain not available\n";
	}

	if ($reg->lookup_feature('rganalysis')) {
		$GST_RGA_ok = 1;
	} else {
		warn "gstreamer plugins 'rganalysis' not found -> replaygain analysis not available\n";
	}

	$GST_visuals_ok = 1;
	eval { require GStreamer::Interfaces };
	if ($@) {
		warn "GStreamer::Interfaces perl module not found -> visuals not available\n";
		$GST_visuals_ok = 0;
	}
...
sub supported_sinks {
	...
	#$::Options{gst_sink} = 'autoaudio'
	#	unless $Sinks{$::Options{gst_sink}};
...
sub init {
	...
	# find a default sink
	$::Options{gst_sink} ||= (grep ($reg->lookup_feature($_ . 'sink'), qw/autoaudio gconfaudio pulse alsa esd oss oss4/), 'autoaudio')[0];
...
sub createPlayBin {
	...
	# not stable with older version of perl-glib due to bug #620099
	# (https://bugzilla.gnome.org/show_bug.cgi?id=620099), and still not quite stable
	$PlayBin->signal_connect(
		"notify::volume" => sub {
			Glib::Idle->add(\&VolumeChanged) unless $VolumeHasChanged++;
		},
		100000
	) if $Glib::VERSION >= 1.251 && $::Options{gst_monitor_pa_volume};
...
sub bus_message_state_changed {
	...
	Glib::Idle->add(
		sub {
			# this will only skip if the state is right, else will wait for
...
sub Play {
	...
	#warn "------play $file\n";
	#$PlayBin->set_state('ready');
	#&Stop;
	#my ($ext) = $file =~ m/\.([^.]*)$/;
	#warn $ext;
	#::ErrorPlay('not supported') and return undef
	#	unless $Plugins{$ext};
	...
	if ($keep) {
		...
		if ($useRG) {
			my ($rgv, $rgl, $ac, $ar) = map GStreamer::ElementFactory->make($_ => $_), qw/rgvolume rglimiter audioconvert audioresample/;
		...
		if (my $custom = $::Options{gst_custom}) {
			...
			if ($elem && $elem->isa('GStreamer::Bin')) {
				...
				# will work at least for simple cases
...
sub EQ_Get_Range {
	...
	{
		my $equalizer = $PlayBin->get_by_name('equalizer') || GStreamer::ElementFactory->make('equalizer-10bands' => 'equalizer');
...
sub EQ_Get_Hz {
	...
	if ($hz =~ m/^(\d+)\s*(k?)Hz/) {
		$hz = $1;
		$hz *= 1000 if $2;
		$hz = $hz >= 1000  ?  sprintf '%.1fkHz', $hz / 1000  :  sprintf '%dHz', $hz;
	}
...
sub Analyse_full {
	...
	# get songs with an album name
	my $IDs_in_album = Filter->new('album:-e:')->filter;
	...
	for my $aid (keys %$again) {
		# ignore albums with less than 2 songs
		next if @{$ids->{$aid}} < 2;
...
sub Analyse {
	...
	unless ($RGA_pipeline) {
		...
		my ($src, $decodebin, $ac, $ar, $rganalysis, $fakesink) = map GStreamer::ElementFactory->make($_ => $_), qw/filesrc decodebin audioconvert audioresample rganalysis fakesink/;
...
sub bus_message_tag {
	...
	#	warn "key=$key => $tags->{$key}\n"
	#}
	# FIXME should check if the message comes from the rganalysis element,
	# but not supported by the bindings yet, instead check if any non
	# replaygain tags => will re-write replaygain tags _before_ analysis
	# for files without other tags
	# return if GStreamer->VERSION >=.10 && $msg->src ==
	# $RGA_pipeline->get_by_name('rganalysis'); FIXME with Gstreamer 0.10 :
	# $message->src should work, not tested yet !!!! TESTME
...
# was in Play()
# for (keys %sockets) {
#	warn "socket $_ : " . $sockets{$_}[1];
#	$stream->signal_emit(add => $_ ) if $sockets{$_}[1];
# }
...
sub make_sink {
	...
	(my ($aconv, $audioresamp, $lame), $stream) = GStreamer::ElementFactory->make(
	...
	#	},
	#	$_
	#) for 'client-removed', 'client_added', 'client-fd-removed';
	#
	#$stream->signal_connect(
	...
	#			sub {
	#				warn $_[0];
	#				warn "-- $_ ".$sockets{$_} for keys %sockets;
	#
	#				unless ($sockets{$_[0]}[1]) {
	#					close $sockets{$_[0]}[0];
	#
	#					warn "closing $_[0]";
	#
...
sub Connection {
	...
	if ($request =~ m#^GET /command\?cmd=(.*?) HTTP/1\.\d\015\012$#) {
		...
		# FIXME add password and disable dangerous commands (RunSysCmd,
		# RunPerlCode and ChangeDisplay)
		if (0) {
