...
INIT {
	...
	%Types = (
		generic => {
			...
			'filter:mi'      => '::superlc(#display#) .=~. m"#VAL#"i',

			'filter_prep:mi' => sub { Filter::QuoteRegEx(::superlc($_[0])) },

			'filter:si'      => 'index( ::superlc(#display#),"#VAL#") .!=. -1',

			'filter_prep:si' => sub { quotemeta ::superlc($_[0]) },

			'filter:s'       => 'index(    #display#, "#VAL#") .!=. -1',

			'filter_prep:s'  => sub { quotemeta $_[0] },

			'filter:fuzzy' => '.!!. Filter::_fuzzy_match(#VAL1#/100,"#VAL2#",lc(#get#))',

			'filter_prep:fuzzy' => sub {
				my @arg = split / /, $_[0], 2;

				$arg[0], quotemeta lc($arg[1]);
			},
		...
		flags   => {
			...
			set => '{
				...
				my $val = @ids < 2 ? $ids[0]||0 : (___group{join(" ",map sprintf("%x",$_),@ids)}||= \@ids);
				___isort{ sprintf("%x",$val) }||= ::superlc( ___sort{ sprintf("%x",$val) }||= join ";",@list );
				#_# = $val;
			}',

			# #VAL# should be a sorted arrayref, as returned by #check#
			diff => 'do {my $v=#_#; my $old=!$v ? "" : ref $v ? join "\\x00",map ___name[$_],@$v : ___name[$v]; $v=#VAL#; my $new= join "\\x00", @$v; $old ne $new; }',
			...
			'filterdesc:si' => ["contains %s", "contains", 'substring', icase => 1,],
			...
			'filterdesc:fuzzy' => ["%s fuzzy match with %s", "fuzzy match", 'fuzzy string',],
		...
		artists => {
			...
			update         => 'my @ids;
				...
				#_# =
					...
					# 1 for @ids==0 is the special gid for unknown artists defined in artist's init
			...
			#FIXME use simpler/faster version if perl5.10 (with ~~)
			'filter:~' => '(ref #_# ?  (.!!. grep $_ == #VAL#, @{#_#}) : (#_# .==. #VAL#))',
			...
			'filterdesc:~' => ["includes artist %s", "includes artist", 'menustring',],
		...
		artist => {
			...
			init => '____=""; __#mainfield#_gid{""}=1; #_iname#[1]=::superlc( #_name#[1]="<Unknown>" );',
		...
		album => {
			...
			init => '____=""; __#mainfield#_gid{"\\x00"}=1; __#mainfield#_empty=""; vec(__#mainfield#_empty,1,1)=1; __#mainfield#_sgid[1]="\\x00"; #_iname#[1]=::superlc( #_name#[1]="<Unknown>" );',

			findgid => 'do{
				my $name=#VAL#;
				my $sgid= $name ."\\x00". (
					$name eq "" ? "artist=".#artist->get# : do {
						my $a = #album_artist_raw->get#;
						$a ne "" ? "album_artist=$a" : #compilation->get# ? "compilation=1" : ""
					}
				);
				__#mainfield#_gid{$sgid}||= do {
					my $n = @#_name#;
					if ($name eq "") {
						vec(__#mainfield#_empty, $n, 1) = 1;
						$name = #unknown#.#artist->get#;
					}
					push @#_name#, $name;
					push @__#mainfield#_sgid, $sgid;
					#newval#;
					$n;
				};
			};',
			...
			set => 'my $oldgid=#_#; my $newgid= #_#= #findgid#; if ($newgid+1==@#_name# && $newgid!=$oldgid) { ___picture[$newgid]= ___picture[$oldgid]; }',
			#same as load, but if gid changed and is new, use picture from old gid
		...
		istring => {
			# _much_ faster with case/accent insensitive operations, at the price of double memory
			...
			'filter:si'      => 'index( #_iname#,"#VAL#") .!=. -1',

			'filter_prep:si' => sub { quotemeta ::superlc($_[0]) },

			'filter:mi'      => '#_iname# .=~. m"#VAL#"i',

			'filter_prep:mi' => sub { Filter::QuoteRegEx(::superlc($_[0])) },

			'filter:fuzzy' => ' .!!. Filter::_fuzzy_match(#VAL1#/100,"#VAL2#",#_iname#)',
...
#		},
		...
		fewstring => {
			# for strings likely to be repeated
			...
			'filter:fuzzy' => '.!!. Filter::_fuzzy_match(#VAL1#/100,"#VAL2#",#_iname#[#_#])',
			...
			'filter:si'      => 'index( #_iname#[#_#],"#VAL#") .!=. -1',
			...
			'filter_prep:~'  => '#sgid_to_gid(VAL=#PAT#)#',
		...
		number => {
			...
			'filterdesc:b' => ["between %s and %s", "between", 'value value'],

			'filterdesc:-b' => "not between %s and %s",

			'filterdesc:-e' => "≠ %s",

			'filterdesc:h'  => ["in the top %s", "in the top", 'number',],    # "the %s most"  "the most",  ?

			'filterdesc:t' => ["in the bottom %s", "in the bottom", 'number',],    # "the %s least" "the least", ?
		...
		'number.div' => {
			...
			#FIXME decimal separator must be always "."
			makefilter => 'Filter->newadd(1, "#field#:-<:".(#GID# * #ARG0#), "#field#:<:".((#GID#+1) * #ARG0#) )',
		...
		integer => {
			...
			'editwidget:all' => sub {
				my $field = $_[0];
				GMB::TagEdit::EntryNumber->new(
					@_,
					min    => $Def{$field}{edit_min},
					max    => $Def{$field}{edit_max},
					digits => 0,
					mode   => $Def{$field}{edit_mode}
				);
			},
		...
		float => {
			# make sure the string doesn't have the utf8 flag, else substr won't work
			...
			diff => (
				$nan == $nan
					? 'do {my $new=#VAL#; $new=#nan# unless length $new; $new!=#_# }'
		...
		'length' => {
			...
			'filter_prep:b' => sub {
				sort { $a <=> $b } map ::ConvertTimeLength($_), split / /, $_[0], 2;
			},
		},

		'length.div' => {
			gid_to_display => 'my $v=#GID# * #ARG0#; sprintf("%d:%02d", $v/60, $v%60);',
		},

		size => {
			display => '( ::format_number( #_#/' . ::MB() . ',"%.1f").q( ' . "MB" . ') )',
			...
			'filter_prep:b' => sub {
				sort { $a <=> $b } map ::ConvertSize($_), split / /, $_[0], 2;
			},
		...
		'size.div' => {
			gid_to_display => '( ::format_number( #GID# * #ARG0#/' . ::MB() . ',"%d").q( '  . "MB" . ') )',
		},
		...
		date => {
			...
			'filter_prep:bago' => sub {
				sort { $a <=> $b } map ::ConvertTime($_), split / /, $_[0], 2;
			},
			...
			'filterpat:date' => [
				display => sub {
					my $var = shift;
					$var = ::strftime_utf8('%c', localtime $var) if $var =~ m/^\d+$/;
					$var;
				},
			],
	    ...
	    'date.year' => {
	        ...
	        makefilter =>
	          '"#field#:".(!#GID# ? "e:0" : "b:".#GID#." ".(::mktime(0,0,0,1,0,(localtime(#GID#))[5]+1)-1))',
	    },
	    'date.month' => {
	        ...
	        makefilter =>
	          '"#field#:".(!#GID# ? "e:0" : "b:".#GID#." ".do{my ($m,$y)= (localtime(#GID#))[4,5]; ::mktime(0,0,0,1,$m+1,$y)-1})',
	    },
	    'date.day' => {
	        ...
	        makefilter =>
	          '"#field#:".(!#GID# ? "e:0" : "b:".#GID#." ".do{my ($d,$m,$y)= (localtime(#GID#))[3,4,5]; ::mktime(0,0,0,$d+1,$m,$y)-1})',
	    },
	   ...
	   # ___free_ : array containing free positions in ___values_ for each size
	      { parent => 'dates',
	        ...
	        , # => max 2**32 songs that share the same number of dates, could maybe use 16 bits instead
	        nb => 'vec(___nb_,#ID#,16)'
	        ,    # => max 2**16 dates per song, could maybe use 8 bits instead
	    ...
	    dates => {
	        ...
	        default    => 'undef',
	        bits       => 32,
	        packformat => 'L',         # replace with 64 and Q for 64bits dates
	        bytes      => '(#bits#/8)',
	        check      => ';',
	        get_list => 'unpack("#packformat#*",#_#||"")',
	        display =>
...
#n_sort		=> 'unpack("#packformat#*",substr(#_#||"",-#bytes#))', #sort by last date, not used
	        ...
	        diff =>
	          'do {my $old=#_#||""; my $new=#VAL#; $new= pack "#packformat#*",sort { $a <=> $b } (ref $new ? @$new : split /\D+/,$new); $old ne $new; }',
	        check_multi =>
	          'for my $lref (@{#VAL#}) {@$lref=grep m/^\d+$/, @$lref}',
	        set_multi =>
	          'do {my %h; $h{$_}=0 for #get_list#; my ($toadd,$torm,$toggle)=@{#VAL#}; $h{$_}= (exists $h{$_} ? -1 : 1) for @$toggle; $h{$_}++ for @$toadd; $h{$_}-- for @$torm; (scalar grep $h{$_}!=0, keys %h) ? [grep $h{$_}>=0, keys %h] : undef; }',
...
#example of use : Songs::BuildHash('artist',$::Library,undef,'playhistory:countrange:DATE1-DATE2'));  where DATE1 and DATE2 are secongs since epoch and DATE1<DATE2
	        'stats:countrange' =>
	          'INIT: my ($$date1,$$date2)= #ARG#=~m/(\d+)/g; ---- #HVAL# ++ for grep $$date1<$_ && $$date2>$_, #get_list#;'
	        ,    #count plays between 2 dates (in seconds since epoch)
	        'stats:countafter' => '#HVAL# ++ for grep #ARG#<$_, #get_list#;'
	        ,    #count plays after date (in seconds since epoch)
	        'stats:countbefore' => '#HVAL# ++ for grep #ARG#>$_, #get_list#;'
	        ,    #count plays before date (in seconds since epoch)
	        ...
	        hashm            => '(#_# ? (map #mktime#,#get_list#) : 0)'
	        ,    #or use post-hash modification for 0 case
	        subtypes_menu => \%timespan_menu,
...
#identical to date.*, except #_# is replaced by $_ in mktime, and "e" filter by "ecount"
	    'dates.year' => {
	        ...
	        gid_to_display =>
	          '(#GID# ? ::strftime_utf8("%Y",localtime(#GID#)) : _"never")',
	        makefilter =>
	          '"#field#:".(!#GID# ? "ecount:0" : "b:".#GID#." ".(::mktime(0,0,0,1,0,(localtime(#GID#))[5]+1)-1))',
	    },
	    'dates.month' => {
	        mktime => '::mktime(0,0,0,1,(localtime($_))[4,5])',
	        gid_to_display =>
	          '(#GID# ? ::strftime_utf8("%b %Y",localtime(#GID#)) : _"never")',
	        makefilter =>
	          '"#field#:".(!#GID# ? "ecount:0" : "b:".#GID#." ".do{my ($m,$y)= (localtime(#GID#))[4,5]; ::mktime(0,0,0,1,$m+1,$y)-1})',
	    },
	    'dates.day' => {
	        mktime => '::mktime(0,0,0,(localtime($_))[3,4,5])',
	        gid_to_display =>
	          '(#GID# ? ::strftime_utf8("%x",localtime(#GID#)) : _"never")',
	        makefilter =>
	          '"#field#:".(!#GID# ? "ecount:0" : "b:".#GID#." ".do{my ($d,$m,$y)= (localtime(#GID#))[3,4,5]; ::mktime(0,0,0,$d+1,$m,$y)-1})',
	    },
	    boolean => {
	        parent  => 'integer',
	        bits    => 1,
	...
	%Def
	  ...
	  ( file => {
	    ...
	    id => {
	        ...
	        'stats:uniq' =>
	          '#HVAL#=undef', #doesn't really belong here, but simpler this way
	        'stats:count' => '#HVAL#++',
	    },
	    path => {
	        name                 => "Folder",
	    ...
	    title => {
	        ...
	        letter            => 't',
	        category          => 'basic',
	        alias_trans       => ::_p('Field_aliases', "title")
	        , #TRANSLATION: comma-separated list of field aliases for title, these are in addition to english aliases
	        articles => 1,
	    },
	    artist => {
	        name          => "Artist",
	        ...
	        category    => 'basic',
	        alias       => 'by',
	        alias_trans => ::_p('Field_aliases', "artist,by")
	        , #TRANSLATION: comma-separated list of field aliases for artist, these are in addition to english aliases
	        articles => 1,
	    },
	    first_artist => {
	        flags         => 'fig',
	    ...
	    album => {
	        ...
	        category      => 'basic',
	        alias         => 'on',
	        alias_trans   => ::_p('Field_aliases', "album,on")
	        , #TRANSLATION: comma-separated list of field aliases for album, these are in addition to english aliases
	        articles => 1,
	    },
...
# },
	    ...
	    album_artist_raw => {
	        ...
	        type   => 'artist',
	        id3v2  => 'TPE2',
	        vorbis => 'albumartist|album_artist',
	        ape    => 'Album Artist|Album_artist',
	        ilst   => "aART",
	    ...
	    album_artist => {
	        ...
	        picture_field => 'artist_picture',
	        _ =>
	          'do {my $n=vec(__album_artist_raw__,#ID#,#bits#); $n==1 ? vec(__artist__,#ID#,#bits#) : $n}',
	        can_group => 1,
	        letter    => 'A',
	    ...
	    album_has_picture => {
	        ...
	        flags     => 'fgcs',
	        type      => 'boolean',
	        _         => '!!(__#mainfield#_picture[ ##mainfield#->get_gid# ])',
	        mainfield => 'album',
	    },
	    artist_has_picture => {
	    ...
	    has_lyrics => {
	        ...
	        flags     => 'fgarscp',
	        type      => 'boolean',
	        id3v2     => 'TXXX;FMPS_Lyrics;%v | USLT;;;%v',
	        vorbis    => 'FMPS_LYRICS|lyrics',
	        ape       => 'FMPS_LYRICS|Lyrics',
	        'ilst'    => "----FMPS_Lyrics|\xA9lyr",
	        lyrics3v2 => 'LYR',
	        category  => 'extra',
	        disable   => 1,
	        options   => 'disable',
	    ...
	    year => {
	        ...
	        'id3v2.4' => 'TDRC|TYER',
	        vorbis    => 'date|year',
	        ape       => 'Record Date|Year',
	        ilst      => "\xA9day",
	        prewrite       => sub { $_[0] ? $_[0] : undef },   #remove tag if 0
	        gid_to_display => '#GID# ? #GID# : _"None"',
	        'stats:range' =>
	          '#HVAL#{#_#}=undef;  ---- AFTER: delete #HVAL#{0}; #HVAL#=do {my ($m0,$m1)=(sort {$a <=> $b} keys %{#HVAL#})[0,-1]; !defined $m0 ? "" : $m0==$m1 ? $m0 : "$m0 - $m1"}',
	        editwidth   => 6,
	        edit_order  => 50,
	        edit_many   => 1,
	        letter      => 'y',
...
#is_set	=> '(__LABEL__=~m/(?:^|\x00)__QVAL__(?:$|\x00)/)? 1 : 0', #for random mode
	    ...
	    comment => {
	        ...
	        id3v1      => 4,
	        id3v2      => 'COMM;;;%v',
	        vorbis     => 'description|comment|comments',
	        ape        => 'Comment',
	        lyrics3v2  => 'INF',
	        ilst       => "\xA9cmt",
	        join_with  => "\n",
	    ...
	    rating => {
	        ...
	        type  => 'rating',
	        id3v2 =>
	          'TXXX;FMPS_Rating_User;%v::%i & TXXX;FMPS_Rating;%v | percent( TXXX;gmbrating;%v ) | five( TXXX;rating;%v )',
	        vorbis =>
	          'FMPS_RATING_USER::%i & FMPS_RATING | percent( gmbrating ) | five( rating )',
	        ape =>
	          'FMPS_RATING_USER::%i & FMPS_RATING | percent( gmbrating ) | five( rating )',
	        ilst =>
	          '----FMPS_Rating_User::%i & ----FMPS_Rating | percent( ----gmbrating ) | five( ----rating )',
	        ...
	        'postread:five' => sub {
	            ...
	            length $v
	              && $v =~ m/^\d+$/
	              && $v <= 5 ? sprintf('%d', $v * 20) : undef;
	        },    # for reading foobar2000 rating 0..5 ?
	        'postread:percent' => sub { $_[0] }
	        ,     # for anyone who used gmbrating
	        FilterList  => {},
	        starprefix  => 'stars',
	        edit_order  => 90,
	        edit_many   => 1,
	        ...
	        'filterpat:value' =>
	          [round => "%d", unit => '%', max => 100, default_value => 50,],
	        category => 'basic',
	        alias    => 'stars',
	    },
	    ...
	    playcount => {
	        ...
	        postread =>
	          sub { my $v = shift; length $v ? sprintf('%d', $v) : undef },
	    ...
	    bitrate => {
	        ...
	        type       => 'integer',
	        bits       => 16,
	        audioinfo  => 'bitrate|bitrate_nominal',
	        check      => '#VAL#= sprintf "%.0f",#VAL#/1000;',
	        display    => '::replace_fnumber("%d kbps",#_#)',
	        FilterList => {type => 'div.32',},
	        'filterpat:value' =>
	          [round => "%d", unit => 'kbps', default_value => 192],
	        category => 'audio',
	    },
	    samprate => {
	        name                 => "Sampling Rate",
	    ...
	    'length' => {
	        ...
	        flags     => 'fgarscp_',
	        type      => 'length',
	        bits      => 16,           # 16 bits limit length to ~18.2 hours
	        audioinfo => 'seconds',
	        check      => '#VAL#= sprintf "%.0f",#VAL#;',
	        FilterList => {type => 'div.60',},
	        'filterpat:value' =>
	          [unit => \%::TIMEUNITS, default_unit => 's', default_value => 1],
	        letter   => 'm',
	        category => 'audio',
	    },
	    ...
	    replaygain_track_gain => {
	        ...
	        type  => 'float',
	        check =>
	          '#VAL#= do{ #VAL# =~m/^([-+]?\d*\.?\d+)\s*(?:dB)?$/i ? $1 : #novalue#};',
	        displayformat => '%.2f dB',
	        id3v2         => 'TXXX;replaygain_track_gain;%v',
	        vorbis        => 'replaygain_track_gain',
	        ape           => 'replaygain_track_gain',
	        ilst          => '----replaygain_track_gain',
	        prewrite      => sub {
	            length($_[0])
	              && $_[0] == $_[0] ? sprintf("%.2f dB", $_[0]) : undef;
	        },    #remove tag if empty string or NaN
	    ...
	    replaygain_track_peak => {
	        ...
	        ilst     => '----replaygain_track_peak',
	        prewrite => sub {
	            length($_[0])
	              && $_[0] == $_[0] ? sprintf("%.6f", $_[0]) : undef;
	        },    #remove tag if empty string or NaN
	        type       => 'float',
	        options    => 'disable',
	        category   => 'replaygain',
	    ...
	    replaygain_album_gain => {
	        ...
	        check =>
	          '#VAL#= do{ #VAL# =~m/^([-+]?\d*\.?\d+)\s*(?:dB)?$/i ? $1 : #novalue#};',
	        displayformat => '%.2f dB',
	        id3v2         => 'TXXX;replaygain_album_gain;%v',
	        vorbis        => 'replaygain_album_gain',
	        ape           => 'replaygain_album_gain',
	        ilst          => '----replaygain_album_gain',
	        prewrite      => sub {
	            length($_[0])
	              && $_[0] == $_[0] ? sprintf("%.2f dB", $_[0]) : undef;
	        },    #remove tag if empty string or NaN
	    ...
	    replaygain_album_peak => {
	        ...
	        ape      => 'replaygain_album_peak',
	        ilst     => '----replaygain_album_peak',
	        prewrite => sub {
	            length($_[0])
	              && $_[0] == $_[0] ? sprintf("%.6f", $_[0]) : undef;
	        },    #remove tag if empty string or NaN
	    ...
	    replaygain_reference_level => {
	        flags => 'w',
	        type  => 'writeonly',    #only used for writing
	        id3v2    => 'TXXX;replaygain_reference_level;%v',
	        vorbis   => 'replaygain_reference_level',
	        ape      => 'replaygain_reference_level',
	        ilst     => '----replaygain_reference_level',
	        category => 'replaygain',
	    },
	    ...
	    playedlength => {
	        ...
	        type   => 'length',
	        flags  => 'g',
	        get    => '#playcount->get# * #length->get#',
	        _      => '#get#',
	        depend => 'playcount length',
	    },
	    version_or_empty => {
	        get    => 'do {my $v=#version->get#; $v eq "" ? "" : " ($v)"}',
	        type   => 'virtual',
	        depend => 'version',
	        flags  => 'g',
	        letter => 'V',
	    ...
	    album_years => {
	        ...
	        flags  => 'g',
	        letter => 'Y',
	    },    #depends on years from other songs too
	    uri => {
	        get => '"file://".::url_escape(#path->get# .::SLASH. #file->get#)',
	        type   => 'virtual',
	        depend => 'file path',
	        flags  => 'g',
	    },
	    ...
	    title_or_file => {
	        get =>
	          '(#title->get# eq "" ? (#show_ext# ? #file->display# : #barefilename->display#) : #title->get#)',
	        type     => 'virtual',
	        flags    => 'gcs',
	        width    => 270,
	    ...
	    missing => {flags => 'gan', type => 'integer', bits => 32,}
	    ,    #FIXME store it using a 8-bit relative number to $::DAYNB
	    missingkey => {
	        get => 'join "\\x1D",'
	          . join(',', map("#$_->get#", @MissingKeyFields)),
	        depend => "@MissingKeyFields",
	        type   => 'virtual',
	    ...
	    filetags =>  # debug field : list of the id3v2 frames / vorbis comments
	      { name         => "filetags",
	        ...
	        "id3v2:read" => sub {
	            my $tag = shift;
	            my %res;
	            for my $key ($tag->get_keys) {
	                my @v = $tag->get_values($key);
	                if ($key =~ m/^TXXX$|^COMM$|^WXXX$/) {
	                    my $i = $key eq 'COMM' ? 1 : 0;
	                    $res{"$key;$_->[$i]"} = undef for @v;
	                }
	                else { $res{$key} = undef; }
	            };
	            return [map "id3v2_$_", keys %res];
	        },
	        ...
	        none          => quotemeta
	          "No tags",    #not translated because made for debugging
	        disable => 1,
	      },
	    list => {
	        type  => 'special',
	...
	our %FieldTemplates = (
	    ...
	    rating => {
	        ...
	        edit_many => 1,
	        can_group => 1,
	        options   => 'customfield rw_ useridwarn userid editsubmenu stars',
	        FilterList  => {},
	        postread    => \&FMPS_rating_postread,
	        prewrite    => \&FMPS_rating_prewrite,
	        id3v2       => 'TXXX;FMPS_Rating_User;%v::%i',
	        vorbis      => 'FMPS_RATING_USER::%i',
	        ape         => 'FMPS_RATING_USER::%i',
...
our $OLD_FIELDS =
  'file path modif length size bitrate filetype channel samprate title artist album disc track year version genre comment author added lastplay playcount rating label missing lastskip skipcount';
...
my (%Get, %Display, $DIFFsub, $NEWsub, $LENGTHsub, %UPDATEsub, $SETsub);
my (%Get_gid, %Gid_to_display, %Gid_to_get);
...
sub LookupCode {
    ...
    for my $action (@actions) {
        ...
        if ($c && !ref $c) {
            1
              while $c
              =~ s/#([_0-9a-z:~.]+)#/(grep defined,map $_->{$1}, @hashlist)[0]/ge;

#			$c=~s/#(\w+)->([_0-9a-z:~.]+)(?:\((\w+)=([^)]+)\))?#/LookupCode($1,$2,($3 ? [$3 => $4] : ()))/ge;
            $c
              =~ s/#(?:(\w+)->)?([_0-9a-z:~.]+)(?:\((\w+)=([^)]+)\))?#/LookupCode($1||$field_opt,$2,($3 ? [$3 => $4] : ()))/ge;
            $c =~ s#___#__${field}_#g;
            $c =~ s#([@%\$\#])__(\w+)#($1||'\$').'Songs::Songs_'.$2#ge;
            $c =~ s#__(\w+)#\$Songs::Songs_$1#g;
            $c =~ s/#(\w+)#/exists $vars{$1} ? $vars{$1} : "#$1#"/ge
...
sub MakeCode    #keep ?
{
    ...
    my (@codes) = LookupCode($field, @actions, \@h)
      ;    #warn join(' ',map {defined $_ ? 1 : 0} @codes);
...
sub Field_property {
    ...
    while ($h) {
        ...
        $h = $Types{$type};
        return $Types{"$type.$subtype"}{$key}
          if $subtype
          && $Types{"$type.$subtype"}
          && exists $Types{"$type.$subtype"}{$key};
    }
}
...
sub filter_properties {
    ...
    unshift @totry, "$inv$cmd:$pat", "$cmd:$pat"
...
sub Field_filter_choices {
    ...
    while ($h) {
        for my $key (keys %$h) {
            next
              unless $key =~ m/^filterdesc:(.+)/
              && !$exclude{$1}
              && !$filters{$1};
            my $value = $h->{$key};
            my $f     = $1;
            if (ref $value) {
                if (@$value < 3) { $exclude{$f} = 1; next }
                else             { $value = $value->[1]; }
            }
            else {
                unless ($f =~ m/:/ && $f !~ m/^-/) { $exclude{$f} = 1; next }
            }    # for constant filters eg: filterdesc:e:44100
            $filters{$f} = $value;
        }
        my $type = $h->{parent} || $h->{type};
        last unless $type;
        if (my $e = $h->{filter_exclude}
          )      #list of filters from parent to ignore, 'ALL' for all
        {
            last if $e eq 'ALL';
...
sub filter_prep_numbers_between {
    sort { $a <=> $b } map filter_prep_numbers($_), split / /, $_[0], 2;
}

sub FilterCode {
    my ($field, $cmd, $pat, $inv) = @_;
    my ($code, $convert) =
      LookupCode($field, "filter:$cmd", "filter_prep:$cmd");
    unless ($code) {
        warn "error can't find code for filter $field,$cmd,$pat,$inv\n";
        return 1;
    }
    $convert ||= sub { quotemeta $_[0] };
    unless (ref $convert) {
        $convert =~ s/#PAT#/\$_[0]/g;
        $convert = eval "sub {$convert}";
    }
    $code =~ s/#ID#/\$_/g;
...
sub SortCode {
    ...
    my ($code, $scode, $sicode) = LookupCode(
        $field,
        (   $for_gid
            ? qw/n_sort:gid s_sort:gid si_sort:gid/
            : qw/n_sort s_sort si_sort/
        )
    );
    my $op = "<=>";
    if ($scode) {
        ...
        if ($::Options{Remove_articles} && CanRemoveArticles($field)) {
            $code = "remove_articles($code)";
        }
    }
    my $init = '';
    $init = $1 if $code =~ s/^(.+) +---- +//;
...
sub CompileArticleRE {
    ...
    s/_/ /g
      for @art
      ; #replace _ by spaces to allow multi-word "articles", not sure if it could be useful
...
sub Compile             #currently return value of the code must be a scalar
{
    my ($name, $code) = @_;
    if ($::debug) {
        $::DebugEvaledCode{$name} = $code;
        $code =~ s/^sub \{/sub { local *__ANON__ = 'evaled $name';/;
    }
    my $res = eval $code;
    if ($@) {
        warn
...
sub UpdateFuncs {
    undef %FuncCache;
    delete $Def{$_}{_depended_on_by}, delete $Def{$_}{_properties}
      for keys %Def;
    @Fields = ();
    ...
    while (@todo) {
        ...
        if ($count == @todo) {
            warn
              "Circular field dependencies, can't order these fields : @todo !\n";
            push @Fields, @todo;
            last;
        }
    }
    $Def{$_}{_depended_on_by} = join ' ', keys %{$_depended_on_by{$_}}
      for keys %_depended_on_by;
    $Def{$_}{_properties} = join ' ', keys %{$_properties{$_}}
      for keys %_properties;
    warn "\@Fields=@Fields\n" if $::debug;
    ...
    for my $f (@Fields) {
        if (my $code = Code($f, 'update', ID => '$ID')) {
            $UPDATEsub{$f} =
              Compile("Update_$f" => 'sub { for my $ID (@{$_[0]}) {'
                  . $code
                  . '} }');
        }
    }
    ...
    {
        my $code =
          'my $ID=$_[0]; my $values=$_[1]; my $val; my @changed;' . "\n";
    ...
    # create SET sub
    {
        ...
        for my $f (grep $Def{$_}{flags} =~ m/[aw]/, @Fields) {
            my $set =
              ($Def{$f}{flags} =~ m/w/ && !$::Options{TAG_nowrite_mode})
            ...
            my $c =
              ...
              . "		if (#diff#) { $set }\n" . "	}\n";
            if ($Def{$f}{flags} =~ m/l/
                && !($Def{$f}{flags} !~ m/r/ && $Def{$f}{flags} =~ m/w/)
              ) # edit mode for multi-value fields, exclude write-only or read-on-demand fields (w without r) as this requires knowing the current values
            {
    ...
    {
        my $code =
            'my $size=0; my $sec=0; for my $ID (@{$_[0]}) {'
          . '$size+='
          . Code('size', 'get', ID => '$ID') . ';'
          . '$sec+='
          . Code('length', 'get', ID => '$ID') . ';'
          . '} return ($size,$sec)';
        $LENGTHsub = Compile(Length => "sub {$code}");
    }
    %::ReplaceFields =
      map { '%' . $Def{$_}{letter} => $_ } grep $Def{$_}{letter}, @Fields;
...
#}
    ...
    $::ReplaceFields{'$' . $_} = $::ReplaceFields{'${' . $_ . '}'} =
      $Aliases{$_}
      for keys %Aliases;
...
# begin with a line that checks if a given path-file has already been loaded into the library
    my $pathfile_code =
      '$_[' . $loadedfields{path} . '] ."/". $_[' . $loadedfields{file} . ']';
    my $code =
        '$uniq_check{ '
      . $pathfile_code
      . ' }++ && do { warn "warning: file ".'
      . $pathfile_code
      . '." already in library, skipping.\\n"; return };' . "\n";
    ...
    for my $field (@Fields) {
        ...
        if (defined $i) {
            ...
            $c = Code(
                $field, 'load|set',
                ID  => '$LastID',
                VAL => Code($field, 'default')
            );
        }
        elsif (my $dep = $Def{$field}{depend}) {
            next if grep !exists $loadedfields{$_}, split / /, $dep;
            $c = Code($field, 'update', ID => '$LastID')
              ; #FIXME maybe add {} around it, to avoid multiple my at the same level
            warn "adding update code for $field\n" if $::debug && $c;
        }
        $code .= $c . ";\n" if $c;

        my ($mainfield, $load_extra) =
          LookupCode($field, 'mainfield', 'load_extra', [SGID => '$_[0]']);
        $mainfield ||= $field;
        if ($load_extra && $extradata->{$mainfield} && !$extra_sub{$mainfield})
        {
            ...
            for my $subfield (split /\t/, $extradata->{$mainfield}[0]) {
                my $c = LookupCode($subfield, 'load_extra',
                    [GID => '$gid', VAL => "\$_[$i]"]);
                $code .= "\t$c;\n" if $c;
                $i++;
            }
            $extra_sub{$mainfield} =
              Compile("LoadSub_$mainfield" => "sub {$code}") || sub { };
        }
    }
    $code .= '; return $LastID;';
...
sub MakeSaveSub {
    ...
    for my $field (sort grep $Def{$_}{flags} =~ m/a/, @Fields) {
        next
          if $::Options{Fields_options}{$field}{remove};  #deleted custom field
        my $save_as = $Def{$field}{_renamed_to} || $field;
        push @saved_fields, $save_as;
        push @code,         Code($field, 'save|get', ID => '$_[0]');
        my ($mainfield, $save_extra) =
          LookupCode($field, 'mainfield', 'save_extra');
        if (   $save_extra
            && $Def{$field}{_properties}
            && (!$mainfield || $mainfield eq $field))
        {
            ...
            if (@subfields) {
                ...
                for my $subfield (@subfields) {
                    my $c =
                      LookupCode($subfield, 'save_extra', [GID => '$gid']);
                    push @extra_code, $c;
                }
                $extra_subfields{$save_as} = join ' ',
                  map $Def{$_}{_renamed_to} || $_, @subfields;
                my $code       = $save_extra;
                my $extra_code = join ',', @extra_code;
                $code =~ s/#SUBFIELDS#/$extra_code/g;
                $extra_sub{$save_as} =
                  Compile("SaveSub_$field" => "sub { $code }") || sub { };
            }
        }
    }
...
sub New {
    ...
    if (defined $ID) {
        ReReadFile($ID);
        ::CheckLength($ID)
          if $::Options{LengthCheckMode} eq 'add'
          && Get($ID, 'length_estimated');
...
#warn "\nNewSub(LastID=$LastID)\n";warn join("\n",map("$_=>$values->{$_}",sort keys %$values))."\n";
    ...
    if ($values->{length_estimated} && $::Options{LengthCheckMode} eq 'add') {
        ::CheckLength($ID);
    }
  ...
  # 3=>force check length (and tags)
{
    ...
    if (-e $file) {
        my ($size1, $modif1, $estimated) =
          Songs::Get($ID, qw/size modif length_estimated/);
...
# { my $res=$sub->($val); unless ($res) {warn "Songs::Set : Invalid value '$v' for field $f\n"; next} }
        if ($multi
          ) #multi eq + or - or ^  => add or remove or toggle values (for labels and genres)
        {
            if ($flags !~ m/l/) {
                warn "Songs::Set : Field $f doesn't support multiple values\n";
                next;
            }
            elsif ($flags !~ m/r/ && $flags =~ m/w/) {
                warn
                  "Songs::Set : Can't add/remove/toggle values of multi-value field $f because it is a write-only or read-on-demand field\n";
                next;
            }
            my $array = $values{"+$f"}
              ||= [[], [], []];    #$array contains [[toset],[torm],[toggle]]
...
sub Write {
    ...
    if ($opt{noidle}) { my $c = 1; $c = $write_next->() until $c == 0; }
    else              { Glib::Idle->add($write_next); }
}
...
sub Changed   # 2nd arg contains list of changed fields as a list or a hash ref
{
    ...
    warn "Songs::Changed : IDs=@$IDs fields="
      . join(' ', keys %$changed) . "\n"
      if $::debug;
    $IDFromFile = undef
      if $IDFromFile
      && !$KeepIDFromFile
      && (exists $changed->{file} || exists $changed->{path});
    $MissingHash = undef
      if $MissingHash && grep(exists $changed->{$_}, @MissingKeyFields);
    my @needupdate;
    for my $f (keys %$changed) {
        if (my $l = $Def{$f}{_depended_on_by}) {
            push @needupdate, split / /, $l;
        }
    }
    for my $f (sort @needupdate) {
        next if exists $changed->{$f};
        $changed->{$f} = undef;
        if (my $update = $UPDATEsub{$f}) {
            warn "Updating field : $f\n" if $::debug;
            $update->($IDs);
        }
    }
...
sub CheckMissing {
    ...
    return
         unless defined $song->{title}
      && length $song->{title}
      && (defined $song->{album} || defined $song->{artist});
    for (qw/title album artist track/) {
        $song->{$_} = "" unless defined $song->{$_};
    }
...
#ugly fix, clean-up the fields so they can be compared to those in library, depends on @MissingKeyFields #FIXME should generate a function using #check# and VAL=>'$song->{$field})'
    ...
    if (@$IDs > 1)    #too many candidates, try to find the best one
    {
        ...
        if (@$IDs > 1
          ) #still more than 1, abort, maybe could continue anyway, the files must be nearly identical anyway
        {
            warn "CheckMissing: more than 1 (" . @$IDs
              . ") possible matches for $song-->{path}/$song->{file}, assume identification is unreliable, considering it a new song.\n";
            return;
        }
    }
    for my $oldID (@$IDs) {
        ...
        if (my $pic = Picture($gid, 'album', 'get')) {
            ...
            unless (-e $pic) {
                my $new;
                if ($pic eq $fullfilename
                  )    # check if cover is embedded picture in this file
                {
                ...
                { # if cover was in same folder or a sub-folder, check if there based on new folder
                    ...
                    $new = undef
                      unless $new =~ s#^\Q$oldpath\E#$newpath# && -e $new;
                }
...
#update $IDFromFile, and prevent its destruction in Changed(), not very nice #FIXME make hashes that update themselves when possible
        $KeepIDFromFile = 1;
        $IDFromFile->{$song->{path}}{$song->{file}} =
          delete $IDFromFile->{Get($oldID, 'path')}{Get($oldID, 'file')}
          if $IDFromFile;
...
sub Makesub {
    my $c = &Code;
    warn "Songs::Makesub(@_) called from : " . join(':', caller) . "\n"
      unless $c;
...
sub Picture {
    ...
    unless ($func) {
        ...
        $func = $FuncCache{$action . ' ' . $mainfield}
          ||= $FuncCache{$action . ' ' . $pfield} =
...
sub ListAll {
    my $field = $_[0];
    my $func  = $FuncCache{'listall ' . $field} ||= do {
        if (my $c = Code($field, 'listall')) {
...
sub Get_grouptitle {
    my ($field, $IDs) = @_;
    ($FuncCache{'grouptitle ' . $field}
          ||= Makesub($field, 'grouptitle', ID => '$_[0][0]', IDs => '$_[0]'))
      ->($IDs);
}
...
sub Search_artistid    #return artist id or undef if not found
{
    my $artistname = shift;
    my $field      = 'artist';
    ($FuncCache{'search_gid ' . $field}
          ||= Makesub($field, 'search_gid', VAL => '$_[0]'))->($artistname);
}
...
sub Get_list    #rarely used, keep ?
{
    ...
    my $func = $FuncCache{'getlist ' . $field}
      ||= Makesub($field, 'get_list', ID => '$_[0]');
    $func->($ID);
}

sub Get_icon_list {
    my ($field, $ID) = @_;
    my $func = $FuncCache{"icon_list $field"} ||= Compile(
        "icon_list $field",
        MakeCode(
            $field,
            'sub {grep Gtk2::IconFactory->lookup_default($_), map #icon_for_gid#, @{#get_gid#}; }',
            ID  => '$_[0]',
            GID => '$_'
        )
    );    #FIXME simplify the code-making process
    return $func->($ID);
}
...
sub DisplayFromGID_sub {
    ...
    return $Gid_to_display{$field}
      ||= Makesub($field, 'gid_to_display', GID => '$_[0]');
}

sub DisplayFromHash_sub #not a good name, very specific, only used for $field=path currently
{
    my $field = $_[0];
    return $FuncCache{"DisplayFromHash_sub $field"}
      ||= Makesub($field, 'hash_to_display', VAL => '$_[0]');
}

sub MakeFilterFromGID {
    my ($field, $gid) = @_
      ; #warn "MakeFilterFromGID:@_\n";#warn Code($field, 'makefilter', GID => '$_[0]');
    my $sub = $FuncCache{'makefilter ' . $field}
      ||= Makesub($field, 'makefilter', GID => '$_[0]');
...
sub MakeFilterFromID #should support most fields, FIXME check if works for year/artists/labels/genres/...
{
    ...
    if (my $code = Code($field, 'makefilter_fromID', ID => '$_[0]')
      ) #FIXME optimize : don't call this every time, for example check for a flag that would indicate that this field has a gid
    {
        my $sub = $FuncCache{'makefilter_fromID ' . $field}
          ||= Compile('makefilter_fromID ' . $field, "sub {$code}")
          ;    #FIXME if method doesn't exist
        return Filter->new($sub->($ID));
    }
    else {
        my $gid = Get_gid($ID, $field);
        if (ref $gid) {
            return Filter->newadd(::FALSE, map MakeFilterFromGID($field, $_),
                @$gid);
        }
...
{
    my ($field, $gid) = @_;
    my $sub = $Gid_to_get{$field}
      ||= Makesub($field, 'gid_to_get', GID => '$_[0]');
    if (ref $gid) { return [map $sub->($_), @$gid] }
    return $sub->($gid);
}
...
{          #warn "Songs::Get(@_) called from : ".join(':',caller)."\n";
    ...
    return
      wantarray
      ? map (($Get{$_} || CompileGet($_))->($ID), @_)
      : ($Get{$_[0]} || CompileGet($_[0]))->($ID);
}
...
{              #warn "Songs::Display(@_) called from : ".join(':',caller)."\n";
    ...
    return
      wantarray
      ? map (($Display{$_} || CompileDisp($_))->($ID), @_)
      : ($Display{$_[0]} || CompileDisp($_[0]))->($ID);
}
...
sub CompileGet {
    ...
    unless ($Def{$field}{flags} =~ m/g/) {
        return $Display{$field} = $Get{$field} = sub {
            warn "Songs::Get or Songs::Display : Invalid field '$field'\n";
        };
    }
...
sub Map {
    my ($field, $IDs) =
      @_;    #warn "Songs::Map(@_) called from : ".join(':',caller)."\n";
...
{
    my ($ID, $field, $value) = @_;
    my $sub = $FuncCache{'is_set ' . $field}
      ||= Makesub($field, 'is_set', ID => '$_[0]', VAL => '$_[1]');
    return $sub->($ID, $value);
}
...
sub UniqList #FIXME same as UniqList2. use "string" (for artist) in this one and not in UniqList2 ?
{
    ...
    my $h = BuildHash($field, $IDs, undef, ':uniq')
      ;    #my $h=BuildHash($field,$IDs,'string',':uniq'); ??????
...
sub CompileArtistsRE {
    my $ref1 = $::Options{Artists_split_re}
      ||= ['\s*&\s*', '\s*;\s*', '\s*,\s+', '\s*/\s*'];
    $Artists_split_re = join '|', @$ref1;
    $Artists_split_re ||= '$';
    $Artists_split_re = qr/$Artists_split_re/;

    my $ref2 = $::Options{Artists_title_re}
      ||= ['\(with\s+([^)]+)\)', '\(feat\.\s+([^)]+)\)'];
...
sub ChooseIcon    #FIXME add a way to create a colored square/circle/... icon
{
    my ($field, $gid) = @_;
    my $string =
      ::__x($Def{$field}{icon_edit_string}, name => Gid_to_Get($field, $gid));
    my $file =
    ...
    if ($file eq '0') { }          #unset icon
        ...
        $destfile .= '.png';
        my $pixbuf = GMB::Picture::load($file, size => -48)
          ; # -48 means it will be resized to 48x48 if wifth or height bigger than 48
...
{
    my %args = @_
      ;  # args may be type=> 'flags' or 'rating'  true=> key_that_must_be_true
    my @l = @Fields;
    if (my $type = $args{type}) {
        @l =
          grep { ($Def{$_}{fieldtype} || $Def{$_}{type}) eq $type }
          @l
          ; # currently type flags all have a type=>'flags' in %Def, but might change, so fieldtype can overide it
    }
...
sub FieldType    #currently used to check "flags" or "rating" types
{
    ...
    return $Def{$field}{fieldtype}
      || $Def{$field}{type}
      ; # currently fieldtype is not used but might be useful as $Def{$field}{type} is an implementation detail and not a field property
}
...
sub ListGroupTypes {
    ...
    for my $field (@list) {
        ...
        if ($types) {
            $val =
              [map((qq($field.$_) => "$name ($types->{$_})"), keys %$types)];
        }
        push @ret, $val, $name;
    }
    return \@ret;
...
sub EditFields    #type is one of qw/single many per_id/
{
    ...
    @fields = sort {
        ($Def{$a}{edit_order} || 1000) <=> ($Def{$b}{edit_order} || 1000)
...
sub EditWidget {
    ...
    my ($sub) =
      LookupCode($field, "editwidget:all|editwidget:$type|editwidget");
    unless ($sub) {
        warn "Can't find editwidget for field $field\n";
        return undef;
    }
...
sub ReplaceFields_to_re {
    ...
    if ($field && $Def{$field}{flags} =~ m/e/) {
        return $Def{$field}{_autofill_re}
          ||= '(' . LookupCode($field, 'autofill_re') . ')';
    }
...
sub SortGroup {
    my $f = $_[0];
    $f =~ s#\..*##
      ; #FIXME should be able to sort on "modif.year" and others, but for now, simplfy it into "modif"
...
sub MakeSortCode {
    ...
    for my $s (split / /, $sort) {
        ...
        next unless $field;
        unless ($Def{$field}) {
            warn "Songs::SortList : Invalid field $field\n";
            next;
        }
        unless ($Def{$field}{flags} =~ m/s/) {
            warn "Don't know how to sort $field\n";
            next;
        }
...
sub FindNext # find the song in listref that would be right after ID if ID was in the list #could be optimized by not re-evaluating left-side for every comparison
{
    ...
    my $func = $FuncCache{"FindNext $sort"} ||= do {
        my ($init, $code) = MakeSortCode($sort);
        $code =
            'sub {my $l=$_[0];$a=$_[1]; '
          . $init
          . 'for $b (@$l) { return $b if ('
...
{
    my ($listref, $sort) = @_;
    my $func = $FuncCache{"FindFirst $sort"} ||= do {
        my ($init, $code) = MakeSortCode($sort);
        $code =
            'sub {my $l=$_[0];$a=$l->[0]; '
          . $init
...
sub SortList #FIXME add a few fields (like 'disc track path file') to all sort so that the sort result is constant, ie doesn't depend on the starting order
{
    ...
    my $listref = $_[0];
    my $sort    = $_[1];
    my $func    = $FuncCache{"sort $sort"} ||= do {
        my ($init, $code) = MakeSortCode($sort);
        $code =
            'sub { my $list=shift; '
          . $init
...
sub CopyFields    #copy values from one song to another
{
    ...
    unless (defined $IDsrc) {
        warn "CopyFields : can't find $srcfile in the library\n";
        return 1;
    }
    unless (defined $IDdst) {
        warn "CopyFields : can't find $dstfile in the library\n";
        return 2;
    }
...
sub GetTagValue    #rename ?
{
    ...
    unless ($Def{$field}{flags} =~ m/g/) {
        warn "GetTagValue : invalid field '$field'\n";
        return undef;
    }
    $ID = FindID($ID);
    unless (defined $ID) {
        warn "GetTagValue : song not found\n";
        return undef;
    }
    return Get($ID, $field);
}
...
#unless (exists $Set{$field}) { warn "SetTagValue : invalid field\n"; return ::FALSE }
    $ID = FindID($ID);
    unless (defined $ID) {
        warn "SetTagValue : song not found\n";
        return ::FALSE;
    }
    Set($ID, $field => $value);
...
my %buildhash_deprecated =
  (idlist => 'id:list', count => 'id:count', uniq => ':uniq');

sub BuildHash {
    my ($field, $IDs, $opt, @types) = @_;    #warn "Songs::BuildHash(@_)\n";
    $opt = $opt ? ':' . $opt : '';
    my ($keycode, $multi) = LookupCode(
        $field,
        'hash' . $opt . '|hash',
        'hashm' . $opt . '|hashm',
        [ID => '$ID']
    ...
    $keycode ||= $multi;
    unless ($keycode || $multi) {
        warn "BuildHash error : can't find code for field $field\n";
        return;
    }                                        #could return empty hashes ?
    ...
    for my $type (@types) {
        $i++;
        if ($buildhash_deprecated{$type}) {
            warn
              "BuildHash: using '$type' is deprecated, use '$buildhash_deprecated{$type}' instead\n"
              if ::VERSION > 1.1009 || $::debug;
            $type = $buildhash_deprecated{$type};
        }
        ...
        my $c =
          LookupCode($f, $opt, [ID => '$ID', ($arg ? (ARG => "'$arg'") : ())]);
...
sub GroupSub {
    ...
    return $FuncCache{"GroupSub $field"} ||= do {
        ...
        Compile(
			...
			{	my $v0=' . $code0 . ';
				push @lastrows, map $_-1, grep {'
              . "$code $op "
              . '$v0 and $v0='
              . $code
              . ',1;} $firstrow+1..$lastrow;
          ...
          || sub {
            warn "invalid groupsub (field=$field)";
            my $lastrows = $_[1];
            return [@$lastrows], [0 .. $#$lastrows];
          };
    };
}
...
sub PrefFields    #preference dialog for fields
{
    my $store = Gtk2::TreeStore->new(
        'Glib::String', 'Glib::String', 'Glib::Boolean', 'Glib::Boolean',
        'Glib::Boolean'
    );
    ...
    my @fields =
      grep !$Def{$_}{template}
      && (!$Def{$_}{disable}
        || ($Def{$_}{options} && $Def{$_}{options} =~ m/\bdisable\b/)),
      keys %Def;
    @fields =
         grep !$Def{$_}{property_of}
      && $Def{$_}{name}
      && $Def{$_}{flags} =~ m/[pc]/, @fields;

    #add custom fields
    push @fields, grep $::Options{Fields_options}{$_}{template},
      keys %{$::Options{Fields_options}};
    ...
    for my $cat (sort { $Categories{$a}[1] <=> $Categories{$b}[1] } keys %tree)
    {
        ...
        $store->set($parent, 0, $Categories{$cat}[0], 1, '+' . $cat, 3,
            ::TRUE);    # category node
        for my $field (::sorted_keys($names)) {
            my $opt = $::Options{Fields_options}{$field};
            my $def = $Def{$field};
            my $sensitive =
                exists $opt->{disable} ? $opt->{disable}
              : $def->{disable}        ? 0
              :                          1;
            $store->set(
                $store->append($parent), 0, $names->{$field}, 1,
                $field,                  2, $editable,        3,
                $sensitive,              4, $opt->{remove}
            );          #child
        }
        $custom_root = $store->get_string_from_iter($parent)
          if $cat eq 'custom';
    }
    $treeview->expand_all;
    ...
    $renderer->signal_connect(
        edited => sub {
            ...
            if ($field eq '') {
                ...
                $::Options{Fields_options}{$field} =
                  {template => 'string', name => $newname,};
...
#$Def{$field}= { template => 'string', options => $FieldTemplates{string}{options}, category=>'custom', name=>$newname };
    ...
    $vbox->{gotofunc} = sub    #go to a specific row
    {
        ...
        while ($parent) {
            ...
            while ($child) {
                if ($store->get($child, 1) eq $field) {
                    $treeview->set_cursor($store->get_path($child));
                    return;
                }
...
sub validate_custom_field_name {
    ...
    $field =~ s/_+/_/g;
    $field
      =~ s/_$//; # custom field id restrictions, might be relaxed in the future
    $field = ucfirst $field if $field =~ m/^[a-z]/;
    my %used;
    $used{$_} = undef for keys %Def;
    $used{$Def{$_}{_renamed_to}} = undef
      for grep $Def{$_}{_renamed_to}, keys %Def;
...
our %Field_options = (    #bits	=>
    ...
    resetnotag => {
        ...
        apply => sub {
            my ($def, $opt, $value) = @_;
            $def->{flags} =~ s/_//g;
            $def->{flags} .= '_' if !$value;
        },
        update => sub { $_[0]{widget}->set_sensitive($_[0]{opt}{rw}); }
    ...
    starpreview => {
        ...
        update => sub {
            ...
            my @pix   = map GMB::Picture::pixbuf($files[$_]), 0,
              int($#files / 2), $#files;
    ...
    convwarn => {
        ...
        update => sub    # show only when field to be disabled or removed
        {
            ...
            my $show =
              $arg->{opt}{currentid}
              && ($arg->{opt}{template} ne
                $Def{$arg->{opt}{currentid}}{template});
            my $w = $arg->{widget};
            $w->set_visible($show);
            $w->set_no_show_all(1);
        },
    ...
    datawarn => {
        ...
        update => sub    # show only when field to be disabled or removed
        {
            ...
            my $opt = $arg->{opt};
            my $show =
              $opt->{currentid} && ($opt->{disable} || $opt->{remove});
            my $w = $arg->{widget};
            $w->set_visible($show);
            $w->set_no_show_all(1);
        },
    ...
    useridwarn => {
        ...
        update => sub {
            my $arg  = shift;
            my $show = $arg->{opt}{rw}
              && (!$arg->{opt}{userid} || $arg->{opt}{userid} =~ m/^ *$/);
            my $w = $arg->{widget};
            $w->set_visible($show);
            $w->set_no_show_all(1);
        },
    ...
    persistent_values => {
        ...
        widget  => sub {
            ...
            my $values =
                FieldType($field) eq 'flags'
              ? ListAll($field)
              : []
              ; # check that field is live and live with correct type, else ListAll will not work
            my %valuesh;
            $valuesh{$_} = $_ for @$values;
            ::NewPrefMultiCombo(
                persistent_values => \%valuesh,
                opthash           => $opt,
                ellipsize         => 'end'
            );
        },
...
#$vbox->{opt_orig}={%$opt};	#warning : shallow copy, sub-hash/array stay linked
    ...
    my $def      = $Def{$field};
    my $option_list =
      ($template ? $FieldTemplates{$template}{options} : $def->{options})
      || '';
    my $flags =
      ($template ? $FieldTemplates{$template}{flags} : $def->{flags}) || '';

    my $sg1 = Gtk2::SizeGroup->new('horizontal');
    my %widgets;
    my @topack;
    $vbox->{FieldProperties} =
      1;    #used to get back to $vbox from one of its (grand)child
    ...
    while (my $option = shift @options) {
        ...
        my $value =
          exists $opt->{$key}
          ? $opt->{$key}
          : Field_option_default($field, $option, $base);
        my @extra;

        if ($keep_option && $keep_option eq $option) {
            ($widget, @extra) = @keep_widgets;
        }
        elsif (ref $widget) {
            ...
            $widget = TextCombo->new($ref->{combo}, $value,
                sub { $opt->{$key} = $_[0]->get_value; &Field_Edit_update });
        }
...

    {
        ...
        if ($template)    #custom fields can be renamed
        {
            ...
            $entry->signal_connect(
                changed => sub {
                    my $t = validate_custom_field_name($_[0]->get_text);
                    $_[0]->parent->{newid} = $t;
                    $_[0]->parent->{brename}
                      ->set_sensitive($t && $t ne $field);
                }
            );
            $bedit->signal_connect(clicked => $toggle_edit, 1);
            $bcancel->signal_connect(clicked => $toggle_edit, 0);
            $brename->signal_connect(
                clicked => sub {
                    my $button = shift;
                    my $new    = $button->parent->{newid};
                    $::Options{Fields_options}{$new} =
                      delete($::Options{Fields_options}{$field});
    ...
    if (!$widgets{rw}) {
        ...
        unshift @topack,
          Gtk2::Label->new_with_format('<small>%s</small>', $text)
          if $text;
    }
...
sub Field_Edit_update {
    ...
    my $sensitive = (
        exists $opt->{disable} ? $opt->{disable}
        : ($Def{$field} && $Def{$field}{disable})) ? 0
      : 1;
    my $iter = $store->get_iter($vbox->{path});
    $store->set($iter, 3, $sensitive,   4, $opt->{remove});
    $store->set($iter, 0, $opt->{name}, 1, $field)
      if $opt->{template};    #only for custom fields
}
...
sub Field_Edit_template {
    ...
    my $combo = TextCombo->new(
        ...
        sub {
            ...
            my $vbox  = $combo;
            $vbox = $vbox->parent until $vbox->{FieldProperties};
            Field_fill_option_box($vbox, $field, template => $combo, $label)
              ;    # will reset the option box but keep $combo and $label
            $combo->grab_focus
              if $focus
              ; #reparenting $combo will make it lose focus, so regrab it #FIXME $focus never true
            my $desc = $FieldTemplates{$t}{desc};

            if ($desc) {
                $label->set_markup_with_format("<small><i>%s</i></small>",
                    $desc);
                $label->show;
            }
            else { $label->hide }
    ...
    if ($desc) {
        $label->set_markup_with_format("<small><i>%s</i></small>", $desc);
        $label->show;
    }
    return $combo, $label;
}
...
sub Field_Apply_options {
    for my $field (keys %{$::Options{Fields_options}}) {
        ...
        if (!$def || $def->{template}) {
            ...
            next
              unless
              $template;    # could remove options of removed standard fields ?
            my $hash = $FieldTemplates{$template};
            next unless $hash;
            $def = $Def{$field} = {%$hash,
                name => $opt->{name}};    #shallow copy of the template hash
            $opt->{currentid} = $field;
        }
        ...
        while (my $option = shift @options) {
            ...
            my $value =
              exists $opt->{$key}
              ? $opt->{$key}
              : Field_option_default($field, $option, $def);
...
sub Field_option_default {
    ...
    if (defined $default) {
        $default = $default->($def, $field) if ref $default;
    }
    else {
        my $key = $ref->{editkey} || $option;
        $default = $def->{$key};
...
our %ReplaceFields = (
    '%' => sub {'%'},
    a   => sub {
        my $s = Songs::Gid_to_Display($_[0], $_[1]);
        defined $s ? $s : $_[1];
    }, #FIXME PHASE1 Gid_to_Display should return something $_[1] if no gid_to_display
    l => sub {
    ...
    L => sub { ::CalcListLength(Get('id:list', $_[0], $_[1]), 'length:sum'); }
    ,    #FIXME is CalcListLength needed ?
    y => sub { Get('year:range', $_[0], $_[1]) || ''; },
    Y =>
      sub { my $y = Get('year:range', $_[0], $_[1]); return $y ? " ($y)" : '' }
    ,
    s => sub {
        my $l = Get('id:list', $_[0], $_[1]) || [];
        ::__n('%d song', '%d songs', scalar @$l);
    },
    x => sub {
        my $nb = @{GetXRef($_[0], $_[1])};
        return $_[0] ne 'album'
          ? ::__("%d Album", "%d Albums", $nb)
          : ::__("%d Artist", "%d Artists", $nb);
    },
    X => sub {
        my $nb = @{GetXRef($_[0], $_[1])};
        return
            $_[0] ne 'album' ? ::__("%d Album", "%d Albums", $nb)
          : $nb > 1          ? ::__("%d Artist", "%d Artists", $nb)
          :                    '';
    },
    b => sub {
        if ($_[0] ne 'album') {
            ...
            return @$l == 1
              ? Songs::Gid_to_Display('artist', $l->[0])
              : ::__("%d artist", "%d artists", scalar(@$l));
        }
    },
);
...
#my $u;$u=$format; #DEBUG DELME
    ...
    if ($esc) {
        $format
          =~ s/%([alLyYsxXb%r])/::PangoEsc($ReplaceFields{$1}->($col,$gid))/ge;
    }
...
sub CreateHash {
    my ($type, $field) = @_;
    warn "AA::CreateHash(@_)\n" if $::debug;
    $GHash_Depend{$_}++ for Songs::Depends($type, $field);
    return $GHash{$field}{$type} =
      Songs::BuildHash($field, $::Library, undef, $type);
}
...
sub Get {
    ...
    my $h =
      $GHash{$field}{$type}
      || CreateHash($type, $field)
      ; #warn( 'DEBUG   '.join('//',(keys %$h)[0..10])."...\n key=$key => $h->{$key}\n" );
    return $h->{$key};
}
...
sub GrepKeys {
    ...
    return [@$list]
      unless length $string;    # m// use last regular expression used
...
sub SortKeys {
    ...
    if ($mode eq 'songs') {
        ...
        $pre =
          'string';    #string because values are of the format : "1994 - 2000"
    }
...
sub GuessBestCommonFolder {
    ...
    ($path) = sort { $h->{$b} <=> $h->{$a} } keys %$h
      if length $path < 5;    #take most common if too differents
    return $path;
}
...
INIT {
    ::Watch(undef,
        SongsRemoved => sub { RemoveFromArrays($_[1], \@list_of_SongArray); });
}
...
sub new_from_string {
    my ($class, $string) = @_;
    my @list = map 0 + $_, split / /,
      $string;    #0+$_ to convert to number => use less memory
...
sub updateIDs     #update IDs coming from an older session
{
    ...
    while (my $l = shift @need_update) {
        @$l = grep $_, map $newIDs->[$_], @$l
          ; #IDs start at 1, removed songs get ID=undef and are then removed by the grep $_
    }
}
...
sub save_to_string {
    return join ' ', map $_, @{$_[0]}
      ;    #map $_ so that the numbers are not stringified => use more memory
}
...
sub Shuffle {
    ...
    $self->Replace(
        [map $self->[$_], sort { $rand[$a] <=> $rand[$b] } 0 .. $#$self]);
}
...
sub Remove {
    my ($self, $rows, $IDs) = @_
      ;  #$IDs may be undef and is ignored, just there to make mirroring easier
...
sub Move {
    my ($self, $dest_row, $rows, $dest_row_final) = @_
      ; #$dest_row_final may be undef and is ignored, just there to make mirroring easier
    my @rows = sort { $a <=> $b } @$rows;
    my @IDs;
    ...
    ::HasChanged('SongArray', $self, 'move', $dest_row_orig, \@rows,
        $dest_row);
}
...
sub Up {
    ...
    my $first = 0;
    shift @rows
      while @rows && $rows[0] == $first++;    #remove rows already at the top
    return unless @rows;
...
sub Down {
    ...
    my $last = $#$self;
    pop @rows
      while @rows && $rows[-1] == $last--;   #remove rows already at the bottom
    return unless @rows;
...
INIT {
    ...
    ::Watch(undef,
        SongsHidden =>
          sub { SongArray::RemoveFromArrays($_[1], \@list_of_AutoUpdate); });
}
    ...
    for my $self (grep defined, @list_of_AutoUpdate) {
        ...
        if ($greponly) {
            ...
            if ($toadd) {
                ...
                push @$self, @$toadd;
                if ($Presence{$self}) {
                    vec($Presence{$self}, $_, 1) = 1 for @$IDs;
                }
            ...
            $needupdate{$self} = 2;
            ::IdleDo(
                '7_autoupdate_update' . $self, 6000,
                \&delayed_update_cb,           $self
            );
        }
    }
}
...
sub SongsChanged_cb {
    ...
    for my $self (grep defined, @list_of_AutoUpdate) {
        ...
        elsif ($self->AreIn($IDs)
            && ::OneInCommon($fields, Songs::SortDepends($Sort{$self})))
        {    #re-sort
            $needupdate{$self} = 1;
            $delayed = 1;
        }
        ::IdleDo('7_autoupdate_update' . $self,
            6000, \&delayed_update_cb, $self)
          if $delayed;
    }
}
...
sub init {
    ...
    $::RandomMode =
      $sort =~ m/^random:/ ? Random->new($sort, $::ListPlay) : undef;
    $::SortFields =
      $::RandomMode ? $::RandomMode->fields : Songs::SortDepends($sort);
    ...
    ::Watch(
        undef,
        SongsHidden => sub {
            SongArray::RemoveFromArrays($_[1], [$::ListPlay])
              unless $::ListMode;
        }
    );
...
sub Sort {
    ...
    if ($::RandomMode || $old  =~ m/shuffle/) {
        $::Options{Sort_LastSR} = $old;
    }    # save sort mode for
    elsif ($old ne '') {
        $::Options{Sort_LastOrdered} = $old;
    }    # quick toggle random/non-random
...
{
    my ($self, $sort, $filter) = @_;
    $self->Replace($filter->filter, filter => $filter)
      ; #$self->SetFilter($filter); #FIXME use SetFilter once multi-level filters are implemented
    $self->Sort($sort);
}
...
sub Replace {
    ...
    my $ID = $::SongID;
    $ID = undef
      if defined $ID && $::Options{AlwaysInPlaylist} && !$newlist->IsIn($ID);
    if (!defined $ID) {
...
sub Insert {
    ...
    if (defined $::Position && $::Position >= $destrow) {
        $::Position += @$IDs;
        ::red("position error after insert")
          if $self->[$::Position] != $::SongID;    #DEBUG
    }
...
sub Remove {
    ...
    $self->_staticfy
      unless $fromlibrary; #do not staticfy list for songs removed from library
    $self->SUPER::Remove($rows);
    $::Options{LastPlayFilter} = $::ListMode = SongArray->new_copy($self)
      unless $fromlibrary;
    if (@$self == 0 && $::Options{AlwaysInPlaylist}) {
        $::Position = undef;
        _updateID(undef);
        return;
    }
    if ($::RandomMode) {
        ...
        $self->Next
          if defined $::SongID
          && $::Options{AlwaysInPlaylist}
          && !$self->IsIn($::SongID)
          ;    #skip to next song if current not in playlist
    }
    if (defined $::Position) {
        my $pos  = $::Position;
        my @rows = sort { $a <=> $b } @$rows;
...
sub SongsChanged_cb {
    ...
    return if $::ToDo{'7_refilter_playlist'};
    if (   $::PlayFilter
        && $::PlayFilter->changes_may_affect($IDs, $fields, $::ListPlay))
    {
        ::IdleDo('7_refilter_playlist', 9000, \&UpdateFilter, $::ListPlay);
    }
...
sub UpdateSort {
    ...
    $self->_sort;
    ::HasChanged('SongArray', $self, 'sort', $::Options{Sort}, \@old)
      unless $::RandomMode;
    $::Position   = undef;
    $::ChangedPos = 1;
    _updateID($::SongID);
...
{
    my ($self, $IDs) = @_;
    $self->SUPER::Insert(scalar @$self, $IDs)
      ; # call SUPER::Insert directly instead of SUPER::Push because SUPER::Push calls $self->Insert and thus statify the list
    if ($::RandomMode) {
...
sub SetID {
    ...
    if ($::TogLock && defined $ID && @$self) {
        ...
        if (::IDIsInList($newlist, $ID)
          )    # is in list without lock -> reset lock
        {
            $self->UpdateLock;
            return;
        }
...
sub _filter {
    ...
    my $ID     = $::SongID;
    $filter = Filter->newadd(1, $filter, Filter->newlock($::TogLock, $ID))
      if $::TogLock && defined $ID;
    $::PlayFilter = $filter;
...
sub _updatelock {
    ...
    $::PlayFilter =
        $::ListMode
      ? $lockfilter
      : Filter->newadd(1, $::SelectedFilter, $lockfilter);
...
sub _staticfy {
    ...
    $::PlayFilter = undef;
    if (!$::RandomMode && $::Options{Sort}) {
        $::SortFields                = [];
        $::Options{Sort_LastOrdered} = $::Options{Sort};
        $::Options{Sort}             = '';
        ::QHasChanged('Sort');
    }
}
...
sub update {
    ...
    if (my $icon = $Songs::Def{$field}{icon}) {
        $store->set($store->append, 0, $_, 1, $icon->($_)) for @$list;
    }    #FIXME
...
sub new {
    ...
    if ($Songs::Def{$field}{icon}) {
        ...
        $cell->set_fixed_size(Gtk2::IconSize->lookup('menu'))
          ;    # fixed size => icon or empty space
...
my %NGrepSubs;
my (%CachedTime, %CachedSize, %CachedList);
my $CachedTotal = 0;
our (%InvOp, $OpRe);
INIT {
    my @Oplist = qw( =~ !~   || &&   > <=   < >=   == !=   eq ne  !! !   0 1 )
      ;    #negated operators for negated filters
    ...
    %NGrepSubs = (
        t => sub {
            ...
            $inv =
              ...
              : "0..(($pat>@\$tmp)? \$#\$tmp : " . ($pat - 1) . ')';
            return
                "\$tmp=$lref; Songs::SortList(\$tmp, '"
              . Songs::SortField($field)
              . "' ); $assign @\$tmp[$inv];";
        },
        h => sub {
            my ($field, $pat, $lref, $assign, $inv) = @_;
...
sub new {
    ...
    elsif (ref $string && $string->isa('Filter')) {
...
sub newadd {
	...
	#warn "Filter::newadd(@_) called from : ".join(':',caller)."\n";
	my $self = bless {}, $class;
	my %sel;
	...
	for my $f (@filters) {
		...
		if (ref $f && ref $f eq 'ARRAY') {
			#array format : first value is true(and)/false(or) followed by filters
		...
		if ($string =~ s/$re/$1/)              # a & (b & c) => a & b & c
		{
		    ...
		    if ($string =~ m/^(-)?(\d+)(\D)(.*)$/) {
		        warn "Old filter $string FIXME\n";
		        $string = ($1 || '') . Songs::FieldUpgrade($2) . ":$3:" . $4;
		        warn " => $string\n";
		    }    #PHASE1
		    push @strings,
		      ( ($string =~ m/^\w+:-?~:/)
		...
		if ($and) {
		    ...
		    push @supersets, @{$f->{superset_filters}}
	...
	$self->{superset_filters} = \@supersets
	  unless $sum =~ m#(?:^|\x1D)\w+:-?[th]:#
	  ; #don't use superset optimization for head/tail filters, as they are not commutative
	return $self;
}
...
sub new_from_smartstring {
	...
	while ($string =~ m/\S/) {
		...
		#un-escape escaped negative or OR or ()
		$string =~ s/^\\(?=[-!O|\(\)])//;
		...
		if ($string =~ s&^(\w+(?:\|\w+)*)?(<=|>=|[:<>=~]|#+)&&) {
			...
			if ($fields) {
				...
				# no recognized field => treat $fields and $op as part of the pattern
				if (@f) {
					$fields = join '|', @f;
				} else {
					$string = $fields . $op . $string;
					$fields = $op = undef;
				}
			}

			# un-escape escaped operators at start of string if no recognized operator
			$string =~ s#^\\([:<>=~\#])#$1#g unless $op;
		}
		...
		{
			# pattern begins with a quote
			if ($string =~ s#^(['"])##) {
				my $quote = $1;

				# closing quote followed by "|", ")", space or end of string
				if ($string =~ s#^(.+?)(?<!\\)$quote((?<!\\)[|)]|\s+|$)##) {
					push @patterns, $1;
					...
					# quote is never closed => pretend it is closed at the end of the string
		...
		# un-escape escaped spaces, quotes, | and )
		s#\\([ "'|)])#$1#g for @patterns;
		...
		for my $field (split /\|/, $fields) {
			for my $pattern (@patterns) {
				...
				# special case for ## and ### : use the same function as for #
				my $op1 = $op =~ m/^#/  ?  '#'  :  $op;

				if ($pattern eq '') {
					# must contain operator ':' pattern
					$filter = Songs::Field_property($field, 'smartfilter:' . $op1 . 'empty');
...
# turn 5m into 4.5m..5.5m or 5.2m into 5.15m..5.25m
sub _smartstring_round_range {
	my ($n, $u) = @_;
	my $l = index $n, '.';
...
sub _smartstring_date_moreless {
	...
	if ($pat =~ m/\d[smhdwMy]/) {
		$suffix = 'ago' # relative date filter
	} else {
...
sub _smartstring_number {
	...
	# allow ranges using - unless = with negative number (could also check if field support negative values ?)
	if ($pat !~ m#\.\.# && ($op ne '=' || $pat !~ m/^-\d*\.?\d+[a-zA-Z]?$/)) {
		$pat =~ s/-($|-|\d*\.?\d+[a-zA-Z]?$)/..$1/;
	}
	...
	if (!$is_lowest_unit and my ($n, $u) = $pat =~ m#^(\d*\.?\d+)([a-zA-Z]+)$#i) {
		# =5m turned into 4.5m..5.5m
...
sub _smartstring_date {
	...
	if ($pat =~ m#\d# and ($date1, $date2) = $pat =~ m#^(\d*\.?\d+[smhdwMy])?(?:\.\.|-)(\d*\.?\d+[smhdwMy])?$#i) {
		# relative date filter

		$suffix = 'ago';

		if ($date1 && $date1 !~ m/[1-9]/) {
			$date1 = '';
		} elsif ($date1 && $date2 && $date2 !~ m/[1-9]/) {
			$date2 = $date1;
			$date1 = '';
		}
		...
		$date1 ||= '';
		$date2 ||= '';
	} elsif ($op eq '=' and my ($n, $u) = $pat =~ m#^(\d*\.?\d+)([smhdwMy])$#i) {
		# =5h turned into 4.5h..5.5h
		$suffix = 'ago';
	...
	if ($date1 . $date2 ne '') {
		...
		return
			  ( length $date1 &&  length $date2)  ?  "b$suffix:$date1 $date2"
			: ( length $date1 && !length $date2)  ?  ">$suffix:" . $date1
			: (!length $date1 &&  length $date2)  ?  "<$suffix:" . $date2
			:                                        undef
			;
	}
...
# indicate a possible superset filter that could be used for optimization when the result of $superset_candidate is cached
sub add_possible_superset {
	my $self = shift;
	my $arrayself = $self->to_array;
	my $string1 = $self->{string};

	# ignores filters with head/tail filters
	return if $string1 =~ m#(?:^|\x1D)\w+:-?[th]:#;

	for my $superset_candidate (@_) {
		my $string2 = $superset_candidate->{string};

		# ignores filters with head/tail filters
		next if $string2 =~ m#(?:^|\x1D)\w+:-?[th]:#;

		next if $string2 eq $string1;
		push @{$self->{superset_filters}}, $string2 if _is_subset($superset_candidate->to_array, $arrayself);
	}

	#if (my $l=$self->{superset_filters}) {
	#	my $s = $self->{string}."\n";
	#	$s .= " superset: ". $_ . "\n" for @$l;
	#	$s =~ s/\x1D/**/g;
	#	warn $s;
...
# returns true if $f2 must be a subset of $f1	#$f1 and $f2 must be in array form	#doesn't check for head/tail filters
sub _is_subset {
	my ($f1, $f2) = @_;
	...
	if (!ref $f1 && !ref $f2) {
		...
		if ($op1 eq 's' || $op1 eq 'si') {
			# XXX handle case-i ?
			return index($pat2, $pat1) != -1;
		} elsif ($op1 eq '-s' || $op1 eq '-si') {
			# XXX handle case-i ?
			return index($pat1, $pat2) != -1;
		} elsif ($op1 eq '>' || $op1 eq '-<') {
			return ($pat1 . "\x00" . $pat2) =~ m/^(-?\d*\.?\d+)(\w*)\x00(-?\d*\.?\d+)\2$/ && $3 > $1;
		} elsif ($op1 eq '<' || $op1 eq '->') {
			return ($pat1 . "\x00" . $pat2) =~ m/^(-?\d*\.?\d+)(\w*)\x00(-?\d*\.?\d+)\2$/ && $3 < $1;
		}
	...
	if ($f1->[0] && $f2->[0]) {
		# A & B -> C & D	# each from f1 must be a superset of one from f2
		...
		# A & B -> C | D	# each from f2 must be a subset of in each from f1
		...
		# A | B -> C & D	# one from f2 must be a subset of one from f1
		...
		# A | B -> C | D	# each from f2 must be a subset of one from f1
...
# returns an array form of the filter, first value of array is false for OR, true for AND
sub to_array {
	my $filter = shift;
...
sub _combine_ranges {
	...
	while (@segs) {
		...
		if (defined $s1 && $s3 >= $s1 && $s3 <= $s2 + $step) {
			...
			push @out, [ $s1, $s2 ] if defined $s1; ($s1, $s2) = ($s3, $s4);
		}
	}
...
# combine ranges of consecutive between filters into fewer ranges if possible
sub _between_simplify {
	my ($and, @in) = @_;
	my @strings;
	my ($field, @segs);
	...
	while (@in) {
		...
		# change of filter or no more filters => push combined range
		if (@segs) {
...
# FIXME PHASE1 remove ? use MakeFilterFromID instead
sub newlock {
	my ($class, $field, $ID) = @_;
...
sub filter {
	...
	if ($on_library  &&  !$self->{nocache}  &&  (%CachedList || %IdleFilter::InProgress)) {
		...
		# the result will be cached if it is not already => update timestamp now
		$CachedTime{$string} = time;
...
sub simplify_with_superset_cache {
	...
	if (@supersets) {
		...
		# Take the smaller set, could find the intersection instead
		return [ unpack 'L*', (sort { length $a <=> length $b } @supersets)[0] ];
	}
...
sub cache_result {
	...
	if ($CachedTotal > 100) {
		...
		my $time = time;
		my @del_order = sort {
			($time - $CachedTime{$b}) * $CachedSize{$b} <=> ($time - $CachedTime{$a}) * $CachedSize{$a}
		} keys %CachedSize;
...
# called with $IDs of a SongsAdded event, return true if new songs may match
# the filter.  If greponly filter, returns a ref to a list of new IDs that
# match, or false if none match
sub added_are_in {
	my ($self, $IDs) = @_;
...
# called with $IDs and $fields of a SongsChanged event, return true if the
# changes might require an update.  Tries harder if a songarray is specified
# and a greponly filter
sub changes_may_affect {
	# $songarray argument is optional, currently untested
	my ($self, $IDs, $fields, $songarray) = @_;
...
sub _optimize_with_hashes {
	...
	for my $i (0 .. $#filter) {
		...
		if ($s =~ m/^\(/) { $d++; $or[$d] = ($s eq '(|') ? 1 : 0; }
			...
			while (my ($icc, $h) = each %$vd) {
				...
				my ($ok, $prephash) = Songs::LookupCode(
					$field,
					'filter:h' . $cmd,
					'filter_prephash:' . $cmd,
					[ HREF => '$_[0]' ]
				...
				if ($last - $first == $#$l && $filter[$first - 1] && $filter[$first - 1] =~ m/^\(/ &&
				    $filter[$last + 1] eq ')')
				{
				    push @$l, $first - 1, $last + 1;
				} # add ( && ) to the removed filters if all those inside are replaced by the hash
	...
	my $filter = join "\x1D", grep defined, @filter;
	#warn "$_\n" for @filter;
...
sub makesub {
	...
	if ($filter eq '') {
		$self->{greponly} = $self->{nocache} = 1;

		return $self->{'sub'} = sub { $_[0] };
	} elsif ($filter eq 'null') {
		$self->{greponly} = $self->{nocache} = 1;
	...
	if ($filter =~ m#(?:^|\x1D)\w+:-?[th]:#) {
		$func = makesub_Ngrep($filter, $depends);
	} else {
		$self->{greponly} = 1;
	...
	my $before = '';
	if ($hashes) {
		$before .= "my \$hash$_=\$hashes->[$_];" for 0 .. $#$hashes;
	}
	...
	if ($@) {
		warn "filter error :\n code:\n$before; sub {$func}\n error:\n$@";
		$sub = sub { $_[0] };
	}; #return empty filter if compilation error
	return $self->{'sub'} = $sub;
}
...
sub makesub_Ngrep {
	...
	for my $f (@filter) {
		if ($f =~ m/^[\(\)]/) {
			...
			unless ($cmd) {
				warn "Invalid filter : $field $cmd $pat\n";
...
sub _explain_element {
	...
	for my $pat (@patterns) {
		...
		if (my $display = $opt2{display}) {
			$pat = $display->($pat);
		} elsif (($unit || $round) && $pat =~ m/^(-?\d*\.?\d+)([a-zA-Z]*)$/) {
			# numbers
			...
			if (ref $unit) {
				# \%::SIZEUNITS, \%::DATEUNITS or %::TIMEUNITS

				if ($letter && $unit->{$letter}) {
					$unit = $unit->{$letter}[1];
				} else {
					$unit = undef;
				}
...
sub QuoteRegEx {
	...
	# check if re valid, else quote everything
	if (!eval { qr/$_/; }) {
		warn "invalid regular expression \"$_[0]\" : $@\n" if $::debug;
...
sub smartstring_fuzzy {
	...
	my $threshold =
		  $pat =~ s/([<>])(\d?\d)$//  ?  ($1 eq '>' ? $2 : int(100 - 100 * $2 / length($pat)) - 1)
		: $op eq '#'                  ?  80
		: $op eq '##'                 ?  70
		:                                60
		;
...
sub _fuzzy_match {
	...
	if ($min > .35) {
		...
		# the main problem of this method is that longer string2 have
		# higher scores, dividing by string2's length is not possible
		# if you want to search sof substrings of string2
	}

	# local levenshtein distance (much slower)
	# note that some strings with low enough distance might have already
	# been discarded because they didn't have enough small substrings in
	# common
	...
	for my $j (1 .. length $string2) {
		for my $i (1 .. $length1) {
			...
			if (substr($string1, $i - 1, 1) eq substr($string2, $j - 1, 1) ||
			    substr($string1, $i - 1, 1) eq "'" ||
			    substr($string2, $j - 1, 1) eq "'") # treat single quote as equal to any character
			{
				my $ident = $mat[$i - 1][$j - 1];
				if ($i == $length1) {
					my $ins = $mat[$i][$j - 1];
					$ident = $ins if $ins < $ident;
				}
				$mat[$i][$j] = $ident;
			} else {
				my $del = $mat[$i - 1][$j] + 1;
...
sub start {
	...
	# 1000 is very low priority (default is 200, Glib::G_PRIORITY_LOW is 300)
	$self->{idle_handle} = Glib::Idle->add(\&filter_some, $self, 1000);

	$self->{count}++;
}

sub is_cached {
	$_[0]{filter}->is_cached;
}
...
sub get_progress {
	...
	unless ($progress) {
		...
		if ($self->{count}++ > 5 || $self->{started} > 15 + time) {
			# give up if progress reset too many times or if it has been too long
			$self->abort;
			return 0;
		}
		...
		$todo ||= [@$::Library];
		$progress = $InProgress{$filter->{string}} = {
			found => [],
			todo => $todo
		};
	}
...
sub filter_some {
	...
	# progress is 0 if given up
	if (!$progress) {
		return $self->{idle_handle} = 0;
	}
...
INIT {
	%ScoreTypes = (
		...
		r => {
			...
			#score => sub { my ($value,$extra)=@_;my @l=split /,/,$extra; return undef unless @l==11; return '('.$extra.')[int('.$value.'/10)]' },
		},
	);
}
...
sub make {
	...
	for my $s (split /\x1D/, $self->{string}) {
		...
		# indicate that the values must be recomputed as time pass
		$self->{time_computed} = 1 if $ScoreTypes{$type}{time_dependant};
	...
	$self->{before}  = '';
	if ($self->{hashes}) {
		$self->{before} .= "my \$hash$_=\$self->{hashes}[$_];" for 0 .. $#{$self->{hashes}};
	}
...
sub MakeGroupScoreFunction {
	...
	unless ($keycode || $multi) {
		warn "MakeGroupScoreFunction error : can't find code for field $field\n";

		return;

		# XXX return dummy sub ?
	}

	($keycode, my $keyafter) = split / +---- +/, $keycode || $multi, 2;
	if ($keyafter) {
		warn "MakeGroupScoreFunction with field $field is not supported yet\n";
		return;
	...
	my $calcIDscore =
		$multi
			? 'my $IDscore=' . $score . '; for my $key (' . $keycode . ') {$score{$key}+=$IDscore}'
			: "\$score\{$keycode}+=$score;"
			;
...
sub Draw {
	...
	my (@list, $sum);
	if ($no_list) {
		# list of IDs that must not be picked
		my %no;
		$no{$_}++ for @$no_list;
		@list = grep !$no{$_}, @$lref;
		$sum  = 0;
...
NEXTDRAW: while ($nb > 0) {
	...
	if ($nb && @list) {
		# if still need more -> select at random (no weights)

		$nb = @list if $nb > @list;
		my @rand;
		push @rand,  rand for @list;
...
sub MakeTab {
	...
	my $func =
		$before
		. 'my $sum; my @tab = (0) x ' . $nbcols
		. '; for (@$::ListPlay) { $sum += my $s = ' . $score . '; $tab[int(.5+' . ($nbcols - 1) . '*$s)]++;}'
		. '; return \@tab,$sum;'
		;
...
sub MakeExample {
	...
	my $string_value =
		$ScoreTypes{$type}{boolean}
			? $v
			: ::format_number($v, $round) . " $unit";
